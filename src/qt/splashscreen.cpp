...
SplashScreen::SplashScreen(const NetworkStyle *networkStyle) : QWidget() {
  ...
  // Text Color: White/Light Gray for Dark Background
  pixPaint.setPen(QColor(230, 230, 230));

  // draw a slightly radial gradient (Dark Mode)
  QRadialGradient gradient(QPoint(0, 0), splashSize.width() / devicePixelRatio);
  gradient.setColorAt(0, QColor(40, 40, 40)); // Inner Dark Gray
  gradient.setColorAt(1, QColor(20, 20, 20)); // Outer Almost Black
  QRect rGradient(QPoint(0, 0), splashSize);
  pixPaint.fillRect(rGradient, gradient);

  // Draw BitMinti Logo centered/fitted (Logic Fixed for Retina)
  const QSize originalSize(1024, 1024);
  QPixmap icon(networkStyle->getAppIcon().pixmap(originalSize));

  // Logical Canvas is 480x320. We define a target box of 400x240 to leave room
  // for text.
  QSize maxLogicalSize(400, 240);
  QSize targetSize = icon.size().scaled(maxLogicalSize, Qt::KeepAspectRatio);

  // Center coordinates (Logical)
  int x = (480 - targetSize.width()) / 2;
  // Shift slightly up to leave room for bottom copyright text
  int y = ((320 - targetSize.height()) / 2) - 10;

  QRect drawRect(x, y, targetSize.width(), targetSize.height());
  pixPaint.drawPixmap(drawRect, icon);
...
static void ShowProgress(SplashScreen *splash, const std::string &title,
  ...
  InitMessage(
      ...
      title + std::string("\n") +
          (resume_possible
               ? SplashScreen::tr("(press q to shutdown and continue later)")
                     .toStdString()
...
void SplashScreen::subscribeToCoreSignals() {
  ...
  m_handler_init_wallet =
      m_node->handleInitWallet([this]() { handleLoadWallet(); });
}
...
void SplashScreen::handleLoadWallet() {
  ...
  if (!WalletModel::isWalletEnabled())
    return;
  m_handler_load_wallet = m_node->walletLoader().handleLoadWallet(
