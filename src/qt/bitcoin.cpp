...
static void RegisterMetaTypes() {
  ...
  // https://doc.qt.io/qt-5/qmetatype.html#qRegisterMetaType) IMPORTANT: if
  // CAmount is no longer a typedef use the normal variant above (see
...
static bool ErrorSettingsRead(const bilingual_str &error,
  ...
  QMessageBox messagebox(
      QMessageBox::Critical, CLIENT_NAME,
...
static const char *qt_argv = "bitminti-qt";
  ...
  if (!optionsModel->Init(error)) {
    ...
    if (gArgs.GetSettingsPath(&settings_path)) {
      ...
      std::string quoted_path =
          strprintf("%s", fs::quoted(fs::PathToString(settings_path)));
      error.original += strprintf(
          "Settings file %s might be corrupt or invalid.", quoted_path);
      error.translated += tr("Settings file %1 might be corrupt or invalid.")
                              .arg(QString::fromStdString(quoted_path))
                              .toStdString();
    }
    InitError(error);
...
bool BitcoinApplication::baseInitialize() { return node().baseInitialize(); }
...
void BitcoinApplication::startThread() {
  ...
  connect(&m_executor.value(), &InitExecutor::initializeResult, this,
          &BitcoinApplication::initializeResult);
  connect(&m_executor.value(), &InitExecutor::shutdownResult, this,
          [] { QCoreApplication::exit(0); });
  connect(&m_executor.value(), &InitExecutor::runawayException, this,
          &BitcoinApplication::handleRunawayException);
  connect(this, &BitcoinApplication::requestedInitialize, &m_executor.value(),
          &InitExecutor::initialize);
  connect(this, &BitcoinApplication::requestedShutdown, &m_executor.value(),
...
void BitcoinApplication::initializeResult(
  ...
  if (success) {
    ...
    // If '-min' option passed, start window minimized (iconified) or minimized
    // to tray
    ...
    if (WalletModel::isWalletEnabled()) {
      ...
      window->setWalletController(m_wallet_controller,
                                  /*show_loading_minimized=*/start_minimized);
      if (paymentServer) {
...
void BitcoinApplication::handleRunawayException(const QString &message) {
  QMessageBox::critical(nullptr, tr("Runaway exception"),
                        tr("A fatal error occurred. %1 can no longer continue "
                           "safely and will quit.")
                                .arg(CLIENT_NAME) +
                            QLatin1String("<br><br>") +
                            GUIUtil::MakeHtmlLink(message, CLIENT_BUGREPORT));
  ::exit(EXIT_FAILURE);
}
...
void BitcoinApplication::handleNonFatalException(const QString &message) {
  ...
  QMessageBox::warning(
      ...
      tr("An internal error occurred. %1 will attempt to continue safely. This "
         "is "
...
static void SetupUIArgs(ArgsManager &argsman) {
  argsman.AddArg("-choosedatadir",
                 strprintf("Choose data directory on startup (default: %u)",
                           DEFAULT_CHOOSE_DATADIR),
                 ArgsManager::ALLOW_ANY, OptionsCategory::GUI);
  argsman.AddArg("-lang=<lang>",
  ...
  argsman.AddArg("-splash",
                 strprintf("Show splash screen on startup (default: %u)",
...
int GuiMain(int argc, char *argv[]) {
  ...
  if (!gArgs.ParseParameters(argc, argv, error)) {
    ...
    // Create a message box, because the gui has neither been created nor has
    // subscribed to core signals
    QMessageBox::critical(
        nullptr, CLIENT_NAME,
        // message cannot be translated because translations have not been
        // initialized
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (payment_server_token_seen && arg.startsWith("-")) {
      InitError(Untranslated(strprintf(
          "Options ('%s') cannot follow a BIP-21 payment URI", argv[i])));
      QMessageBox::critical(
          nullptr, CLIENT_NAME,
          // message cannot be translated because translations have not been
          // initialized
          QString::fromStdString(
              "Options ('%1') cannot follow a BIP-21 payment URI")
              .arg(QString::fromStdString(argv[i])));
      return EXIT_FAILURE;
    }
    if (invalid_token) {
      InitError(
          Untranslated(strprintf("Command line contains unexpected token '%s', "
                                 "see bitcoin-qt -h for a list of options.",
                                 argv[i])));
      QMessageBox::critical(
          nullptr, CLIENT_NAME,
          // message cannot be translated because translations have not been
  ...
  if (!Intro::showIfNeeded(did_show_intro, prune_MiB))
    return EXIT_SUCCESS;
  ...
  if (auto error = common::InitConfig(gArgs, ErrorSettingsRead)) {
    ...
    if (error->status == common::ConfigStatus::FAILED_WRITE) {
      ...
      QMessageBox::critical(
          nullptr, CLIENT_NAME,
          QObject::tr("Error: %1")
              .arg(QString::fromStdString(error->message.translated)));
    }
    return EXIT_FAILURE;
  }
#ifdef ENABLE_WALLET
  ...
  // is used in the name of the server.
  ...
  // Install global event filter that makes sure that out-of-focus labels do not
  // contain text cursor.
  app.installEventFilter(new GUIUtil::LabelOutOfFocusEventFilter(&app));
#if defined(Q_OS_WIN)
  // Install global event filter for processing Windows session related Windows
  // messages (WM_QUERYENDSESSION and WM_ENDSESSION) Note: it is safe to call
  // app.node() in the lambda below despite the fact that app.createNode()
  // hasn't been called yet, because native events will not be processed until
  // the Qt event loop is executed.
  ...
  try {
    ...
    // thread This is acceptable because this function only contains steps that
    // are quick to execute, so the GUI thread won't be held up.
    if (app.baseInitialize()) {
      app.requestInitialize();
#if defined(Q_OS_WIN)
      WinShutdownMonitor::registerShutdownBlockReason(
          QObject::tr("%1 didn't yet exit safely…").arg(CLIENT_NAME),
