...
QString BitcoinUnits::longName(Unit unit) {
  switch (unit) {
  case Unit::BTC:
    return QString("BITMINTI");
  case Unit::mBTC:
    return QString("mBITMINTI");
  case Unit::uBTC:
    return QString::fromUtf8("µBITMINTI (bits)");
  case Unit::SAT:
    return QString("Minti (mnt)");
...
QString BitcoinUnits::description(Unit unit) {
  ...
  case Unit::SAT:
    return QString("Satoshi (sat) (1 / 100" THIN_SP_UTF8 "000" THIN_SP_UTF8
                   "000)");
  } // no default case, so the compiler can warn about missing cases
  assert(false);
}
...
QString BitcoinUnits::format(Unit unit, const CAmount &nIn, bool fPlus,
  ...
  if (justify) {
    quotient_str =
        quotient_str.rightJustified(MAX_DIGITS_BTC - num_decimals, ' ');
  }
...
QString BitcoinUnits::formatHtmlWithUnit(Unit unit, const CAmount &amount,
                                         bool plussign,
...
QString BitcoinUnits::formatWithPrivacy(Unit unit, const CAmount &amount,
                                        SeparatorStyle separators,
                                        bool privacy) {
  assert(amount >= 0);
  QString value;
