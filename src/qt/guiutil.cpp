...
static std::string DummyAddress(const CChainParams &params) {
  ...
  if (Assume(!IsValidDestinationString(addr)))
    return addr;
  return {};
}
...
void AddButtonShortcut(QAbstractButton *button, const QKeySequence &shortcut) {
  QObject::connect(new QShortcut(shortcut, button), &QShortcut::activated,
                   [button]() { button->animateClick(); });
}
...
QString formatBitcoinURI(const SendCoinsRecipient &info) {
  bool bech_32 = info.address.startsWith(
      QString::fromStdString(Params().Bech32HRP() + "1"));

  QString ret = QString("bitcoin:%1")
                    .arg(bech_32 ? info.address.toUpper() : info.address);
  int paramCount = 0;

  if (info.amount) {
    ret += QString("?amount=%1")
               .arg(BitcoinUnits::format(BitcoinUnit::BTC, info.amount, false,
...
bool isDust(interfaces::Node &node, const QString &address,
...
QString getDefaultDataDirectory() { return PathToQString(GetDefaultDataDir()); }
...
bool openBitcoinConf() {
  ...
  bool res =
      QDesktopServices::openUrl(QUrl::fromLocalFile(PathToQString(pathConfig)));
#ifdef Q_OS_MACOS
  // Workaround for macOS-specific behavior; see #15409.
  if (!res) {
    res = QProcess::startDetached("/usr/bin/open",
                                  QStringList{"-t", PathToQString(pathConfig)});
  }
#endif
...
fs::path static StartupShortcutPath() {
  ...
  if (chain == ChainType::MAIN)
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BitMinti.lnk";
  if (chain == ChainType::TESTNET) // Remove this special case when testnet
                                   // CBaseChainParams::DataDir() is incremented
                                   // to "testnet4"
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BitMinti (testnet).lnk";
  return GetSpecialFolderPath(CSIDL_STARTUP) /
         fs::u8path(strprintf("BitMinti (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup() {
  // check for BitMinti*.lnk
...
fs::path static GetAutostartFilePath() {
  ChainType chain = gArgs.GetChainType();
  if (chain == ChainType::MAIN)
    return GetAutostartDir() / "bitminti.desktop";
  return GetAutostartDir() /
         fs::u8path(strprintf("bitminti-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart) {
  if (!fAutoStart)
    ...
    std::ofstream optionFile{GetAutostartFilePath().std_path(),
                             std::ios_base::out | std::ios_base::trunc};
    if (!optionFile.good())
      return false;
    ChainType chain = gArgs.GetChainType();
    // Write a bitminti.desktop file to the autostart directory:
    optionFile << "[Desktop Entry]\n";
    optionFile << "Type=Application\n";
    if (chain == ChainType::MAIN)
      optionFile << "Name=BitMinti\n";
    else
      optionFile << strprintf("Name=BitMinti (%s)\n", ChainTypeToString(chain));
...
QString NetworkToQString(Network net) {
  ...
  case NET_IPV4:
    return QObject::tr("IPv4", "network name");
  //: Name of IPv6 network in peer info
  case NET_IPV6:
    return QObject::tr("IPv6", "network name");
  //: Name of Tor network in peer info
  case NET_ONION:
    return QObject::tr("Onion", "network name");
  //: Name of I2P network in peer info
  case NET_I2P:
    return QObject::tr("I2P", "network name");
  //: Name of CJDNS network in peer info
  case NET_CJDNS:
    return QObject::tr("CJDNS", "network name");
  case NET_INTERNAL:
    return "Internal"; // should never actually happen
  case NET_MAX:
    assert(false);
  } // no default case, so the compiler can warn about missing cases
  assert(false);
...
QString ConnectionTypeToQString(ConnectionType conn_type,
  ...
  case ConnectionType::OUTBOUND_FULL_RELAY:
    return prefix + QObject::tr("Full Relay");
  /*: Peer connection type that relays network information about
      blocks and not transactions or addresses. */
  case ConnectionType::BLOCK_RELAY:
    return prefix + QObject::tr("Block Relay");
  //: Peer connection type established manually through one of several methods.
  case ConnectionType::MANUAL:
    return prefix + QObject::tr("Manual");
  //: Short-lived peer connection type that tests the aliveness of known
  //: addresses.
  case ConnectionType::FEELER:
    return prefix + QObject::tr("Feeler");
  //: Short-lived peer connection type that solicits known addresses from a
  //: peer.
  case ConnectionType::ADDR_FETCH:
    return prefix + QObject::tr("Address Fetch");
  } // no default case, so the compiler can warn about missing cases
  assert(false);
}
...
QString formatDurationStr(std::chrono::seconds dur) {
  ...
  const auto s{
      std::chrono::duration_cast<std::chrono::seconds>(dur - d - h - m)};
  QStringList str_list;
  if (auto d2{d.count()})
    str_list.append(QObject::tr("%1 d").arg(d2));
  if (auto h2{h.count()})
    str_list.append(QObject::tr("%1 h").arg(h2));
  if (auto m2{m.count()})
    str_list.append(QObject::tr("%1 m").arg(m2));
  const auto s2{s.count()};
  if (s2 || str_list.empty())
    str_list.append(QObject::tr("%1 s").arg(s2));
...
QString FormatPeerAge(std::chrono::seconds time_connected) {
  ...
  if (age >= 24h)
    return QObject::tr("%1 d").arg(age / 24h);
  if (age >= 1h)
    return QObject::tr("%1 h").arg(age / 1h);
  if (age >= 1min)
    return QObject::tr("%1 m").arg(age / 1min);
...
QString formatPingTime(std::chrono::microseconds ping_time) {
  return (ping_time == std::chrono::microseconds::max() || ping_time == 0us)
             ? QObject::tr("N/A")
             : QObject::tr("%1 ms").arg(QString::number(
...
void ThemedLabel::setThemedPixmap(const QString &image_filename, int width,
                                  int height) {
...
void ThemedLabel::updateThemedPixmap() {
  setPixmap(m_platform_style->SingleColorIcon(m_image_filename)
                .pixmap(m_pixmap_width, m_pixmap_height));
}

ClickableLabel::ClickableLabel(const PlatformStyle *platform_style,
                               QWidget *parent)
...
void PolishProgressDialog(QProgressDialog *dialog) {
  ...
  // for steps), and only shows itself if that estimate is beyond
...
QDateTime StartOfDay(const QDate &date) { return date.startOfDay(); }
...
QString MakeHtmlLink(const QString &source, const QString &link) {
  return QString(source).replace(link, QLatin1String("<a href=\"") + link +
                                           QLatin1String("\">") + link +
                                           QLatin1String("</a>"));
}

void PrintSlotException(const std::exception *exception, const QObject *sender,
