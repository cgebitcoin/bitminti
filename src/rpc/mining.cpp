 ...
 * If 'height' is a valid block height, compute the estimate at the time when a
 * given block was found.
 */
  ...
  return RPCHelpMan{
      ...
      "certain block was found.\n",
      {
          {"nblocks", RPCArg::Type::NUM, RPCArg::Default{120},
           "The number of previous blocks to calculate estimate from, or -1 "
           "for blocks since last difficulty change."},
...
static bool GenerateBlock(ChainstateManager &chainman, CBlock &&block,
  ...
  if (!chainman.ProcessNewBlock(block_out, /*force_processing=*/true,
...
static bool getScriptFromDescriptor(std::string_view descriptor,
  ...
  // Combo descriptors can have 2 or 4 scripts, so we can't just check
  // scripts.size() == 1
...
static RPCHelpMan generatetodescriptor() {
  return RPCHelpMan{
      ...
      [&](const RPCHelpMan &self, const JSONRPCRequest &request) -> UniValue {
        ...
        std::string error;
        if (!getScriptFromDescriptor(self.Arg<std::string_view>("descriptor"),
                                     coinbase_output_script, error)) {
...
static RPCHelpMan generate() {
  return RPCHelpMan{
      "generate",
      "has been replaced by the -generate cli option. Refer to -help for more "
      "information.",
      {},
      {},
      RPCExamples{""},
...
static RPCHelpMan generatetoaddress() {
  return RPCHelpMan{
      ...
      RPCExamples{"\nGenerate 11 blocks to myaddress\n" +
                  ...
                  " wallet, you can get a new address to send the newly "
...
static RPCHelpMan generateblock() {
  return RPCHelpMan{
      "generateblock",
      "Mine a set of ordered transactions to a specified address or descriptor "
      "and return the block hash.",
      {
          ...
          {"submit", RPCArg::Type::BOOL, RPCArg::Default{true},
           "Whether to submit the block before the RPC call returns or to "
           "return it as hex."},
      },
      RPCResult{
          RPCResult::Type::OBJ,
      ...
      [&](const RPCHelpMan &self, const JSONRPCRequest &request) -> UniValue {
        ...
        if (!getScriptFromDescriptor(address_or_descriptor,
                                     coinbase_output_script, error)) {
        ...
        for (size_t i = 0; i < raw_txs_or_txids.size(); i++) {
          ...
          if (auto txid{Txid::FromHex(str)}) {
            ...
            throw JSONRPCError(
                ...
                strprintf("Transaction decode failed for %s. Make sure the tx "
        ...
        ChainstateManager &chainman = EnsureChainman(node);
        {
          ...
          if (BlockValidationState state{TestBlockValidity(
                  chainman.ActiveChainstate(), block, /*check_pow=*/false,
  ...
  return RPCHelpMan{
      ...
      RPCResult{
          ...
          "",
          {
              ...
              {RPCResult::Type::NUM, "currentblockweight", /*optional=*/true,
               "The block weight (including reserved weight for block header, "
               "txs count and coinbase tx) of the last assembled block (only "
               "present if a block was ever assembled)"},
              {RPCResult::Type::NUM, "currentblocktx", /*optional=*/true,
               "The number of block transactions (excluding coinbase) of the "
               "last assembled block (only present if a block was ever "
               "assembled)"},
              {RPCResult::Type::STR_HEX, "bits",
               "The current nBits, compact representation of the block "
               "difficulty target"},
              ...
              {RPCResult::Type::STR_AMOUNT, "blockmintxfee",
               "Minimum feerate of packages selected for block inclusion in " +
                   CURRENCY_UNIT + "/kvB"},
              {RPCResult::Type::STR, "chain",
               "current network name (" LIST_CHAIN_NAMES ")"},
              {RPCResult::Type::STR_HEX, "signet_challenge", /*optional=*/true,
               "The block challenge (aka. block script), in hexadecimal (only "
               "present if the current network is a signet)"},
              {RPCResult::Type::OBJ,
               "next",
               "The next block",
              ...
              (IsDeprecatedRPCEnabled("warnings")
                   ? RPCResult{RPCResult::Type::STR, "warnings",
                               "any network and blockchain warnings "
                               "(DEPRECATED)"}
                   : RPCResult{RPCResult::Type::ARR,
                               "warnings",
                               "any network and blockchain warnings (run with "
                               "`-deprecatedrpc=warnings` to return the latest "
                               "warning as a single string)",
                               {
      ...
      [&](const RPCHelpMan &self, const JSONRPCRequest &request) -> UniValue {
        ...
        obj.pushKV("target",
                   GetTarget(tip, chainman.GetConsensus().powLimit).GetHex());
        ...
        next.pushKV(
            "target",
            GetTarget(next_index, chainman.GetConsensus().powLimit).GetHex());
        ...
        obj.pushKV("warnings",
                   node::GetWarningsForRpc(*CHECK_NONFATAL(node.warnings),
                                           IsDeprecatedRPCEnabled("warnings")));
        return obj;
      },
  };
  ...
  return RPCHelpMan{
      ...
      "priority\n",
      {
          ...
          {"fee_delta", RPCArg::Type::NUM, RPCArg::Optional::NO,
           ...
           "                  Note, that this value is not a fee rate. It is a "
           "value to modify absolute fee of the TX.\n"
      ...
      [&](const RPCHelpMan &self, const JSONRPCRequest &request) -> UniValue {
        ...
        if (dummy && *dummy != 0) {
          throw JSONRPCError(RPC_INVALID_PARAMETER,
                             "Priority is no longer supported, dummy argument "
                             "to prioritisetransaction must be 0.");
        }

        CTxMemPool &mempool = EnsureAnyMemPool(request.context);

        // Non-0 fee dust transactions are not allowed for entry, and
        // modification not allowed afterwards
        const auto &tx = mempool.get(txid);
        if (mempool.m_opts.require_standard && tx &&
            !GetDust(*tx, mempool.m_opts.dust_relay_feerate).empty()) {
          throw JSONRPCError(
...
static RPCHelpMan getprioritisedtransactions() {
  return RPCHelpMan{
      ...
      "Returns a map of all user-created (see prioritisetransaction) fee "
      "deltas by txid, and whether the tx is present in mempool.",
      {},
      RPCResult{
          ...
          {{RPCResult::Type::OBJ,
            ...
            "",
            {
                {RPCResult::Type::NUM, "fee_delta",
                 "transaction fee delta in satoshis"},
                {RPCResult::Type::BOOL, "in_mempool",
                 "whether this transaction is currently in mempool"},
                {RPCResult::Type::NUM, "modified_fee", /*optional=*/true,
                 "modified fee in satoshis. Only returned if in_mempool=true"},
            }}},
      },
      RPCExamples{HelpExampleCli("getprioritisedtransactions", "") +
...
static std::string gbt_rule_value(const std::string &name,
      ...
      {
          {
              ...
              {
                  {"mode", RPCArg::Type::STR,
                   /* treat as named arg */ RPCArg::Optional::OMITTED,
                  ...
                  {"capabilities",
                   ...
                   "A list of strings",
                   {
                       {"str", RPCArg::Type::STR, RPCArg::Optional::OMITTED,
                  ...
                  {"rules",
                   ...
                   "A list of strings",
                   {
                       {"segwit", RPCArg::Type::STR, RPCArg::Optional::NO,
                        "(literal) indicates client side segwit support"},
                       {"str", RPCArg::Type::STR, RPCArg::Optional::OMITTED,
                        "other client side supported softfork deployment"},
                   }},
                  {"longpollid", RPCArg::Type::STR, RPCArg::Optional::OMITTED,
                   "delay processing request until the result would vary "
                   "significantly from the \"longpollid\" of a prior template"},
                  {"data", RPCArg::Type::STR_HEX, RPCArg::Optional::OMITTED,
                   "proposed block data to check, encoded in hexadecimal; "
                   "valid only for mode=\"proposal\""},
              },
          },
      },
      {
          ...
          RPCResult{
              ...
              "",
              {
                  ...
                  {RPCResult::Type::ARR,
                   ...
                   {
                       {RPCResult::Type::STR, "",
                        "name of a rule the client must understand to some "
                        "extent; see BIP 9 for format"},
                   }},
                  {RPCResult::Type::OBJ_DYN,
                   "vbavailable",
                   "set of pending, supported versionbit (BIP 9) softfork "
                   "deployments",
                   {
                       {RPCResult::Type::NUM, "rulename",
                        "identifies the bit number as indicating acceptance "
                        "and readiness for the named softfork rule"},
                   }},
                  {RPCResult::Type::ARR,
                   "capabilities",
                   "",
                   {
                       {RPCResult::Type::STR, "value",
                        "A supported feature, for example 'proposal'"},
                   }},
                  {RPCResult::Type::NUM, "vbrequired",
                   "bit mask of versionbits the server requires set in "
                   "submissions"},
                       ...
                       {RPCResult::Type::OBJ,
                        ...
                        "",
                        {
                            ...
                            {RPCResult::Type::STR_HEX, "txid",
                             "transaction hash excluding witness data, shown "
                             "in byte-reversed hex"},
                            {RPCResult::Type::STR_HEX, "hash",
                             "transaction hash including witness data, shown "
                             "in byte-reversed hex"},
                            {RPCResult::Type::ARR,
                             "depends",
                             "array of numbers",
                            ...
                            {RPCResult::Type::NUM, "fee",
                             ...
                             "the block subsidy); if key is not present, fee "
                             "is unknown and clients MUST NOT assume there "
                             "isn't one"},
                            {RPCResult::Type::NUM, "sigops",
                             "total SigOps cost, as counted for purposes of "
                             "block limits; if key is not present, sigop cost "
                             "is unknown and clients MUST NOT assume it is "
                             "zero"},
                            {RPCResult::Type::NUM, "weight",
                             "total transaction weight, as counted for "
                             "purposes of block limits"},
                        }},
                   }},
                  {RPCResult::Type::OBJ_DYN,
                   "coinbaseaux",
                  ...
                  {RPCResult::Type::STR, "longpollid",
                   "an id to include with a request to longpoll on an update "
                   "to this template"},
                  ...
                  {RPCResult::Type::NUM_TIME, "mintime",
                   ...
                   "expressed in " +
                       UNIX_EPOCH_TIME +
                       ". Adjusted for the proposed BIP94 timewarp rule."},
                  {RPCResult::Type::ARR,
                   "mutable",
                  ...
                  {RPCResult::Type::NUM, "weightlimit", /*optional=*/true,
                  ...
                  {RPCResult::Type::STR_HEX, "signet_challenge",
                   /*optional=*/true, "Only on signet"},
                  {RPCResult::Type::STR_HEX, "default_witness_commitment",
                   /*optional=*/true,
                   "a valid witness commitment for the unmodified block "
                   "template"},
              }},
      },
      RPCExamples{
      ...
      [&](const RPCHelpMan &self, const JSONRPCRequest &request) -> UniValue {
        ...
        if (!miner.isTestChain()) {
          ...
          if (miner.isInitialBlockDownload()) {
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, CLIENT_NAME
        ...
        if (!lpval.isNull()) {
          /**
           * Wait to respond until either the best block changes, OR there are
           * more transactions.
           *
           * The check for new transactions first happens after 1 minute and
           * subsequently every 10 seconds. BIP22 does not require this
           * particular interval. On mainnet the mempool changes frequently
           * enough that in practice this RPC returns after 60 seconds, or
           * sooner if the best block changes.
           *
           ...
           * timeout.
           *
           * The longpollid is assumed to be a tip hash if it has the right
           * format.
           */
          ...
          // TODO: Maybe recheck connections/IBD and (if something wrong) send
          // an expires-immediately template to stop miners?
        }
        ...
        if (consensusParams.signet_blocks &&
            !setClientRules.contains("signet")) {
          throw JSONRPCError(
              RPC_INVALID_PARAMETER,
              "getblocktemplate must be called with the signet rule set (call "
              "with {\"rules\": [\"segwit\", \"signet\"]})");
        }

        // GBT must be called with 'segwit' set in the rules
        if (!setClientRules.contains("segwit")) {
          throw JSONRPCError(RPC_INVALID_PARAMETER,
                             "getblocktemplate must be called with the segwit "
                             "rule set (call with {\"rules\": [\"segwit\"]})");
        }
        ...
        if (!pindexPrev || pindexPrev->GetBlockHash() != tip ||
          ...
          CBlockIndex *pindexPrevNew =
              chainman.m_blockman.LookupBlockIndex(tip);
          time_start = GetTime();
        ...
        // NOTE: If at some point we support pre-segwit miners
        // post-segwit-activation, this needs to take segwit support into
        // consideration
        const bool fPreSegWit = !DeploymentActiveAfter(
            pindexPrev, chainman, Consensus::DEPLOYMENT_SEGWIT);
        ...
        result.pushKV(
            "mintime",
            GetMinimumTime(pindexPrev,
                           consensusParams.DifficultyAdjustmentInterval()));
        ...
        if (consensusParams.signet_blocks) {
          result.pushKV("signet_challenge",
                        HexStr(consensusParams.signet_challenge));
        }
