...
static void ErrorLogCallback(void *arg, int code, const char *msg) {
  ...
  // function whenever that function is invoked." Assert that this is the case:
  assert(arg == nullptr);
  LogWarning("SQLite Error. Code: %d. Message: %s", code, msg);
}

static int TraceSqlCallback(unsigned code, void *context, void *param1,
                            void *param2) {
  ...
  if (code == SQLITE_TRACE_STMT) {
    ...
    char *expanded{sqlite3_stmt_readonly(stmt) ? sqlite3_expanded_sql(stmt)
                                               : nullptr};
    LogTrace(BCLog::WALLETDB, "[%s] SQLite Statement: %s\n", db->Filename(),
             expanded ? expanded : sqlite3_sql(stmt));
    if (expanded)
...
static bool BindBlobToStatement(sqlite3_stmt *stmt, int index,
  ...
  int res = sqlite3_bind_blob(
      stmt, index, blob.data() ? static_cast<const void *>(blob.data()) : "",
      blob.size(), SQLITE_STATIC);
  if (res != SQLITE_OK) {
    LogWarning("Unable to bind %s to statement: %s", description,
               sqlite3_errstr(res));
...
static std::optional<int> ReadPragmaInteger(sqlite3 *db, const std::string &key,
                                            const std::string &description,
                                            bilingual_str &error) {
  std::string stmt_text = strprintf("PRAGMA %s", key);
  sqlite3_stmt *pragma_read_stmt{nullptr};
  int ret =
      sqlite3_prepare_v2(db, stmt_text.c_str(), -1, &pragma_read_stmt, nullptr);
  if (ret != SQLITE_OK) {
    sqlite3_finalize(pragma_read_stmt);
    error = Untranslated(strprintf(
        "SQLiteDatabase: Failed to prepare the statement to fetch %s: %s",
        description, sqlite3_errstr(ret)));
    return std::nullopt;
  }
  ret = sqlite3_step(pragma_read_stmt);
  if (ret != SQLITE_ROW) {
    sqlite3_finalize(pragma_read_stmt);
    error = Untranslated(strprintf("SQLiteDatabase: Failed to fetch %s: %s",
                                   description, sqlite3_errstr(ret)));
    return std::nullopt;
  }
...
static void SetPragma(sqlite3 *db, const std::string &key,
                      const std::string &value, const std::string &err_msg) {
...
SQLiteDatabase::SQLiteDatabase(const fs::path &dir_path,
                               ...
                               const DatabaseOptions &options, bool mock)
    : WalletDatabase(), m_mock(mock), m_dir_path(dir_path),
      m_file_path(fs::PathToString(file_path)), m_write_semaphore(1),
      m_use_unsafe_sync(options.use_unsafe_sync) {
  {
    ...
    if (++g_sqlite_count == 1) {
      ...
      if (ret != SQLITE_OK) {
        LogWarning("SQLiteDatabase: Failed to configure serialized threading "
                   "mode: %s (Continuing anyway)\n",
                   sqlite3_errstr(ret));
      }
    }
    int ret = sqlite3_initialize(); // This is a no-op if sqlite3 is already
                                    // initialized
    if (ret != SQLITE_OK) {
      throw std::runtime_error(
...
void SQLiteBatch::SetupSQLStatements() {
  ...
  for (const auto &[stmt_prepared, stmt_text] : statements) {
    if (*stmt_prepared == nullptr) {
      int res = sqlite3_prepare_v2(m_database.m_db, stmt_text, -1,
...
SQLiteDatabase::~SQLiteDatabase() { Cleanup(); }
...
void SQLiteDatabase::Cleanup() noexcept {
  ...
  if (--g_sqlite_count == 0) {
    ...
    if (ret != SQLITE_OK) {
      LogWarning("SQLiteDatabase: Failed to shutdown SQLite: %s",
                 sqlite3_errstr(ret));
    }
  }
}
...
bool SQLiteDatabase::Verify(bilingual_str &error) {
  ...
  auto read_result =
      ReadPragmaInteger(m_db, "application_id", "the application id", error);
  ...
  read_result = ReadPragmaInteger(m_db, "user_version",
                                  "sqlite wallet schema version", error);
  if (!read_result.has_value())
    return false;
  int32_t user_ver = read_result.value();
  if (user_ver != WALLET_SCHEMA_VERSION) {
    error = strprintf(_("SQLiteDatabase: Unknown sqlite wallet schema version "
                        "%d. Only version %d is supported"),
                      user_ver, WALLET_SCHEMA_VERSION);
    return false;
  }
  ...
  if (ret != SQLITE_OK) {
    ...
    error = strprintf(
        _("SQLiteDatabase: Failed to prepare statement to verify database: %s"),
        sqlite3_errstr(ret));
    return false;
  }
  while (true) {
    ...
    if (ret != SQLITE_ROW) {
      error = strprintf(_("SQLiteDatabase: Failed to execute statement to "
                          "verify database: %s"),
                        sqlite3_errstr(ret));
      break;
    }
    ...
    if (!msg) {
      error = strprintf(
          _("SQLiteDatabase: Failed to read database verification error: %s"),
          sqlite3_errstr(ret));
      break;
    }
...
void SQLiteDatabase::Open() {
  ...
  if (m_db == nullptr) {
    ...
    // Trace SQL statements if tracing is enabled with -debug=walletdb
    // -loglevel=walletdb:trace
  ...
  if (sqlite3_db_readonly(m_db, "main") != 0) {
    throw std::runtime_error("SQLiteDatabase: Database opened in readonly mode "
                             "but read-write permissions are needed");
  }

  // Acquire an exclusive lock on the database
  // First change the locking mode to exclusive
  SetPragma(m_db, "locking_mode", "exclusive",
  ...
  int ret = sqlite3_exec(m_db, "BEGIN EXCLUSIVE TRANSACTION", nullptr, nullptr,
                         nullptr);
  if (ret != SQLITE_OK) {
    throw std::runtime_error(
        "SQLiteDatabase: Unable to obtain an exclusive lock on the database, "
        "is it being used by another instance of " CLIENT_NAME "?\n");
  }
  ...
  if (m_use_unsafe_sync) {
    ...
    LogWarning("SQLite is configured to not wait for data to be flushed to "
               "disk. Data loss and corruption may occur.");
    SetPragma(m_db, "synchronous", "OFF",
  ...
  ret = sqlite3_prepare_v2(
      m_db, "SELECT name FROM sqlite_master WHERE type='table' AND name='main'",
      -1, &check_main_stmt, nullptr);
  if (ret != SQLITE_OK) {
    throw std::runtime_error(
        strprintf("SQLiteDatabase: Failed to prepare statement to check table "
                  "existence: %s\n",
                  sqlite3_errstr(ret)));
  }
  ...
  if (sqlite3_finalize(check_main_stmt) != SQLITE_OK) {
    throw std::runtime_error(
        strprintf("SQLiteDatabase: Failed to finalize statement checking table "
                  "existence: %s\n",
                  sqlite3_errstr(ret)));
  }
  bool table_exists;
  if (ret == SQLITE_DONE) {
    ...
    throw std::runtime_error(
        strprintf("SQLiteDatabase: Failed to execute statement to check table "
                  "existence: %s\n",
                  sqlite3_errstr(ret)));
  }
  ...
  if (!table_exists) {
    ...
    SetPragma(m_db, "application_id",
              strprintf("%d", static_cast<int32_t>(app_id)),
...
bool SQLiteDatabase::HasActiveTxn() {
  // 'sqlite3_get_autocommit' returns true by default, and false if a
  // transaction has begun and not been committed or rolled back.
  return m_db && sqlite3_get_autocommit(m_db) == 0;
}

int SQliteExecHandler::Exec(SQLiteDatabase &database,
                            const std::string &statement) {
  return sqlite3_exec(database.m_db, statement.data(), nullptr, nullptr,
                      nullptr);
}
...
void SQLiteBatch::Close() {
  ...
  // If we began a transaction, and it wasn't committed, abort the transaction
  // in progress
  if (m_txn) {
    if (TxnAbort()) {
      LogWarning("SQLiteBatch: Batch closed unexpectedly without the "
                 "transaction being explicitly committed or aborted");
    } else {
      // If transaction cannot be aborted, it means there is a bug or there has
      // been data corruption. Try to recover in this case by closing and
      // reopening the database. Closing the database should also ensure that
      // any changes made since the transaction was opened will be rolled back
      // and future transactions can succeed without committing old data.
      force_conn_refresh = true;
      LogWarning("SQLiteBatch: Batch closed and failed to abort transaction, "
  ...
  if (force_conn_refresh) {
    ...
    try {
      m_database.Open();
      // If TxnAbort failed and we refreshed the connection, the semaphore was
      // not released, so release it here to avoid deadlocks on future writes.
...
bool SQLiteBatch::WriteKey(DataStream &&key, DataStream &&value,
  ...
  if (!BindBlobToStatement(stmt, 1, key, "key"))
    return false;
  if (!BindBlobToStatement(stmt, 2, value, "value"))
    return false;
...
bool SQLiteBatch::ExecStatement(sqlite3_stmt *stmt,
  ...
  // Bind: leftmost parameter in statement is index 1
  if (!BindBlobToStatement(stmt, 1, blob, "key"))
    return false;
...
SQLiteBatch::GetNewPrefixCursor(std::span<const std::byte> prefix) {
  if (!m_database.m_db)
    return nullptr;

  // To get just the records we want, the SQL statement does a comparison of the
  // binary data where the data must be greater than or equal to the prefix, and
  // less than the prefix incremented by one (when interpreted as an integer)
  ...
  if (!BindBlobToStatement(cursor->m_cursor_stmt, 1,
                           cursor->m_prefix_range_start, "prefix_start"))
    return nullptr;
  if (!end_range.empty()) {
    if (!BindBlobToStatement(cursor->m_cursor_stmt, 2,
                             cursor->m_prefix_range_end, "prefix_end"))
      return nullptr;
  }
...
MakeSQLiteDatabase(const fs::path &path, const DatabaseOptions &options,
  ...
  try {
    ...
    auto db = std::make_unique<SQLiteDatabase>(data_file.parent_path(),
                                               data_file, options);
