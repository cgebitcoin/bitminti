...
static void RemovePidFile(const ArgsManager &args) {
  ...
  if (std::error_code error; !fs::remove(pid_path, error)) {
    ...
    LogWarning("Unable to remove PID file (%s): %s", fs::PathToString(pid_path),
               msg);
  }
}
...
void Interrupt(NodeContext &node) {
  ...
  if (node.notifications)
    WITH_LOCK(node.notifications->m_tip_block_mutex,
...
void Shutdown(NodeContext &node) {
  ...
  /// failed part of the way, for example if the data directory was found to be
  ...
  for (auto &client : node.chain_clients) {
    try {
      ...
      LogDebug(BCLog::IPC, "Chain client did not disconnect cleanly: %s",
               e.what());
      client.reset();
    }
  }
  ...
  // the the scheduler. After this point, SyncWithValidationInterfaceQueue()
  // should not be called anymore as this would prevent the shutdown from
  // completing.
  if (node.scheduler)
    node.scheduler->stop();
  ...
  // Drop transactions we were still watching, record fee estimations and
  // unregister fee estimator from validation interface.
  ...
  // catch up with our current chain to avoid any strange pruning edge cases and
  // make next startup faster by avoiding rescan.
...
void SetupServerArgs(ArgsManager &argsman, bool can_listen_ipc) {
  ...
  const auto testnet4ChainParams =
      CreateChainParams(argsman, ChainType::TESTNET4);
...
#if HAVE_SYSTEM
  argsman.AddArg("-alertnotify=<cmd>",
                 "Execute command when an alert is raised (%s in cmd is "
                 "replaced by message)",
  ...
  argsman.AddArg(
      ...
      strprintf(
          ...
          "all, default: %s, testnet3: %s, testnet4: %s, signet: %s)",
          defaultChainParams->GetConsensus().defaultAssumeValid.GetHex(),
          testnetChainParams->GetConsensus().defaultAssumeValid.GetHex(),
          testnet4ChainParams->GetConsensus().defaultAssumeValid.GetHex(),
          signetChainParams->GetConsensus().defaultAssumeValid.GetHex()),
      ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
  ...
  argsman.AddArg(
      ...
      strprintf("Whether an XOR-key applies to blocksdir *.dat files. "
                "The created XOR-key will be zeros for an existing blocksdir "
                "or when `-blocksxor=0` is "
  ...
  argsman.AddArg(
      "-blocksonly",
      strprintf("Whether to reject transactions from network peers. Disables "
                "automatic broadcast and rebroadcast of transactions, unless "
                "the source peer has the 'forcerelay' permission. RPC "
                "transactions are not affected. (default: %u)",
                DEFAULT_BLOCKSONLY),
      ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
  ...
  argsman.AddArg(
      "-conf=<file>",
      strprintf("Specify path to read-only configuration file. Relative paths "
                "will be prefixed by datadir location (only useable from "
                "command line, not configuration file) (default: %s)",
  ...
  argsman.AddArg(
      "-dbcache=<n>",
      strprintf(
          "Maximum database cache size <n> MiB (minimum %d, default: %d). Make "
  ...
  argsman.AddArg(
      ...
      strprintf("Minimum work assumed to exist on a valid chain in hex "
                "(default: %s, testnet3: %s, testnet4: %s, signet: %s)",
                defaultChainParams->GetConsensus().nMinimumChainWork.GetHex(),
                testnetChainParams->GetConsensus().nMinimumChainWork.GetHex(),
                testnet4ChainParams->GetConsensus().nMinimumChainWork.GetHex(),
                signetChainParams->GetConsensus().nMinimumChainWork.GetHex()),
      ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY,
      OptionsCategory::OPTIONS);
  argsman.AddArg(
      "-par=<n>",
      strprintf("Set the number of script verification threads (0 = auto, up "
                "to %d, <0 = leave that many cores free, default: %d)",
  ...
  argsman.AddArg(
      ...
      strprintf("Whether a mempool.dat file created by -persistmempool or the "
                "savemempool RPC will be written in the legacy format "
                "(version 1) or the current format (version 2). This temporary "
                "option will be removed in the future. (default: %u)",
                DEFAULT_PERSIST_V1_DAT),
      ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
  argsman.AddArg("-pid=<file>",
  ...
  argsman.AddArg(
      "-prune=<n>",
      strprintf("Reduce storage requirements by enabling pruning (deleting) of "
                "old blocks. This allows the pruneblockchain RPC to be called "
                "to delete specific blocks and enables automatic pruning of "
                "old blocks if a target size in MiB is provided. This mode is "
  ...
  argsman.AddArg(
      ...
      "that are active. If an assumeutxo snapshot was loaded, its chainstate "
      "will be wiped as well. The snapshot can then be reloaded via RPC.",
      ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
  argsman.AddArg(
      "-reindex-chainstate",
  ...
  argsman.AddArg(
      ...
      strprintf(
          ...
          "by users (use %s instead for custom settings). Relative paths will "
  ...
  argsman.AddArg("-startupnotify=<cmd>", "Execute command on startup.",
                 ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
  argsman.AddArg("-shutdownnotify=<cmd>",
                 "Execute command immediately before beginning shutdown. The "
                 "need for shutdown may be urgent, so be careful not to delay "
                 "it long (if the command doesn't require interaction with the "
  ...
  argsman.AddArg(
      "-addnode=<ip>",
      strprintf("Add a node to connect to and attempt to keep the connection "
                "open (see the addnode RPC help for more info). This option "
                "can be specified multiple times to add multiple nodes; "
                "connections are limited to %u at a time and are counted "
  ...
  argsman.AddArg(
      ...
      strprintf("Bind to given address and always listen on it (default: "
                ...
                "tag any incoming connections to that address and port as "
                "incoming Tor connections (default: 127.0.0.1:%u=onion, "
                "testnet3: 127.0.0.1:%u=onion, testnet4: 127.0.0.1:%u=onion, "
                "signet: 127.0.0.1:%u=onion, regtest: 127.0.0.1:%u=onion)",
                defaultChainParams->GetDefaultPort() + 1,
                testnetChainParams->GetDefaultPort() + 1,
                testnet4ChainParams->GetDefaultPort() + 1,
                signetChainParams->GetDefaultPort() + 1,
                regtestChainParams->GetDefaultPort() + 1),
      ArgsManager::ALLOW_ANY | ArgsManager::NETWORK_ONLY,
      OptionsCategory::CONNECTION);
  argsman.AddArg("-cjdnsreachable",
                 "If set, then this host is configured for CJDNS (connecting "
                 "to fc00::/8 addresses would lead us to the CJDNS network, "
                 "see doc/cjdns.md) (default: 0)",
                 ArgsManager::ALLOW_ANY, OptionsCategory::CONNECTION);
  argsman.AddArg("-connect=<ip>",
                 "Connect only to the specified node; -noconnect disables "
                 "automatic connections (the rules for this peer are the same "
                 "as for -addnode). This option can be specified multiple "
                 "times to connect to multiple nodes.",
                 ArgsManager::ALLOW_ANY | ArgsManager::NETWORK_ONLY,
                 OptionsCategory::CONNECTION);
  argsman.AddArg("-discover",
  ...
  argsman.AddArg(
      "-dnsseed",
      strprintf("Query for peer addresses via DNS lookup, if low on addresses "
                "(default: %u unless -connect used or -maxconnections=0)",
                DEFAULT_DNSSEED),
      ArgsManager::ALLOW_ANY, OptionsCategory::CONNECTION);
  ...
  argsman.AddArg(
      "-forcednsseed",
      strprintf("Always query for peer addresses via DNS lookup (default: %u)",
                DEFAULT_FORCEDNSSEED),
      ArgsManager::ALLOW_ANY, OptionsCategory::CONNECTION);
  argsman.AddArg("-listen",
                 strprintf("Accept connections from outside (default: %u if no "
                           "-proxy, -connect or -maxconnections=0)",
                           DEFAULT_LISTEN),
                 ArgsManager::ALLOW_ANY, OptionsCategory::CONNECTION);
  argsman.AddArg(
      "-listenonion",
  ...
  argsman.AddArg(
      "-maxconnections=<n>",
      strprintf(
          "Maintain at most <n> automatic connections to peers (default: %u). "
          "This limit does not apply to connections manually added via "
          "-addnode or the addnode RPC, which have a separate limit of %u.",
          DEFAULT_MAX_PEER_CONNECTIONS, MAX_ADDNODE_CONNECTIONS),
      ArgsManager::ALLOW_ANY, OptionsCategory::CONNECTION);
  argsman.AddArg(
      "-maxreceivebuffer=<n>",
      strprintf(
      ...
      ArgsManager::ALLOW_ANY, OptionsCategory::CONNECTION);
  argsman.AddArg("-maxsendbuffer=<n>",
                 strprintf("Maximum per-connection memory usage for the send "
                           "buffer, <n>*1000 bytes (default: %u)",
                           DEFAULT_MAXSENDBUFFER),
                 ArgsManager::ALLOW_ANY, OptionsCategory::CONNECTION);
  argsman.AddArg(
      "-maxuploadtarget=<n>",
      strprintf("Tries to keep outbound traffic under the given target per "
                "24h. Limit does not apply to peers with 'download' permission "
                "or blocks created within past week. 0 = no limit (default: "
                "%s). Optional suffix units [k|K|m|M|g|G|t|T] (default: M). "
                "Lowercase is 1000 base while uppercase is 1024 base",
                DEFAULT_MAX_UPLOAD_TARGET),
      ArgsManager::ALLOW_ANY, OptionsCategory::CONNECTION);
#ifdef HAVE_SOCKADDR_UN
  argsman.AddArg("-onion=<ip:port|path>",
                 "Use separate SOCKS5 proxy to reach peers via Tor onion "
                 "services, set -noonion to disable (default: -proxy). May be "
                 "a local file path prefixed with 'unix:'.",
                 ArgsManager::ALLOW_ANY, OptionsCategory::CONNECTION);
#else
  argsman.AddArg("-onion=<ip:port>",
                 "Use separate SOCKS5 proxy to reach peers via Tor onion "
                 "services, set -noonion to disable (default: -proxy)",
                 ArgsManager::ALLOW_ANY, OptionsCategory::CONNECTION);
#endif
  ...
  argsman.AddArg(
      "-i2pacceptincoming",
      strprintf(
          "Whether to accept inbound I2P connections (default: %i). Ignored if "
  ...
  argsman.AddArg(
      "-onlynet=<net>",
      "Make automatic outbound connections only to network <net> (" +
  ...
  argsman.AddArg(
      "-txreconciliation",
      strprintf("Enable transaction reconciliations per BIP 330 (default: %d)",
                DEFAULT_TXRECONCILIATION_ENABLE),
      ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY,
      OptionsCategory::CONNECTION);
  argsman.AddArg(
      "-port=<port>",
      strprintf("Listen for connections on <port> (default: %u, testnet3: %u, "
                "testnet4: %u, signet: %u, regtest: %u). Not relevant for I2P "
                "(see doc/i2p.md). If set to a value x, the default onion "
                "listening port will be set to x+1.",
                defaultChainParams->GetDefaultPort(),
                testnetChainParams->GetDefaultPort(),
                testnet4ChainParams->GetDefaultPort(),
                signetChainParams->GetDefaultPort(),
...
#ifdef HAVE_SOCKADDR_UN
      "May be a local file path prefixed with 'unix:' if the proxy supports "
      "it. ";
#else
      "";
#endif
  argsman.AddArg(
      "-proxy=" + proxy_doc_for_value + "[=<network>]",
      ...
      OptionsCategory::CONNECTION);
  argsman.AddArg("-proxyrandomize",
                 strprintf("Randomize credentials for every proxy connection. "
                           "This enables Tor stream isolation (default: %u)",
  ...
  argsman.AddArg(
      ...
      ArgsManager::ALLOW_ANY, OptionsCategory::CONNECTION);
  argsman.AddArg("-networkactive",
                 "Enable all P2P network activity (default: 1). Can be changed "
                 "by the setnetworkactive RPC command",
                 ArgsManager::ALLOW_ANY, OptionsCategory::CONNECTION);
  argsman.AddArg(
      "-timeout=<n>",
      strprintf("Specify socket connection timeout in milliseconds. If an "
                "initial attempt to connect is unsuccessful after this amount "
                "of time, drop it (minimum: 1, default: %d)",
                DEFAULT_CONNECT_TIMEOUT),
      ArgsManager::ALLOW_ANY, OptionsCategory::CONNECTION);
  argsman.AddArg(
      "-peertimeout=<n>",
      strprintf(
          "Specify a p2p connection timeout delay in seconds. After connecting "
          "to a peer, wait this amount of time before considering "
          "disconnection based on inactivity (minimum: 1, default: %d)",
          DEFAULT_PEER_CONNECT_TIMEOUT),
      ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY,
      OptionsCategory::CONNECTION);
  argsman.AddArg("-torcontrol=<ip>:<port>",
                 strprintf("Tor control host and port to use if onion "
                           "listening enabled (default: %s). If no port is "
                           "specified, the default port of %i will be used.",
                           DEFAULT_TOR_CONTROL, DEFAULT_TOR_CONTROL_PORT),
                 ArgsManager::ALLOW_ANY, OptionsCategory::CONNECTION);
  ...
  argsman.AddArg(
      ...
      "to incoming connections and/or manual (default: incoming only). "
  ...
  argsman.AddArg(
      "-checkblockindex",
      strprintf("Do a consistency check for the block tree, chainstate, and "
                "other validation data structures every <n> operations. Use 0 "
                "to disable. (default: %u, regtest: %u)",
  ...
  argsman.AddArg(
      "-checkmempool=<n>",
      strprintf("Run mempool consistency checks every <n> transactions. Use 0 "
                "to disable. (default: %u, regtest: %u)",
                defaultChainParams->DefaultConsistencyChecks(),
                regtestChainParams->DefaultConsistencyChecks()),
      ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY,
      OptionsCategory::DEBUG_TEST);
  // Checkpoints were removed. We keep `-checkpoints` as a hidden arg to display
  // a more user friendly error when set.
  argsman.AddArg("-checkpoints", "", ArgsManager::ALLOW_ANY,
  ...
  argsman.AddArg("-stopatheight",
                 strprintf("Stop running after reaching the given height in "
                           "the main chain (default: %u). Blocks after target "
                           "height may be processed during shutdown.",
                           DEFAULT_STOPATHEIGHT),
                 ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY,
                 OptionsCategory::DEBUG_TEST);
  argsman.AddArg(
      "-limitancestorcount=<n>",
      strprintf("Deprecated setting to not accept transactions if number of "
                "in-mempool ancestors is <n> or more (default: %u); replaced "
                "by cluster limits (see -limitclustercount) and only used by "
                "wallet for coin selection",
  ...
  argsman.AddArg("-limitancestorsize", "", ArgsManager::ALLOW_ANY,
                 OptionsCategory::HIDDEN);
  argsman.AddArg("-limitdescendantsize", "", ArgsManager::ALLOW_ANY,
                 OptionsCategory::HIDDEN);
  argsman.AddArg(
      "-limitdescendantcount=<n>",
      strprintf("Deprecated setting to not accept transactions if any ancestor "
                "would have <n> or more in-mempool descendants (default: %u); "
                "replaced by cluster limits (see -limitclustercount) and only "
                "used by wallet for coin selection",
                DEFAULT_DESCENDANT_LIMIT),
      ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY,
      OptionsCategory::DEBUG_TEST);
  argsman.AddArg("-test=<option>",
                 "Pass a test-only option. Options include : " +
                     Join(TEST_OPTIONS_DOC, ", ") + ".",
                 ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY,
                 OptionsCategory::DEBUG_TEST);
  argsman.AddArg(
      "-limitclustercount=<n>",
      strprintf("Do not accept transactions into mempool which are directly or "
                "indirectly connected to <n> or more other unconfirmed "
                "transactions (default: %u, maximum: %u)",
                DEFAULT_CLUSTER_LIMIT, MAX_CLUSTER_COUNT_LIMIT),
      ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY,
      OptionsCategory::DEBUG_TEST);
  argsman.AddArg(
      "-limitclustersize=<n>",
      strprintf(
          "Do not accept transactions whose virtual size with all in-mempool "
          "connected transactions exceeds <n> kilobytes (default: %u)",
  ...
  argsman.AddArg("-mocktime=<n>",
                 ...
                 ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY,
                 OptionsCategory::DEBUG_TEST);
  argsman.AddArg("-maxsigcachesize=<n>",
                 strprintf("Limit sum of signature cache and script execution "
                           "cache sizes to <n> MiB (default: %u)",
                           DEFAULT_VALIDATION_CACHE_BYTES >> 20),
                 ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY,
                 OptionsCategory::DEBUG_TEST);
  ...
  argsman.AddArg(
      ...
      strprintf(
          ...
          CURRENCY_UNIT, DEFAULT_PRINT_MODIFIED_FEE),
      ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY,
      OptionsCategory::DEBUG_TEST);
  argsman.AddArg("-uacomment=<cmt>", "Append comment to the user agent string",
                 ArgsManager::ALLOW_ANY, OptionsCategory::DEBUG_TEST);

  SetupChainParamsBaseOptions(argsman);

  argsman.AddArg("-acceptnonstdtxn",
                 strprintf("Relay and mine \"non-standard\" transactions (test "
                           "networks only; default: %u)",
                           DEFAULT_ACCEPT_NON_STD_TXN),
                 ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY,
                 OptionsCategory::NODE_RELAY);
  argsman.AddArg(
      "-incrementalrelayfee=<amt>",
      strprintf("Fee rate (in %s/kvB) used to define cost of relay, used for "
                "mempool limiting and replacement policy. (default: %s)",
                CURRENCY_UNIT, FormatMoney(DEFAULT_INCREMENTAL_RELAY_FEE)),
      ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY,
      OptionsCategory::NODE_RELAY);
  argsman.AddArg(
      "-dustrelayfee=<amt>",
      strprintf("Fee rate (in %s/kvB) used to define dust, the value of an "
                "output such that it will cost more than its value in fees at "
                "this fee rate to spend it. (default: %s)",
                CURRENCY_UNIT, FormatMoney(DUST_RELAY_TX_FEE)),
      ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY,
      OptionsCategory::NODE_RELAY);
  argsman.AddArg(
      "-acceptstalefeeestimates",
      strprintf("Read fee estimates even if they are stale (%sdefault: %u) fee "
                "estimates are considered stale if they are %s hours old",
  ...
  argsman.AddArg(
      "-datacarriersize",
      strprintf(
          "Relay and mine transactions whose data-carrying raw scriptPubKeys "
          "in aggregate "
  ...
  argsman.AddArg(
      "-minrelaytxfee=<amt>",
      strprintf("Fees (in %s/kvB) smaller than this are considered zero fee "
  ...
  argsman.AddArg(
      ...
      strprintf("Add 'forcerelay' permission to whitelisted peers with default "
                ...
                "transactions were already in the mempool. (default: %d)",
                DEFAULT_WHITELISTFORCERELAY),
      ArgsManager::ALLOW_ANY, OptionsCategory::NODE_RELAY);
  argsman.AddArg(
      "-whitelistrelay",
      strprintf("Add 'relay' permission to whitelisted peers with default "
                "permissions. This will accept relayed transactions even when "
                "not relaying transactions (default: %d)",
                DEFAULT_WHITELISTRELAY),
      ArgsManager::ALLOW_ANY, OptionsCategory::NODE_RELAY);

  argsman.AddArg("-blockmaxweight=<n>",
                 strprintf("Set maximum BIP141 block weight (default: %d)",
                           DEFAULT_BLOCK_MAX_WEIGHT),
                 ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY,
                 OptionsCategory::BLOCK_CREATION);
  argsman.AddArg("-blockreservedweight=<n>",
                 strprintf("Reserve space for the fixed-size block header plus "
                           "the largest coinbase transaction the mining "
                           "software may add to the block. (default: %d).",
                           DEFAULT_BLOCK_RESERVED_WEIGHT),
                 ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
  argsman.AddArg("-blockmintxfee=<amt>",
                 strprintf("Set lowest fee rate (in %s/kvB) for transactions "
                           "to be included in block creation. (default: %s)",
                           CURRENCY_UNIT,
  ...
  argsman.AddArg(
      "-miningfastmode",
      "Enable RandomX Fast Mode (uses 2.1GB RAM for better performance)",
      ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);

  argsman.AddArg("-rest",
                 strprintf("Accept public REST requests (default: %u)",
                           DEFAULT_REST_ENABLE),
  ...
  argsman.AddArg("-rpcbind=<addr>[:port]",
                 "Bind to given address to listen for JSON-RPC connections. Do "
                 "not expose the RPC server to untrusted networks such as the "
                 "public internet! This option is ignored unless -rpcallowip "
                 "is also passed. Port is optional and overrides -rpcport. Use "
  ...
  argsman.AddArg("-rpccookiefile=<loc>",
  ...
  argsman.AddArg(
      ...
      strprintf("Listen for JSON-RPC connections on <port> (default: %u, "
                "testnet3: %u, testnet4: %u, signet: %u, regtest: %u)",
                defaultBaseParams->RPCPort(), testnetBaseParams->RPCPort(),
                testnet4BaseParams->RPCPort(), signetBaseParams->RPCPort(),
  ...
  argsman.AddArg(
      "-rpcthreads=<n>",
      strprintf("Set the number of threads to service RPC calls (default: %d)",
  ...
  argsman.AddArg("-rpcworkqueue=<n>",
                 strprintf("Set the maximum depth of the work queue to service "
                           "RPC calls (default: %d)",
  ...
  if (can_listen_ipc) {
    argsman.AddArg(
        "-ipcbind=<address>",
        "Bind to Unix socket address and listen for incoming connections. "
        "Valid address values are \"unix\" to listen on the default path, "
        "<datadir>/node.sock, or \"unix:/custom/path\" to specify a custom "
        "path. Can be specified multiple times to listen on multiple paths. "
        "Default behavior is not to listen on any path. If relative paths are "
        "specified, they are interpreted relative to the network data "
        "directory. If paths include any parent directory components and the "
        "parent directories do not exist, they will be created.",
        ArgsManager::ALLOW_ANY, OptionsCategory::IPC);
  }
...
void InitParameterInteraction(ArgsManager &args) {
  ...
  if (!args.GetArgs("-connect").empty() || args.IsArgNegated("-connect") ||
    ...
    // default do the same when connections are disabled
    if (args.SoftSetBoolArg("-dnsseed", false))
      LogInfo("parameter interaction: -connect or -maxconnections=0 set -> "
              "setting -dnsseed=0\n");
  ...
  if (args.GetBoolArg("-blocksonly", DEFAULT_BLOCKSONLY)) {
    ...
    if (args.SoftSetBoolArg("-whitelistrelay", false))
      LogInfo("parameter interaction: -blocksonly=1 -> setting "
              "-whitelistrelay=0\n");
    // Reduce default mempool size in blocksonly mode to avoid unexpected
    // resource usage
    if (args.SoftSetArg("-maxmempool",
                        ToString(DEFAULT_BLOCKSONLY_MAX_MEMPOOL_SIZE_MB)))
      LogInfo(
          "parameter interaction: -blocksonly=1 -> setting -maxmempool=%d\n",
  ...
  if (args.GetBoolArg("-whitelistforcerelay", DEFAULT_WHITELISTFORCERELAY)) {
    if (args.SoftSetBoolArg("-whitelistrelay", true))
      LogInfo("parameter interaction: -whitelistforcerelay=1 -> setting "
              "-whitelistrelay=1\n");
  }
  const auto onlynets = args.GetArgs("-onlynet");
  if (!onlynets.empty()) {
    bool clearnet_reachable =
        std::any_of(onlynets.begin(), onlynets.end(), [](const auto &net) {
          const auto n = ParseNetwork(net);
          return n == NET_IPV4 || n == NET_IPV6;
        });
    if (!clearnet_reachable && args.SoftSetBoolArg("-dnsseed", false)) {
      LogInfo("parameter interaction: -onlynet excludes IPv4 and IPv6 -> "
              "setting -dnsseed=0\n");
    }
  }
}
...
bool AppInitParameterInteraction(const ArgsManager &args) {
  ...
  // We removed checkpoints but keep the option to warn users who still have it
  // in their config.
  if (args.IsArgSet("-checkpoints")) {
    InitWarning(_("Option '-checkpoints' is set but checkpoints were removed. "
                  "This option has no effect."));
  }
  if (args.IsArgSet("-limitancestorsize")) {
    InitWarning(_("Option '-limitancestorsize' is given but ancestor size "
                  "limits have been replaced with cluster size limits (see "
                  "-limitclustersize). This option has no effect."));
  }
  if (args.IsArgSet("-limitdescendantsize")) {
    InitWarning(_("Option '-limitdescendantsize' is given but descendant size "
                  "limits have been replaced with cluster size limits (see "
                  "-limitclustersize). This option has no effect."));
  }
  ...
  for (const auto &arg : args.GetUnsuitableSectionOnlyArgs()) {
    errors +=
        strprintf(_("Config setting for %s only applied on %s network when in "
                    "[%s] section."),
  ...
  if (chain == ChainType::TESTNET) {
    LogInfo("Warning: Support for testnet3 is deprecated and will be removed "
            "in an upcoming release. Consider switching to testnet4.\n");
  }
  ...
  for (const auto &section : args.GetUnrecognizedSections()) {
    warnings +=
        Untranslated(strprintf("%s:%i ", section.m_file, section.m_line)) +
  ...
  // Signal NODE_COMPACT_FILTERS if peerblockfilters and basic filters index are
  // both enabled.
  ...
  if (args.GetIntArg("-prune", 0)) {
    ...
    if (args.GetBoolArg("-reindex-chainstate", false)) {
      return InitError(_("Prune mode is incompatible with -reindex-chainstate. "
  ...
  if (args.GetBoolArg("-forcednsseed", DEFAULT_FORCEDNSSEED) &&
    ...
    return InitError(
        _("Cannot set -forcednsseed to true when setting -dnsseed to false."));
  }
  ...
  if (nUserBind != 0 && !args.GetBoolArg("-listen", DEFAULT_LISTEN)) {
    return InitError(
        Untranslated("Cannot set -bind or -whitebind together with -listen=0"));
  }
  ...
  if (!args.GetBoolArg("-listen", DEFAULT_LISTEN) &&
    ...
    return InitError(
        Untranslated("Cannot set -listen=0 together with -listenonion=1"));
  }

  // Make sure enough file descriptors are available. We need to reserve enough
  // FDs to account for the bare minimum, plus all manual connections and all
  // bound interfaces. Any remainder will be available for connection sockets

  // Number of bound interfaces (we have at least one)
  int nBind = std::max(nUserBind, size_t(1));
  // Maximum number of connections with other nodes, this accounts for all types
  // of outbounds and inbounds except for manual
  int user_max_connection =
      args.GetIntArg("-maxconnections", DEFAULT_MAX_PEER_CONNECTIONS);
  if (user_max_connection < 0) {
    return InitError(
        Untranslated("-maxconnections must be greater or equal than zero"));
  }
  // Reserve enough FDs to account for the bare minimum, plus any manual
  // connections, plus the bound interfaces
  int min_required_fds = MIN_CORE_FDS + MAX_ADDNODE_CONNECTIONS + nBind;

  // Try raising the FD limit to what we need (available_fds may be smaller than
  // the requested amount if this fails)
  available_fds =
      RaiseFileDescriptorLimit(user_max_connection + min_required_fds);
  // If we are using select instead of poll, our actual limit may be even
  // smaller
#ifndef USE_POLL
  ...
  if (available_fds < min_required_fds)
    return InitError(strprintf(
        _("Not enough file descriptors available. %d available, %d required."),
        available_fds, min_required_fds));

  // Trim requested connection counts, to fit into system limitations
  nMaxConnections =
      std::min(available_fds - min_required_fds, user_max_connection);
...

  {
    ...
    if (max_block_weight > MAX_BLOCK_WEIGHT) {
      return InitError(strprintf(_("Specified -blockmaxweight (%d) exceeds "
                                   "consensus maximum block weight (%d)"),
                                 max_block_weight, MAX_BLOCK_WEIGHT));
    }
  }

  {
    const auto block_reserved_weight =
        args.GetIntArg("-blockreservedweight", DEFAULT_BLOCK_RESERVED_WEIGHT);
    if (block_reserved_weight > MAX_BLOCK_WEIGHT) {
      return InitError(
          strprintf(_("Specified -blockreservedweight (%d) exceeds consensus "
                      "maximum block weight (%d)"),
                    block_reserved_weight, MAX_BLOCK_WEIGHT));
    }
    if (block_reserved_weight < MINIMUM_BLOCK_RESERVED_WEIGHT) {
      return InitError(strprintf(_("Specified -blockreservedweight (%d) is "
                                   "lower than minimum safety value of (%d)"),
  ...
  if (!test_options.empty()) {
    if (chainparams.GetChainType() != ChainType::REGTEST) {
      return InitError(
          Untranslated("-test=<option> can only be used with regtest"));
    }
    for (const std::string &option : test_options) {
      auto it = std::find_if(TEST_OPTIONS_DOC.begin(), TEST_OPTIONS_DOC.end(),
                             [&option](const std::string &doc_option) {
                               size_t pos = doc_option.find(" (");
                               return (pos != std::string::npos) &&
                                      (doc_option.substr(0, pos) == option);
                             });
      if (it == TEST_OPTIONS_DOC.end()) {
        InitWarning(strprintf(
            _("Unrecognised option \"%s\" provided in -test=<option>."),
            option));
      }
    }
  }
...
static bool LockDirectory(const fs::path &dir, bool probeOnly) {
  ...
  case util::LockResult::ErrorWrite:
    return InitError(
        strprintf(_("Cannot write to directory '%s'; check permissions."),
                  fs::PathToString(dir)));
  case util::LockResult::ErrorLock:
    return InitError(strprintf(_("Cannot obtain a lock on directory %s. %s is "
                                 "probably already running."),
                               fs::PathToString(dir), CLIENT_NAME));
...
bool AppInitSanityChecks(const kernel::Context &kernel) {
  ...
  if (!ECC_InitSanityCheck()) {
    return InitError(strprintf(_("Elliptic curve cryptography sanity check "
                                 "failure. %s is shutting down."),
                               CLIENT_NAME));
  }

  // Probe the directory locks to give an early error message, if possible
  // We cannot hold the directory locks here, as the forking for daemon() hasn't
  // yet happened, and a fork will cause weird behavior to them.
  return LockDirectories(true);
}

bool AppInitLockDirectories() {
  // After daemonization get the directory locks again and hold on to them until
  // exit This creates a slight window for a race condition to happen, however
  // this condition is harmless: it will at most make us exit without printing a
  // message to console.
...
bool CheckHostPortOptions(const ArgsManager &args) {
  ...
  for ([[maybe_unused]] const auto &[param_name, unix, suffix_allowed] :
    ...
    for (const std::string &param_value : args.GetArgs(param_name)) {
      const std::string param_value_hostport{
          suffix_allowed ? param_value.substr(0, param_value.rfind('='))
                         : param_value};
      std::string host_out;
      uint16_t port_out{0};
 ...
 *
 * @param[in] conn_options Connection options containing the binding vectors to
 * check
 * @return std::optional<CService> containing the first duplicate found, or
 * std::nullopt if no duplicates
 */
static std::optional<CService>
CheckBindingConflicts(const CConnman::Options &conn_options) {
...
// A GUI user may opt to retry once with do_reindex set if there is a failure
// during chainstate initialization. The function therefore has to support
// re-entry.
static ChainstateLoadResult InitAndLoadChainstate(
    NodeContext &node, bool do_reindex, const bool do_reindex_chainstate,
    const kernel::CacheSizes &cache_sizes, const ArgsManager &args) {
  ...
  Assert(ApplyArgsManOptions(
      args, chainparams, mempool_opts)); // no error can happen, already checked
  ...
  Assert(ApplyArgsManOptions(
      args, chainman_opts)); // no error can happen, already checked in
  ...
  Assert(ApplyArgsManOptions(
      args, blockman_opts)); // no error can happen, already checked in
                             // AppInitParameterInteraction
  ...
  try {
    node.chainman = std::make_unique<ChainstateManager>(
        *Assert(node.shutdown_signal), chainman_opts, blockman_opts);
  } catch (dbwrapper_error &e) {
    LogError("%s", e.what());
    return {ChainstateLoadStatus::FAILURE, _("Error opening block database")};
  } catch (std::exception &e) {
    return {ChainstateLoadStatus::FAILURE_FATAL,
            Untranslated(strprintf("Failed to initialize ChainstateManager: %s",
                                   e.what()))};
  }
  ...
  // This is defined and set here instead of inline in validation.h to avoid a
  // hard dependency between validation and index/base, since the latter is not
  // in libbitcoinkernel.
  ...
  if (status == node::ChainstateLoadStatus::SUCCESS) {
    ...
    if (chainman.m_blockman.m_have_pruned &&
      ...
      LogWarning("pruned datadir may not have more than %d blocks; only "
...
bool AppInitMain(NodeContext &node,
  ...
  auto opt_max_upload = ParseByteUnits(
      args.GetArg("-maxuploadtarget", DEFAULT_MAX_UPLOAD_TARGET), ByteUnit::M);
  if (!opt_max_upload) {
    return InitError(strprintf(_("Unable to parse -maxuploadtarget: '%s'"),
                               args.GetArg("-maxuploadtarget", "")));
  }
  ...
  node.validation_signals = std::make_unique<ValidationSignals>(
      std::make_unique<SerialTaskRunner>(scheduler));
    ...
    for (std::string address : gArgs.GetArgs("-ipcbind")) {
      try {
        ...
        return InitError(Untranslated(strprintf(
            "Unable to bind to IPC address '%s'. %s", address, e.what())));
      }
  ...
  // ********************************************************* Step 6: network
  // initialization Note that we absolutely cannot open any actual connections
  ...
  node.banman = std::make_unique<BanMan>(
      args.GetDataDirNet() / "banlist", &uiInterface,
      args.GetIntArg("-bantime", DEFAULT_MISBEHAVING_BANTIME));
  assert(!node.connman);
  node.connman = std::make_unique<CConnman>(
      rng.rand64(), rng.rand64(), *node.addrman, *node.netgroupman, chainparams,
      args.GetBoolArg("-networkactive", true));

  assert(!node.fee_estimator);
  // Don't initialize fee estimation with old data if we don't relay
  // transactions, as they would never get updated.
  if (!peerman_opts.ignore_incoming_txs) {
    bool read_stale_estimates = args.GetBoolArg(
        "-acceptstalefeeestimates", DEFAULT_ACCEPT_STALE_FEE_ESTIMATES);
    if (read_stale_estimates &&
        (chainparams.GetChainType() != ChainType::REGTEST)) {
      return InitError(
          strprintf(_("acceptstalefeeestimates is not supported on %s chain."),
                    chainparams.GetChainTypeString()));
    }
    node.fee_estimator = std::make_unique<CBlockPolicyEstimator>(
        FeeestPath(args), read_stale_estimates);

    // Flush estimates to disk periodically
    CBlockPolicyEstimator *fee_estimator = node.fee_estimator.get();
    scheduler.scheduleEvery(
        [fee_estimator] { fee_estimator->FlushFeeEstimates(); },
        FEE_FLUSH_INTERVAL);
  ...
  // Requesting DNS seeds entails connecting to IPv4/IPv6, which -onlynet
  // options may prohibit: If -dnsseed=1 is explicitly specified, abort. If it's
  // left unspecified by the user, we skip the DNS seeds by adjusting -dnsseed
  // in InitParameterInteraction.
  if (args.GetBoolArg("-dnsseed") == true &&
      !g_reachable_nets.Contains(NET_IPV4) &&
      !g_reachable_nets.Contains(NET_IPV6)) {
    return InitError(strprintf(
        _("Incompatible options: -dnsseed=1 was explicitly specified, but "
          "-onlynet forbids connections to IPv4/IPv6")));
  };
  ...
  // -noproxy, -proxy=0 or -proxy="" can be used to remove the proxy setting,
  // this is the default
  Proxy ipv4_proxy;
  Proxy ipv6_proxy;
  Proxy onion_proxy;
  ...
  for (const std::string &param_value : args.GetArgs("-proxy")) {
    ...
    const std::string proxy_str{param_value.substr(
        0, eq_pos)}; // e.g. 127.0.0.1:9050=ipv4 -> 127.0.0.1:9050
    std::string net_str;
    if (eq_pos != std::string::npos) {
      if (eq_pos + 1 == param_value.length()) {
        return InitError(strprintf(
            _("Invalid -proxy address or hostname, ends with '=': '%s'"),
            param_value));
      }
      net_str = ToLower(
          param_value.substr(eq_pos + 1)); // e.g. 127.0.0.1:9050=ipv4 -> ipv4
    }
    ...
    if (!proxy_str.empty() && proxy_str != "0") {
      if (IsUnixSocketPath(proxy_str)) {
        ...
        const std::optional<CService> addr{
            Lookup(proxy_str, DEFAULT_TOR_SOCKS_PORT, fNameLookup)};
        if (!addr.has_value()) {
          return InitError(strprintf(
              _("Invalid -proxy address or hostname: '%s'"), proxy_str));
        }
    ...
    if (net_str.empty()) { // For all networks.
      ...
      return InitError(strprintf(_("Unrecognized network in -proxy='%s': '%s'"),
                                 param_value, net_str));
    }
  }
  ...
  const bool onlynet_used_with_onion{!onlynets.empty() &&
                                     g_reachable_nets.Contains(NET_ONION)};

  // -onion can be used to set only a proxy for .onion, or override normal proxy
  // for .onion addresses -noonion (or -onion=0) disables connecting to .onion
  // entirely An empty string is used to not override the onion proxy (in which
  ...
  if (onionArg != "") {
    if (onionArg == "0") { // Handle -noonion/-onion=0
      ...
      if (onlynet_used_with_onion) {
        return InitError(
            _("Outbound connections restricted to Tor (-onlynet=onion) but the "
              "proxy for "
      ...
      if (IsUnixSocketPath(onionArg)) {
        ...
        const std::optional<CService> addr{
            Lookup(onionArg, DEFAULT_TOR_SOCKS_PORT, fNameLookup)};
        ...
        onion_proxy =
            Proxy(addr.value(), /*tor_stream_isolation=*/proxyRandomize);
      }
    }
  }
  ...
  if (onion_proxy.IsValid()) {
    ...
    // If -listenonion is set, then we will (try to) connect to the Tor control
    // port later from the torcontrol thread and may retrieve the onion proxy
    // from there.
    const bool listenonion_disabled{
        !args.GetBoolArg("-listenonion", DEFAULT_LISTEN_ONION)};
    if (onlynet_used_with_onion && listenonion_disabled) {
      return InitError(_(
          "Outbound connections restricted to Tor (-onlynet=onion) but the "
          "proxy for "
  ...
  g_zmq_notification_interface = CZMQNotificationInterface::Create(
      [&chainman = node.chainman](std::vector<std::byte> &block,
                                  const CBlockIndex &index) {
        assert(chainman);
        if (auto ret{chainman->m_blockman.ReadRawBlock(
                WITH_LOCK(cs_main, return index.GetBlockPos()))}) {
          block = std::move(*ret);
          return true;
        }
        return false;
  ...
  // Chainstate initialization and loading may be retried once with reindexing
  // by GUI users
  auto [status, error] = InitAndLoadChainstate(
      node, do_reindex, do_reindex_chainstate, kernel_cache_sizes, args);
  if (status == ChainstateLoadStatus::FAILURE && !do_reindex &&
      !ShutdownRequested(node)) {
    // suggest a reindex
    bool do_retry{
        HasTestOption(args, "reindex_after_failure_noninteractive_yes") ||
  ...
  if (status != ChainstateLoadStatus::SUCCESS &&
      status != ChainstateLoadStatus::INTERRUPTED) {
  ...
  node.peerman =
      PeerManager::make(*node.connman, *node.addrman, node.banman.get(),
                        chainman, *node.mempool, *node.warnings, peerman_opts);
  ...
  if (args.GetBoolArg("-txindex", DEFAULT_TXINDEX)) {
    g_txindex = std::make_unique<TxIndex>(interfaces::MakeChain(node),
                                          index_cache_sizes.tx_index, false,
                                          do_reindex);
    node.indexes.emplace_back(g_txindex.get());
  }

  for (const auto &filter_type : g_enabled_filter_types) {
    InitBlockFilterIndex([&] { return interfaces::MakeChain(node); },
                         filter_type, index_cache_sizes.filter_index, false,
                         do_reindex);
  ...
  if (args.GetBoolArg("-coinstatsindex", DEFAULT_COINSTATSINDEX)) {
    g_coin_stats_index = std::make_unique<CoinStatsIndex>(
        interfaces::MakeChain(node), /*cache_size=*/0, false, do_reindex);
  ...
  if (!do_reindex && !do_reindex_chainstate && chain_active_height <= 1) {
    ...
    uint64_t additional_bytes_needed{
        chainman.m_blockman.IsPruneMode()
            ? std::min(chainman.m_blockman.GetPruneTarget(),
                       assumed_chain_bytes)
            : assumed_chain_bytes};

    if (!CheckDiskSpace(args.GetBlocksDirPath(), additional_bytes_needed)) {
      InitWarning(strprintf(
          _("Disk space for %s may not accommodate the block files. "
  ...
  auto check_and_warn_fs{[&](const fs::path &path, std::string_view desc) {
    const auto path_desc{
        strprintf("%s (\"%s\")", desc, fs::PathToString(path))};
    switch (GetFilesystemType(path)) {
    case FSType::EXFAT:
      InitWarning(strprintf(_("The %s path uses exFAT, which is known to have "
                              "intermittent corruption problems on macOS. "
                              "Move this directory to a different filesystem "
                              "to avoid data loss."),
                            path_desc));
      break;
    case FSType::ERROR:
  ...
  if (!block_notify.empty()) {
    uiInterface.NotifyBlockTip_connect(
        [block_notify](SynchronizationState sync_state,
  ...
  node.background_init_thread =
      std::thread(&util::TraceThread, "initload", [=, &chainman, &args, &node] {
        ...
        if (args.GetBoolArg("-stopafterblockimport",
          ...
          if (!(Assert(node.shutdown_request))()) {
            LogError("Failed to send shutdown signal after finishing block "
                     "import\n");
          }
          return;
        }
  ...
  /*
   * Wait for genesis block to be processed. Typically
   * kernel_notifications.m_tip_block has already been set by a call to
   * LoadChainTip() in CompleteChainstateInitialization(). But this is skipped
   * if the chainstate doesn't exist yet or is being wiped:
   *
   * 1. first startup with an empty datadir
   * 2. reindex
   * 3. reindex-chainstate
   *
   * In these case it's connected by a call to ActivateBestChain() in the
   * initload thread.
   */
  {
    WAIT_LOCK(kernel_notifications.m_tip_block_mutex, lock);
    kernel_notifications.m_tip_block_cv.wait(
        lock,
  ...
  if (node.peerman)
    node.peerman->SetBestBlock(chain_active_height,
                               std::chrono::seconds{best_block_time});
  ...
  const auto BadPortWarning = [](const char *prefix, uint16_t port) {
    return strprintf(
        _("%s request to listen on port %u. This port is considered \"bad\" "
          "and "
  ...
  if (args.GetBoolArg("-listenonion", DEFAULT_LISTEN_ONION)) {
    if (connOptions.onion_binds.size() > 1) {
      InitWarning(
          strprintf(_("More than one onion bind address is provided. Using %s "
  ...
  for (const auto &net : args.GetArgs("-whitelist")) {
    ...
    bilingual_str error;
    if (!NetWhitelistPermissions::TryParse(net, subnet, connection_direction,
                                           error))
      return InitError(error);
  ...
  if (!i2psam_arg.empty()) {
    ...
    if (!addr.has_value() || !addr->IsValid()) {
      return InitError(strprintf(_("Invalid -i2psam address or hostname: '%s'"),
                                 i2psam_arg));
    }
  ...
  connOptions.m_i2p_accept_incoming =
      args.GetBoolArg("-i2pacceptincoming", DEFAULT_I2P_ACCEPT_INCOMING);
  ...
  scheduler.scheduleEvery([banman] { banman->DumpBanlist(); },
                          DUMP_BANS_INTERVAL);
...
bool StartIndexBackgroundSync(NodeContext &node) {
  ...
  // This block is used to verify that we have the required blocks' data stored
  // on disk, starting from that point up to the current tip.
  // indexes_start_block='nullptr' means "start from height 0".
  std::optional<const CBlockIndex *> indexes_start_block;
  std::string older_index_name;
  ChainstateManager &chainman = *Assert(node.chainman);
  const Chainstate &chainstate =
      WITH_LOCK(::cs_main, return chainman.ValidatedChainstate());
  ...
  for (auto index : node.indexes) {
    ...
    const CBlockIndex *pindex =
        chainman.m_blockman.LookupBlockIndex(summary.best_block_hash);
    ...
    if (!indexes_start_block || !pindex ||
      ...
      if (!pindex)
        break; // Starting from genesis so no need to look for earlier block.
    }
  };
  ...
  if (indexes_start_block) {
    ...
    if (!chainman.m_blockman.CheckBlockDataAvailability(*index_chain.Tip(),
                                                        *Assert(start_block))) {
      return InitError(Untranslated(
          strprintf("%s best block of the index goes beyond pruned data. "
                    "Please disable the index or reindex (which will download "
