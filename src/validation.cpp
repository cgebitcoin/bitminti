 ...
 * disk. Randomize writing time inside the window to prevent a situation where
 * the network over time settles into a few cohorts of synchronized writers.
 */
  ...
  for (size_t i = 0; i < tx.vin.size(); ++i) {
    if (auto coin{coins.GetCoin(tx.vin[i].prevout)}) {
      prev_heights[i] = coin->nHeight == MEMPOOL_HEIGHT
                            ? tip.nHeight + 1 // Assume all mempool transaction
...
std::optional<LockPoints> CalculateLockPointsAtTip(CBlockIndex *tip,
  ...
  return LockPoints{min_height, min_time,
                    Assert(tip->GetAncestor(max_input_height))};
}
...
static void LimitMempoolSize(CTxMemPool &pool, CCoinsViewCache &coins_cache)
  ...
  int expired =
      pool.Expire(GetTime<std::chrono::seconds>() - pool.m_opts.expiry);
  if (expired != 0) {
...
static bool IsCurrentForFeeEstimation(Chainstate &active_chainstate)
  ...
  if (active_chainstate.m_chain.Tip()->GetBlockTime() <
      count_seconds(GetTime<std::chrono::seconds>() -
                    MAX_FEE_ESTIMATION_TIP_AGE))
    return false;
  if (active_chainstate.m_chain.Height() <
      active_chainstate.m_chainman.m_best_header->nHeight - 1) {
    return false;
  }
  return true;
}
...
void Chainstate::MaybeUpdateMempoolForReorg(
    DisconnectedBlockTransactions &disconnectpool, bool fAddToMempool) {
  if (!m_mempool)
    return;
  ...
  {
    // disconnectpool is ordered so that the front is the most
    // recently-confirmed transaction (the last tx of the block at the tip) in
    // the disconnected chain. Iterate disconnectpool in reverse, so that we add
    // transactions back to the mempool starting with the earliest transaction
    // that had been previously seen in a block.
  ...
  // AcceptToMemoryPool/addNewTransaction all assume that new mempool entries
  ...
  // Checks whether the transaction is still final and, if it spends a coinbase
  // output, mature. Also updates valid entries' cached LockPoints if needed. If
  // false, the tx is still valid and its lockpoints are updated. If true, the
  // tx would be invalid in the next block; remove this entry and all of its
  // descendants. Note that TRUC rules are not applied here, so reorgs may cause
  // violations of TRUC inheritance or topology restrictions.
  const auto filter_final_and_mature =
      [&](CTxMemPool::txiter it)
          EXCLUSIVE_LOCKS_REQUIRED(m_mempool->cs, ::cs_main) {
            ...
            if (!CheckFinalTxAtTip(*Assert(m_chain.Tip()), tx))
              return true;

            const LockPoints &lp = it->GetLockPoints();
            // CheckSequenceLocksAtTip checks if the transaction will be final
            // in the next block to be created on top of the new chain.
            if (TestLockPointValidity(m_chain, lp)) {
              ...
              const std::optional<LockPoints> new_lock_points{
                  CalculateLockPointsAtTip(m_chain.Tip(), view_mempool, tx)};
              if (new_lock_points.has_value() &&
                  CheckSequenceLocksAtTip(m_chain.Tip(), *new_lock_points)) {
            ...
            if (it->GetSpendsCoinbase()) {
              for (const CTxIn &txin : tx.vin) {
                if (m_mempool->exists(txin.prevout.hash))
                  continue;
...
static bool CheckInputsFromMempoolAndCache(
    ...
    script_verify_flags flags, PrecomputedTransactionData &txdata,
    CCoinsViewCache &coins_tip, ValidationCache &validation_cache)
    EXCLUSIVE_LOCKS_REQUIRED(cs_main, pool.cs) {
  AssertLockHeld(cs_main);
  AssertLockHeld(pool.cs);
...
public:
  ...
  struct ATMPArgs {
    ...
    /** When true, the transaction or package will not be submitted to the
     * mempool. */
    const bool m_test_accept;
    /** Whether we allow transactions to replace mempool transactions. If false,
     * any transaction spending the same inputs as a transaction in the mempool
     * is considered a conflict. */
    const bool m_allow_replacement;
    /** When true, allow sibling eviction. This only occurs in single
     * transaction package settings. */
    const bool m_allow_sibling_eviction;
    /** Used to skip the LimitMempoolSize() call within
     * AcceptSingleTransaction(). This should be used when multiple
     * AcceptSubPackage calls are expected and the mempool will be trimmed at
     * the end of AcceptPackage(). */
     ...
     * due to fee miscalculation by wallets. std:nullopt implies unset, allowing
     * any feerates. Any individual transaction failing this check causes
     * immediate failure.
     */
    const std::optional<CFeeRate> m_client_maxfeerate;

    /** Parameters for single transaction mempool validation. */
    static ATMPArgs SingleAccept(const CChainParams &chainparams,
     ...
     * testmempoolaccept. */
    static ATMPArgs
    PackageTestAccept(const CChainParams &chainparams, int64_t accept_time,
                      std::vector<COutPoint> &coins_to_uncache) {
      return ATMPArgs{
          /* m_chainparams */ chainparams,
          /* m_accept_time */ accept_time,
    ...
    static ATMPArgs
    PackageChildWithParents(const CChainParams &chainparams,
                            int64_t accept_time,
                            std::vector<COutPoint> &coins_to_uncache,
  ...
  private:
    ...
    // possibility of mixing up the order of the arguments. Use static functions
    // above instead.
    ATMPArgs(const CChainParams &chainparams, int64_t accept_time,
             bool bypass_limits, std::vector<COutPoint> &coins_to_uncache,
             bool test_accept, bool allow_replacement,
             bool allow_sibling_eviction, bool package_submission,
             bool package_feerates, std::optional<CFeeRate> client_maxfeerate)
        : m_chainparams{chainparams}, m_accept_time{accept_time},
          m_bypass_limits{bypass_limits}, m_coins_to_uncache{coins_to_uncache},
          m_test_accept{test_accept}, m_allow_replacement{allow_replacement},
          m_allow_sibling_eviction{allow_sibling_eviction},
          m_package_submission{package_submission},
          m_package_feerates{package_feerates}, m_client_maxfeerate{
                                                    client_maxfeerate} {
  ...
  MempoolAcceptResult
  AcceptSingleTransactionAndCleanup(const CTransactionRef &ptx, ATMPArgs &args)
      EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
    LOCK(m_pool.cs);
  ...
  MempoolAcceptResult
  AcceptSingleTransactionInternal(const CTransactionRef &ptx, ATMPArgs &args)
      EXCLUSIVE_LOCKS_REQUIRED(cs_main, m_pool.cs);

  /**
   * Multiple transaction acceptance. Transactions may or may not be
   * interdependent, but must not conflict with each other, and the transactions
   * cannot already be in the mempool. Parents must come before children if any
   * dependencies exist.
   */
  PackageMempoolAcceptResult
  AcceptMultipleTransactionsAndCleanup(const std::vector<CTransactionRef> &txns,
                                       ATMPArgs &args)
      EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
    LOCK(m_pool.cs);
  ...
  PackageMempoolAcceptResult
  AcceptMultipleTransactionsInternal(const std::vector<CTransactionRef> &txns,
                                     ATMPArgs &args)
      EXCLUSIVE_LOCKS_REQUIRED(cs_main, m_pool.cs);
   ...
   * ATMPArgs to enable sibling eviction and creates a
  ...
  AcceptSubPackage(const std::vector<CTransactionRef> &subpackage,
                   ATMPArgs &args) EXCLUSIVE_LOCKS_REQUIRED(cs_main, m_pool.cs);

  /**
   * Package (more specific than just multiple transactions) acceptance. Package
   * must be a child with all of its unconfirmed parents, and topologically
   * sorted.
   */
...
private:
  ...
  struct Workspace {
    explicit Workspace(const CTransactionRef &ptx)
        : m_ptx(ptx), m_hash(ptx->GetHash()) {}
    /** Txids of mempool transactions that this transaction directly conflicts
     * with or may replace via sibling eviction. */
    std::set<Txid> m_conflicts;
    /** Iterators to mempool entries that this transaction directly conflicts
     * with or may replace via sibling eviction. */
     ...
     * m_replaced_transactions) include a sibling in addition to txns with
     * conflicting inputs. */
    bool m_sibling_eviction{false};

    /** Virtual size of the transaction as used by the mempool, calculated using
     * serialized size of the transaction and sigops. */
    int64_t m_vsize;
    /** Fees paid by this transaction: total input amounts subtracted by total
     * output amounts. */
    CAmount m_base_fees;
     ...
     * "effective" package feerate of this transaction is the total fees divided
     * by the total size of transactions (which may include its ancestors and/or
     * descendants). */
    ...
    /** A temporary cache containing serialized transaction data for signature
     * verification. Reused across PolicyScriptChecks and ConsensusScriptChecks.
     */
  ...
  // Run checks for mempool replace-by-fee, only used in
  ...
  // individual ancestor/descendant limits done in PreChecks) and run Package
  // RBF checks.
  ...
  // Run the script checks using our policy flags. As this can be slow, we
  // should only invoke this on transactions that have otherwise passed policy
  // checks.
  ...
  void FinalizeSubpackage(const ATMPArgs &args)
      EXCLUSIVE_LOCKS_REQUIRED(cs_main, m_pool.cs);

  // Submit all transactions to the mempool and call ConsensusScriptChecks to
  // add to the script cache - should only be called after successful validation
  // of all transactions in the package. Does not call LimitMempoolSize(), so
  // mempool max_size_bytes may be temporarily exceeded.
  ...
  bool CheckFeeRate(size_t package_size, CAmount package_fee,
...
private:
  ...
  /** Holds a cached view of available coins from the UTXO set, mempool, and
   * artificial temporary coins (to enable package validation). The view doesn't
   * track whether a coin previously existed but has now been spent. We detect
   * conflicts in other ways:
   * - conflicts within a transaction are checked in CheckTransaction
   * (bad-txns-inputs-duplicate)
   * - conflicts within a package are checked in IsWellFormedPackage
   * (conflict-in-package)
   * - conflicts with an existing mempool transaction are found in
   * CTxMemPool::GetConflictTx and replacements are allowed The temporary coins
   * should persist between individual transaction checks so that package
   * validation is possible, but must be cleaned up when we finish validating a
   * subpackage, whether accepted or rejected. The cache must also be cleared
   * when mempool contents change (when a changeset is applied or when the
   * mempool trims itself) because it can return cached coins that no longer
   * exist in the backend. Use CleanupTemporaryCoins() anytime you are finished
   * with a SubPackageState or call LimitMempoolSize().
   */
  CCoinsViewCache m_view;

  // These are the two possible backends for m_view.
  /** When m_view is connected to m_viewmempool as its backend, it can pull
   * coins from the mempool and from the UTXO set. This is also where temporary
   * coins are stored. */
  CCoinsViewMemPool m_viewmempool;
  /** When m_view is connected to m_dummy, it can no longer look up coins from
   * the mempool or UTXO set (meaning no disk operations happen), but can still
   * return coins it accessed previously. Useful for keeping track of which
   * coins were pulled from disk. */
  CCoinsView m_dummy;

  Chainstate &m_active_chainstate;

  // Fields below are per *sub*package state and must be reset prior to
  // subsequent AcceptSingleTransaction and AcceptMultipleTransactions
  // invocations
  struct SubPackageState {
    /** Aggregated modified fees of all transactions, used to calculate package
     * feerate. */
    CAmount m_total_modified_fees{0};
    /** Aggregated virtual size of all transactions, used to calculate package
     * feerate. */
    int64_t m_total_vsize{0};

    // RBF-related members
    /** Whether the transaction(s) would replace any mempool transactions and/or
     * evict any siblings. If so, RBF rules apply. */
    bool m_rbf{false};
...
bool MemPoolAccept::PreChecks(ATMPArgs &args, Workspace &ws) {
  ...
  for (const CTxIn &txin : tx.vin) {
    ...
    if (ptxConflicting) {
      if (!args.m_allow_replacement) {
        // Transaction conflicts with a mempool tx, but we're not allowing
        // replacements in this context.
  ...
  for (const CTxIn &txin : tx.vin) {
    ...
    if (!m_view.HaveCoin(txin.prevout)) {
      ...
      // Otherwise assume this might be an orphan tx for which we just haven't
      // seen parents yet
  ...
  // This is const, but calls into the back end CoinsViews. The CCoinsViewDB at
  // the bottom of the hierarchy brings the best block into scope. See
  ...
  // can't be mined yet. Pass in m_view which has all of the relevant inputs
  // cached. Note that, since m_view's backend was removed, it no longer pulls
  // coins from the mempool.
  ...
  if (tx.HasWitness() && m_pool.m_opts.require_standard &&
    ...
    return state.Invalid(TxValidationResult::TX_WITNESS_MUTATED,
                         "bad-witness-nonstandard");
  }

  int64_t nSigOpsCost =
      GetTransactionSigOpCost(tx, m_view, STANDARD_SCRIPT_VERIFY_FLAGS);
  ...
  // Set entry_sequence to 0 when bypass_limits is used; this allows txs from a
  // block reorg to be marked earlier than any child txs that were already in
  // the mempool.
  const uint64_t entry_sequence = bypass_limits ? 0 : m_pool.GetSequence();
  if (!m_subpackage.m_changeset) {
    m_subpackage.m_changeset = m_pool.GetChangeSet();
  }
  ws.m_tx_handle = m_subpackage.m_changeset->StageAddition(
      ptx, ws.m_base_fees, nAcceptTime, m_active_chainstate.m_chain.Height(),
      entry_sequence, fSpendsCoinbase, nSigOpsCost, lock_points.value());
  ...
  if (m_pool.m_opts.require_standard) {
    if (!PreCheckEphemeralTx(*ptx, m_pool.m_opts.dust_relay_feerate,
                             ws.m_base_fees, ws.m_modified_fees, state)) {
  ...
  // from disconnected blocks. This requirement, unlike CheckFeeRate, cannot be
  // bypassed using m_package_feerates because, while a tx could be package
  // CPFP'd when entering the mempool, we do not have a DoS-resistant method of
  // ensuring the tx remains bumped. For example, the fee-bumping child could
  // disappear due to a replacement. The only exception is TRUC transactions.
  if (!bypass_limits && ws.m_ptx->version != TRUC_VERSION &&
      ws.m_modified_fees < m_pool.m_opts.min_relay_feerate.GetFee(ws.m_vsize)) {
    // Even though this is a fee-related failure, this result is
    // TX_MEMPOOL_POLICY, not TX_RECONSIDERABLE, because it cannot be bypassed
    // using package validation.
    return state.Invalid(
        TxValidationResult::TX_MEMPOOL_POLICY, "min relay fee not met",
        strprintf("%d < %d", ws.m_modified_fees,
                  m_pool.m_opts.min_relay_feerate.GetFee(ws.m_vsize)));
  }
  // No individual transactions are allowed below the mempool min feerate except
  // from disconnected blocks and transactions in a package. Package
  ...
  if (!args.m_bypass_limits) {
    ...
    if (const auto err{SingleTRUCChecks(m_pool, ws.m_ptx, ws.m_parents,
                                        ws.m_conflicts, ws.m_vsize)}) {
      // Single transaction contexts only.
      if (args.m_allow_sibling_eviction && err->second != nullptr) {
        // We should only be considering where replacement is considered valid
        // as well.
        Assume(args.m_allow_replacement);
        // Potential sibling eviction. Add the sibling to our list of mempool
        // conflicts to be included in RBF checks.
        ws.m_conflicts.insert(err->second->GetHash());
        // Adding the sibling to m_iters_conflicting here means that it doesn't
        // count towards RBF Carve Out above. This is correct, since removing
        // to-be-replaced transactions from the descendant count is done
        // separately in SingleTRUCChecks for TRUC transactions.
        ws.m_iters_conflicting.insert(
            m_pool.GetIter(err->second->GetHash()).value());
        ws.m_sibling_eviction = true;
        // The sibling will be treated as part of the to-be-replaced set in
        // ReplacementChecks. Note that we are not checking whether it opts in
        // to replaceability via BIP125 or TRUC (which is normally done in
        // PreChecks). However, the only way a TRUC transaction can have a
        // non-TRUC and non-BIP125 descendant is due to a reorg.
      } else {
        return state.Invalid(TxValidationResult::TX_MEMPOOL_POLICY,
                             "TRUC-violation", err->first);
      }
    }
  }

  // We want to detect conflicts in any tx in a package to trigger package RBF
  // logic
...
bool MemPoolAccept::ReplacementChecks(Workspace &ws) {
  ...
  if (const auto err_string{GetEntriesForConflicts(
          tx, m_pool, ws.m_iters_conflicting, all_conflicts)}) {
    return state.Invalid(
        TxValidationResult::TX_MEMPOOL_POLICY,
        strprintf("too many potential replacements%s",
                  ws.m_sibling_eviction ? " (including sibling eviction)" : ""),
        *err_string);
  }

  // Check if it's economically rational to mine this transaction rather than
  // the ones it replaces and pays for its own relay fees. Enforce Rules #3 and
  // #4.
  ...
  if (const auto err_string{PaysForRBF(
          m_subpackage.m_conflicting_fees, ws.m_modified_fees, ws.m_vsize,
          m_pool.m_opts.incremental_relay_feerate, hash)}) {
    // Result may change in a package context
    return state.Invalid(
        TxValidationResult::TX_RECONSIDERABLE,
        strprintf("insufficient fee%s",
                  ws.m_sibling_eviction ? " (including sibling eviction)" : ""),
        *err_string);
  }
  ...
  if (const auto err_string{
          ImprovesFeerateDiagram(*m_subpackage.m_changeset)}) {
    ...
    return state.Invalid(TxValidationResult::TX_RECONSIDERABLE,
                         "replacement-failed", err_string->second);
  }
...
bool MemPoolAccept::PackageMempoolChecks(
    ...
    std::vector<Workspace> &workspaces, const int64_t total_vsize,
    PackageValidationState &package_state) {
  AssertLockHeld(cs_main);
  AssertLockHeld(m_pool.cs);
  ...
  // N.B. To relax this constraint we will need to revisit how
  // CCoinsViewMemPool::PackageAddTransaction is being used inside
  // AcceptMultipleTransactions to track available inputs while processing a
  // package. Specifically we would need to check that the ancestors of the new
  ...
  // Don't consider replacements that would cause us to remove a large number of
  // mempool entries. This limit is not increased in a package RBF. Use the
  // aggregate number of transactions.
  CTxMemPool::setEntries all_conflicts;
  if (const auto err_string{GetEntriesForConflicts(
          *child_ws.m_ptx, m_pool, direct_conflict_iters, all_conflicts)}) {
    return package_state.Invalid(
        PackageValidationResult::PCKG_POLICY,
  ...
  // Ensure this two transaction package is a "chunk" on its own; we don't want
  // the child to be only paying anti-DoS fees
  const CFeeRate parent_feerate(parent_ws.m_modified_fees, parent_ws.m_vsize);
  const CFeeRate package_feerate(m_subpackage.m_total_modified_fees,
  ...
  if (package_feerate <= parent_feerate) {
    return package_state.Invalid(
        ...
        "feerate",
        strprintf("package feerate %s <= parent feerate is %s",
                  package_feerate.ToString(), parent_feerate.ToString()));
  }
  ...
  // Check if it's economically rational to mine this package rather than the
  // ones it replaces.
  ...
  LogDebug(BCLog::TXPACKAGES,
           "package RBF checks passed: parent %s (wtxid=%s), child %s "
           "(wtxid=%s), package hash (%s)\n",
           txns.front()->GetHash().ToString(),
           txns.front()->GetWitnessHash().ToString(),
           txns.back()->GetHash().ToString(),
           txns.back()->GetWitnessHash().ToString(),
...
bool MemPoolAccept::PolicyScriptChecks(const ATMPArgs &args, Workspace &ws) {
  ...
  constexpr script_verify_flags scriptVerifyFlags =
  ...
  if (!CheckInputScripts(tx, state, m_view, scriptVerifyFlags, true, false,
    ...
    // Detect a failure due to a missing witness so that p2p code can handle
    // rejection caching appropriately.
...
bool MemPoolAccept::ConsensusScriptChecks(const ATMPArgs &args, Workspace &ws) {
  ...
  script_verify_flags currentBlockScriptVerifyFlags{GetBlockScriptFlags(
      *m_active_chainstate.m_chain.Tip(), m_active_chainstate.m_chainman)};
  if (!CheckInputsFromMempoolAndCache(
          tx, state, m_view, m_pool, currentBlockScriptVerifyFlags,
          ws.m_precomputed_txdata, m_active_chainstate.CoinsTip(),
          GetValidationCache())) {
    LogError("BUG! PLEASE REPORT THIS! CheckInputScripts failed against "
...
void MemPoolAccept::FinalizeSubpackage(const ATMPArgs &args) {
  ...
  for (CTxMemPool::txiter it : m_subpackage.m_changeset->GetRemovals()) {
    std::string log_string = strprintf(
        "replacing mempool tx %s (wtxid=%s, fees=%s, vsize=%s). ",
        it->GetTx().GetHash().ToString(),
        it->GetTx().GetWitnessHash().ToString(), it->GetFee(), it->GetTxSize());
    FeeFrac feerate{m_subpackage.m_total_modified_fees,
                    int32_t(m_subpackage.m_total_vsize)};
    ...
    if (replaced_with_tx) {
      ...
      log_string += strprintf(
          "New tx %s (wtxid=%s, fees=%s, vsize=%s)", tx.GetHash().ToString(),
          tx.GetWitnessHash().ToString(), feerate.fee, feerate.size);
    } else {
      tx_or_package_hash =
    ...
    TRACEPOINT(mempool, replaced, it->GetTx().GetHash().data(), it->GetTxSize(),
               it->GetFee(),
               std::chrono::duration_cast<std::chrono::duration<std::uint64_t>>(
                   it->GetTime())
                   .count(),
               tx_or_package_hash.data(), feerate.size, feerate.fee,
               replaced_with_tx);
...
bool MemPoolAccept::SubmitPackage(
  ...
  AssertLockHeld(cs_main);
  AssertLockHeld(m_pool.cs);
  // Sanity check: none of the transactions should be in the mempool, and none
  // of the transactions should have a same-txid-different-witness equivalent in
  // the mempool.
  assert(std::all_of(
      workspaces.cbegin(), workspaces.cend(),
      [this](const auto &ws) { return !m_pool.exists(ws.m_ptx->GetHash()); }));
  ...
  for (Workspace &ws : workspaces) {
    if (!ConsensusScriptChecks(args, ws)) {
      ...
      if (!m_subpackage.m_changeset)
        m_subpackage.m_changeset = m_pool.GetChangeSet();
      m_subpackage.m_changeset->StageRemoval(
          m_pool.GetIter(ws.m_ptx->GetHash()).value());
    }
  }
  if (!all_submitted) {
  ...
  std::transform(workspaces.cbegin(), workspaces.cend(),
                 std::back_inserter(all_package_wtxids),
                 [](const auto &ws) { return ws.m_ptx->GetWitnessHash(); });

  if (!m_subpackage.m_replaced_transactions.empty()) {
    LogDebug(BCLog::MEMPOOL,
             "replaced %u mempool transactions with %u new one(s) for %s "
             "additional fees, %d delta bytes\n",
             m_subpackage.m_replaced_transactions.size(), workspaces.size(),
             m_subpackage.m_total_modified_fees -
                 m_subpackage.m_conflicting_fees,
             m_subpackage.m_total_vsize -
  ...
  for (Workspace &ws : workspaces) {
    ...
    const auto effective_feerate_wtxids =
        args.m_package_feerates
            ? all_package_wtxids
            : std::vector<Wtxid>{ws.m_ptx->GetWitnessHash()};
    results.emplace(ws.m_ptx->GetWitnessHash(),
                    MempoolAcceptResult::Success(
                        std::move(m_subpackage.m_replaced_transactions),
                        ws.m_vsize, ws.m_base_fees, effective_feerate,
                        effective_feerate_wtxids));
    if (!m_pool.m_opts.signals)
      continue;
    const CTransaction &tx = *ws.m_ptx;
    const auto tx_info = NewMempoolTransactionInfo(
        ws.m_ptx, ws.m_base_fees, ws.m_vsize, (*iter)->GetHeight(),
        args.m_bypass_limits, args.m_package_submission,
        IsCurrentForFeeEstimation(m_active_chainstate),
        m_pool.HasNoInputsOf(tx));
    m_pool.m_opts.signals->TransactionAddedToMempool(
        tx_info, m_pool.GetAndIncrementSequence());
  }
  return all_submitted;
}

MempoolAcceptResult
MemPoolAccept::AcceptSingleTransactionInternal(const CTransactionRef &ptx,
                                               ATMPArgs &args) {
  AssertLockHeld(cs_main);
  AssertLockHeld(m_pool.cs);
  ...
  if (!PreChecks(args, ws)) {
    if (ws.m_state.GetResult() == TxValidationResult::TX_RECONSIDERABLE) {
      // Failed for fee reasons. Provide the effective feerate and which tx was
      // included.
      return MempoolAcceptResult::FeeFailure(
          ws.m_state, CFeeRate(ws.m_modified_fees, ws.m_vsize), single_wtxid);
    }
  ...
  if (m_subpackage.m_rbf && !ReplacementChecks(ws)) {
    if (ws.m_state.GetResult() == TxValidationResult::TX_RECONSIDERABLE) {
      // Failed for incentives-based fee reasons. Provide the effective feerate
      // and which tx was included.
      return MempoolAcceptResult::FeeFailure(
          ws.m_state, CFeeRate(ws.m_modified_fees, ws.m_vsize), single_wtxid);
    }
    return MempoolAcceptResult::Failure(ws.m_state);
  ...
  if (ws.m_conflicts.size()) {
    ...
    // pathological case by making sure ws.m_conflicts and this tx's ancestors
    // don't intersect.
    if (const auto err_string{EntriesAndTxidsDisjoint(ancestors, ws.m_conflicts,
                                                      ptx->GetHash())}) {
      // We classify this as a consensus error because a transaction depending
      // on something it conflicts with would be inconsistent.
      ws.m_state.Invalid(TxValidationResult::TX_CONSENSUS,
                         "bad-txns-spends-conflicting-tx", *err_string);
      return MempoolAcceptResult::Failure(ws.m_state);
    }
  }
  ...
  if (m_pool.m_opts.require_standard) {
    Wtxid dummy_wtxid;
    if (!CheckEphemeralSpends(/*package=*/{ptx},
                              m_pool.m_opts.dust_relay_feerate, m_pool,
                              ws.m_state, dummy_wtxid)) {
      return MempoolAcceptResult::Failure(ws.m_state);
    }
  }
  ...
  const CFeeRate effective_feerate{ws.m_modified_fees,
                                   static_cast<int32_t>(ws.m_vsize)};
  // Tx was accepted, but not added
  if (args.m_test_accept) {
    return MempoolAcceptResult::Success(
        std::move(m_subpackage.m_replaced_transactions), ws.m_vsize,
  ...
  if (!args.m_package_submission && !args.m_bypass_limits) {
    ...
    // If mempool contents change, then the m_view cache is dirty. Given this
    // isn't a package submission, we won't be using the cache anymore, but
    // clear it anyway for clarity.
    CleanupTemporaryCoins();

    if (!m_pool.exists(ws.m_hash)) {
      // The tx no longer meets our (new) mempool minimum feerate but could be
      // reconsidered in a package.
      ws.m_state.Invalid(TxValidationResult::TX_RECONSIDERABLE, "mempool full");
      return MempoolAcceptResult::FeeFailure(
          ws.m_state, CFeeRate(ws.m_modified_fees, ws.m_vsize),
          {ws.m_ptx->GetWitnessHash()});
    }
  }
  ...
  if (!m_subpackage.m_replaced_transactions.empty()) {
    LogDebug(BCLog::MEMPOOL,
             "replaced %u mempool transactions with 1 new transaction for %s "
             "additional fees, %d delta bytes\n",
...
PackageMempoolAcceptResult MemPoolAccept::AcceptMultipleTransactionsInternal(
  ...
  // lock.
  PackageValidationState package_state;
  if (!IsWellFormedPackage(txns, package_state, /*require_sorted=*/true))
  ...
  for (Workspace &ws : workspaces) {
    if (!PreChecks(args, ws)) {
      package_state.Invalid(PackageValidationResult::PCKG_TX,
                            "transaction failed");
      // Exit early to avoid doing pointless work. Update the failed tx result;
    ...
    // Individual modified feerate exceeded caller-defined max; abort
    // N.B. this doesn't take into account CPFPs. Chunk-aware validation may be
    // more robust.
    if (args.m_client_maxfeerate && CFeeRate(ws.m_modified_fees, ws.m_vsize) >
                                        args.m_client_maxfeerate.value()) {
    ...
    // transactions in the package to spend. If there are no conflicts within
    // the package, no transaction can spend a coin needed by another
    // transaction in the package. We also need to make sure that no package tx
    // replaces (or replaces the ancestor of) the parent of another package tx.
    // As long as we check these two things, we don't need to track the coins
    // spent. If a package tx conflicts with a mempool tx,
    // PackageMempoolChecks() ensures later that any package RBF attempt has
    // *no* in-mempool ancestors, so we don't have to worry about subsequent
    // transactions in same package spending the same in-mempool outpoints. This
    // needs to be revisited for general package RBF.
    m_viewmempool.PackageAddTransaction(ws.m_ptx);
  }

  // At this point we have all in-mempool parents, and we know every
  // transaction's vsize. Run the TRUC checks on the package.
  for (Workspace &ws : workspaces) {
    if (auto err{PackageTRUCChecks(m_pool, ws.m_ptx, ws.m_vsize, txns,
                                   ws.m_parents)}) {
      package_state.Invalid(PackageValidationResult::PCKG_POLICY,
                            "TRUC-violation", err.value());
      return PackageMempoolAcceptResult(package_state, {});
    }
  }
  ...
  // parents, it suffices to use the package feerate (total modified fees /
  // total virtual size) to check this requirement. Note that this is an
  // aggregate feerate; this function has not checked that there are
  // transactions too low feerate to pay for themselves, or that the child
  // transactions are higher feerate than their parents. Using aggregate feerate
  // may allow "parents pay for child" behavior and permit a child that is below
  // mempool minimum feerate. To avoid these behaviors, callers of
  // AcceptMultipleTransactions need to restrict txns topology (e.g. to ancestor
  // sets) and check the feerates of individuals and subsets.
  m_subpackage.m_total_vsize =
      std::accumulate(workspaces.cbegin(), workspaces.cend(), int64_t{0},
                      [](int64_t sum, auto &ws) { return sum + ws.m_vsize; });
  m_subpackage.m_total_modified_fees = std::accumulate(
      workspaces.cbegin(), workspaces.cend(), CAmount{0},
  ...
  if (args.m_package_feerates &&
      !CheckFeeRate(m_subpackage.m_total_vsize,
                    m_subpackage.m_total_modified_fees, placeholder_state)) {
    package_state.Invalid(PackageValidationResult::PCKG_TX,
                          "transaction failed");
    return PackageMempoolAcceptResult(
        package_state, {{workspaces.back().m_ptx->GetWitnessHash(),
                         MempoolAcceptResult::FeeFailure(
                             placeholder_state,
                             CFeeRate(m_subpackage.m_total_modified_fees,
                                      m_subpackage.m_total_vsize),
                             all_package_wtxids)}});
  }

  // Apply package mempool RBF checks.
  if (!PackageMempoolChecks(txns, workspaces, m_subpackage.m_total_vsize,
                            package_state)) {
    return PackageMempoolAcceptResult(package_state, std::move(results));
  }
  ...
  // Now that we've bounded the resulting possible ancestry count, check package
  // for dust spends
  if (m_pool.m_opts.require_standard) {
    TxValidationState child_state;
    Wtxid child_wtxid;
    if (!CheckEphemeralSpends(txns, m_pool.m_opts.dust_relay_feerate, m_pool,
                              child_state, child_wtxid)) {
...
void MemPoolAccept::CleanupTemporaryCoins() {
  ...
  // m_viewmempool. (2) Mempool coins from transactions in the mempool,
  // constructed by m_viewmempool. (3) Confirmed coins fetched from our current
  // UTXO set.
  //
  // (1) Temporary coins need to be removed, regardless of whether the
  // transaction was submitted. If the transaction was submitted to the mempool,
  // m_viewmempool will be able to fetch them from there. If it wasn't submitted
  // to mempool, it is incorrect to keep them - future calls may try to spend
  // those coins that don't actually exist. (2) Mempool coins also need to be
  // removed. If the mempool contents have changed as a result of submitting or
  // replacing transactions, coins previously fetched from mempool may now be
  ...
  // spent after we submitted a tx to mempool, but we have already checked that
  // the package does not have 2 transactions spending the same coin and we
  // check whether a mempool transaction spends conflicting coins
  // (CTxMemPool::GetConflictTx). Keeping them in m_view is an optimization to
  // not re-fetch confirmed coins if we later look up inputs for this
  // transaction again.
  for (const auto &outpoint : m_viewmempool.GetNonBaseCoins()) {
    // In addition to resetting m_viewmempool, we also need to manually delete
    // these coins from m_view because it caches copies of the coins it fetched
    // from m_viewmempool previously.
...
MemPoolAccept::AcceptSubPackage(const std::vector<CTransactionRef> &subpackage,
  ...
  // minimize re-fetching coins from the UTXO set. Clean up package feerate and
  // rbf calculations
  ClearSubPackageState();
...
PackageMempoolAcceptResult MemPoolAccept::AcceptPackage(const Package &package,
  ...
  // transaction fails by itself but succeeds later (i.e. when evaluated with a
  // fee-bumping child), the result in this map may be discarded.
  std::map<Wtxid, MempoolAcceptResult> individual_results_nonfinal;
  // Tracks whether we think package submission could result in successful entry
  // to the mempool
  ...
  for (const auto &tx : package) {
    ...
    // There are 3 possibilities: already in mempool, same-txid-diff-wtxid
    // already in mempool, or not in mempool. An already confirmed tx is treated
    // as one not in mempool, because all we know is that the inputs aren't
    // available.
    if (m_pool.exists(wtxid)) {
      ...
      // differences to pin or delay propagation of transactions. As such, it's
      // possible for some package transaction(s) to already be in the mempool,
      // and we don't want to reject the entire package in that case (as that
      // could be a censorship vector). De-duplicate the transactions that are
      // already in the mempool, and only call AcceptMultipleTransactions() with
      // the new transactions. This ensures we don't double-count transaction
      // counts and sizes when checking ancestor/descendant limits, or
      // double-count transaction fees for fee-related policy.
      const auto &entry{*Assert(m_pool.GetEntry(txid))};
      results_final.emplace(wtxid, MempoolAcceptResult::MempoolTx(
                                       entry.GetTxSize(), entry.GetFee()));
    } else if (m_pool.exists(txid)) {
      // Transaction with the same non-witness data but different witness (same
      // txid, different wtxid) already exists in the mempool.
      //
      // We don't allow replacement transactions right now, so just swap the
      // package transaction for the mempool one. Note that we are ignoring the
      // validity of the package transaction passed in.
      ...
      if (single_res.m_result_type == MempoolAcceptResult::ResultType::VALID) {
        // The transaction succeeded on its own and is now in the mempool. Don't
        // include it in package validation, because its fees should only be
        // "used" once.
        assert(m_pool.exists(wtxid));
        results_final.emplace(wtxid, single_res);
      } else if (package.size() == 1 || // If there is only one transaction, no
                                        // need to retry it "as a package"
                 (single_res.m_state.GetResult() !=
                      TxValidationResult::TX_RECONSIDERABLE &&
                  single_res.m_state.GetResult() !=
                      TxValidationResult::TX_MISSING_INPUTS)) {
        // Package validation policy only differs from individual policy in its
        // evaluation of feerate. For example, if a transaction fails here due
        // to violation of a consensus rule, the result will not change when it
        // is submitted as part of a package. To minimize the amount of repeated
        // work, unless the transaction fails due to feerate or missing inputs
        // (its parent is a previous transaction in the package that failed due
        // to feerate), don't run package validation. Note that this decision
        // might not make sense if different types of packages are allowed in
        // the future.  Continue individually validating the rest of the
        // transactions, because some of them may still be valid.
        quit_early = true;
        package_state_quit_early.Invalid(PackageValidationResult::PCKG_TX,
                                         "transaction failed");
  ...
  // may be evicted. If mempool contents change, then the m_view cache is dirty.
  // It has already been cleared above.
  LimitMempoolSize(m_pool, m_active_chainstate.CoinsTip());
  ...
  for (const auto &tx : package) {
    ...
    if (multi_submission_result.m_tx_results.contains(wtxid)) {
      ...
      // If it was submitted, check to see if the tx is still in the mempool. It
      // could have been evicted due to LimitMempoolSize() above.
      ...
      if (txresult.m_result_type == MempoolAcceptResult::ResultType::VALID &&
        ...
        mempool_full_state.Invalid(TxValidationResult::TX_MEMPOOL_POLICY,
                                   "mempool full");
        results_final.emplace(wtxid,
    ...
    } else if (const auto it{results_final.find(wtxid)};
               it != results_final.end()) {
      // Already-in-mempool transaction. Check to see if it's still there, as it
      // could have been evicted when LimitMempoolSize() was called.
      Assume(it->second.m_result_type !=
    ...
    } else if (const auto it{individual_results_nonfinal.find(wtxid)};
               it != individual_results_nonfinal.end()) {
      Assume(it->second.m_result_type ==
             MempoolAcceptResult::ResultType::INVALID);
...
MempoolAcceptResult AcceptToMemoryPool(Chainstate &active_chainstate,
  ...
  auto args = MemPoolAccept::ATMPArgs::SingleAccept(
      chainparams, accept_time, bypass_limits, coins_to_uncache, test_accept);
  MempoolAcceptResult result = MemPoolAccept(pool, active_chainstate)
                                   .AcceptSingleTransactionAndCleanup(tx, args);

  if (result.m_result_type != MempoolAcceptResult::ResultType::VALID) {
    // Remove coins that were not present in the coins cache before calling
    // AcceptSingleTransaction(); this is to prevent memory DoS in case we
    // receive a large number of invalid transactions that attempt to overrun
    // the in-memory coins cache
    // (`CCoinsViewCache::cacheCoins`).

    for (const COutPoint &hashTx : coins_to_uncache)
      active_chainstate.CoinsTip().Uncache(hashTx);
    TRACEPOINT(mempool, rejected, tx->GetHash().data(),
  ...
  auto result = [&]() EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
    ...
    if (test_accept) {
      auto args = MemPoolAccept::ATMPArgs::PackageTestAccept(
          chainparams, GetTime(), coins_to_uncache);
      return MemPoolAccept(pool, active_chainstate)
          .AcceptMultipleTransactionsAndCleanup(package, args);
    } else {
      auto args = MemPoolAccept::ATMPArgs::PackageChildWithParents(
          chainparams, GetTime(), coins_to_uncache, client_maxfeerate);
      return MemPoolAccept(pool, active_chainstate)
...
Chainstate::Chainstate(CTxMemPool *mempool, BlockManager &blockman,
    ...
    : m_mempool(mempool), m_blockman(blockman), m_chainman(chainman),
      m_assumeutxo(from_snapshot_blockhash ? Assumeutxo::UNVALIDATED
...
const CBlockIndex *Chainstate::TargetBlock() const {
  if (!m_target_blockhash)
    return nullptr;
  if (!m_cached_target_block)
    m_cached_target_block =
        Assert(m_chainman.m_blockman.LookupBlockIndex(*m_target_blockhash));
...
void Chainstate::CheckForkWarningConditions() {
  ...
  if (m_chainman.m_best_invalid &&
    ...
    LogWarning("Found invalid chain more than 6 blocks longer than our best "
               "chain. This could be due to database corruption or consensus "
               "incompatibility with peers.");
    m_chainman.GetNotifications().warningSet(
        kernel::Warning::LARGE_WORK_INVALID_CHAIN,
        _("Warning: Found invalid chain more than 6 blocks longer than our "
          "best chain. This could be due to database corruption or consensus "
          "incompatibility with peers."));
  } else {
    m_chainman.GetNotifications().warningUnset(
        kernel::Warning::LARGE_WORK_INVALID_CHAIN);
  }
}
...
std::optional<std::pair<ScriptError, std::string>> CScriptCheck::operator()() {
  ...
  if (VerifyScript(scriptSig, m_tx_out.scriptPubKey, witness, m_flags,
    ...
    auto debug_str = strprintf(
        "input %i of %s (wtxid %s), spending %s:%i", nIn,
        ptxTo->GetHash().ToString(), ptxTo->GetWitnessHash().ToString(),
...
ValidationCache::ValidationCache(const size_t script_execution_cache_bytes,
  ...
  LogInfo("Using %zu MiB out of %zu MiB requested for script execution cache, "
 ...
 *
 * Note that we may set state.reason to NOT_STANDARD for extra soft-fork flags
...
bool CheckInputScripts(const CTransaction &tx, TxValidationState &state,
  ...
  if (validation_cache.m_script_execution_cache.contains(
          hashCacheEntry, !cacheFullScriptStore)) {
    return true;
  }
  ...
  for (unsigned int i = 0; i < tx.vin.size(); i++) {
    ...
    // Verify signature
    CScriptCheck check(txdata.m_spent_outputs[i], tx,
                       validation_cache.m_signature_cache, i, flags,
    ...
    if (pvChecks) {
      ...
      if (flags & STANDARD_NOT_MANDATORY_VERIFY_FLAGS) {
        return state.Invalid(TxValidationResult::TX_NOT_STANDARD,
                             strprintf("mempool-script-verify-flag-failed (%s)",
...
DisconnectResult Chainstate::DisconnectBlock(const CBlock &block,
  ...
  // Ignore blocks that contain transactions which are 'overwritten' by later
  // transactions, unless those are already completely spent. See
  // https://github.com/bitcoin/bitcoin/issues/22596 for additional information.
  // Note: the blocks specified here are different than the ones used in
  // ConnectBlock because DisconnectBlock unwinds the blocks in reverse. As a
  // result, the inconsistency is not discovered until the earlier blocks with
  // the duplicate coinbase transactions are disconnected.
  bool fEnforceBIP30 =
      !((pindex->nHeight == 91722 &&
         pindex->GetBlockHash() == uint256{"00000000000271a2dc26e7667f8419f2e15"
                                           "416dc6955e5a6c6cdf3f2574dd08e"}) ||
        (pindex->nHeight == 91812 &&
         pindex->GetBlockHash() == uint256{"00000000000af0aed4792b1acee3d966af3"
                                           "6cf5def14935db8de83d6f9306f2f"}));
...
bool Chainstate::ConnectBlock(const CBlock &block, BlockValidationState &state,
  ...
  if (!CheckBlock(block, state, params.GetConsensus(), !fJustCheck,
                  !fJustCheck)) {
    if (state.GetResult() == BlockValidationResult::BLOCK_MUTATED) {
      ...
      return FatalError(
          ...
          _("Corrupt block found indicating potential hardware failure."));
    }
  ...
  if (m_chainman.AssumedValidBlock().IsNull()) {
    ...
    if (it == m_blockman.m_block_index.end()) {
    ...
    } else if (m_chainman.m_best_header->GetAncestor(pindex->nHeight) !=
               pindex) {
      script_check_reason = "block not in best header chain";
    } else if (m_chainman.m_best_header->nChainWork <
      ...
      // This block is a member of the assumed verified chain and an ancestor of
      // the best header. Script verification is skipped when connecting blocks
      // under the
      ...
      //  implication of the demand. This also avoids having release candidates
      //  that are hardly doing any signature verification at all in testing
      //  without having to artificially set the default assumed verified block
      //  further back.
      // The test against the minimum chain work prevents the skipping when
      // denied access to any chain at
  ...
  // See BIP30, CVE-2012-1909, and https://r6.ca/blog/20120206T005236Z.html for
  // more information. This rule was originally applied to all blocks with a
  // timestamp after March 15, 2012, 0:00 UTC. Now that the whole chain is
  // irreversibly beyond that time it is applied to all blocks except the two in
  // the chain that violate it. This prevents exploiting the issue against nodes
  // during their initial block download.
  ...
  // sufficiently buried it's no longer possible to create further duplicate
  ...
  CBlockIndex *pindexBIP34height =
      pindex->pprev->GetAncestor(params.GetConsensus().BIP34Height);
  // Only continue to enforce if we're below BIP34 activation height or the
  // block hash at that height doesn't correspond.
  fEnforceBIP30 = fEnforceBIP30 &&
                  (!pindexBIP34height || !(pindexBIP34height->GetBlockHash() ==
                                           params.GetConsensus().BIP34Hash));
  ...
  if (script_check_reason != m_last_script_check_reason_logged &&
      role.validated && !role.historical) {
    if (fScriptChecks) {
  ...
  if (auto &queue = m_chainman.GetCheckQueue();
      queue.HasThreads() && fScriptChecks)
  ...
  for (unsigned int i = 0; i < block.vtx.size(); i++) {
    ...
    if (!tx.IsCoinBase()) {
      ...
      if (!Consensus::CheckTxInputs(tx, tx_state, view, pindex->nHeight,
                                    txfee)) {
    ...
    if (!tx.IsCoinBase() && fScriptChecks) {
      ...
      bool tx_ok;
      TxValidationState tx_state;
      // If CheckInputScripts is called with a pointer to a checks vector, the
      // resulting checks are appended to it. In that case they need to be added
      // to control which runs them asynchronously. Otherwise, CheckInputScripts
      // runs the checks before returning.
      if (control) {
        std::vector<CScriptCheck> vChecks;
        tx_ok = CheckInputScripts(tx, tx_state, view, flags, fCacheResults,
                                  fCacheResults, txsdata[i],
  ...
  LogDebug(
      ...
      "(%.2fms/blk)]\n",
      (unsigned)block.vtx.size(), Ticks<MillisecondsDouble>(time_3 - time_2),
  ...
  CAmount blockReward =
      nFees + GetBlockSubsidy(pindex->nHeight, params.GetConsensus());
  ...
  LogDebug(BCLog::BENCH, "    - Write undo data: %.2fms [%.2fs (%.2fms/blk)]\n",
           ...
           Ticks<MillisecondsDouble>(m_chainman.time_undo) /
               m_chainman.num_blocks_total);
  ...
  TRACEPOINT(validation, block_connected, block_hash.data(), pindex->nHeight,
             block.vtx.size(), nInputs, nSigOpsCost,
  ...
  try {
    {
      ...
      if (m_blockman.IsPruneMode() &&
        ...
        std::optional<std::string>
            limiting_lock; // prune lock that actually was the limiting factor,
                           // only used for logging

        for (const auto &prune_lock : m_blockman.m_prune_locks) {
          if (prune_lock.second.height_first == std::numeric_limits<int>::max())
            continue;
          // Remove the buffer and one additional block here to get actual
          // height that is outside of the buffer
        ...
        if (nManualPruneHeight > 0) {
          LOG_TIME_MILLIS_WITH_CATEGORY("find files to prune (manual)",
                                        BCLog::BENCH);

          m_blockman.FindFilesToPruneManual(
              setFilesToPrune, std::min(last_prune, nManualPruneHeight), *this);
        } else {
          LOG_TIME_MILLIS_WITH_CATEGORY("find files to prune", BCLog::BENCH);

          m_blockman.FindFilesToPrune(setFilesToPrune, last_prune, *this,
                                      m_chainman);
      ...
      // It's been a while since we wrote the block index and chain state to
      // disk. Do this frequently, so we don't need to redownload or reindex
      // after a crash.
      bool fPeriodicWrite =
          mode == FlushStateMode::PERIODIC && nNow >= m_next_write;
      // Combine all conditions that result in a write to disk.
      bool should_write = (mode == FlushStateMode::ALWAYS) || fCacheLarge ||
                          fCacheCritical || fPeriodicWrite || fFlushForPrune;
      // Write blocks, block index and best chain related state to disk.
      if (should_write) {
        LogDebug(BCLog::COINDB,
                 "Writing chainstate to disk: flush mode=%s, prune=%d, "
                 "large=%d, critical=%d, periodic=%d",
                 FlushStateModeNames[size_t(mode)], fFlushForPrune, fCacheLarge,
                 fCacheCritical, fPeriodicWrite);
        ...
        {
          LOG_TIME_MILLIS_WITH_CATEGORY("write block and undo data to disk",
                                        BCLog::BENCH);
        ...
        if (!CoinsTip().GetBestBlock().IsNull()) {
          if (coins_mem_usage >= WARN_FLUSH_COINS_SIZE)
            LogWarning("Flushing large (%d GiB) UTXO set to disk, it may take "
                       "several minutes",
                       coins_mem_usage >> 30);
          LOG_TIME_MILLIS_WITH_CATEGORY(
              strprintf("write coins cache to disk (%d coins, %.2fKiB)",
          ...
          if (empty_cache ? !CoinsTip().Flush() : !CoinsTip().Sync()) {
            return FatalError(m_chainman.GetNotifications(), state,
                              _("Failed to write to coin database."));
          }
          full_flush_completed = true;
          TRACEPOINT(utxocache, flush,
                     int64_t{Ticks<std::chrono::microseconds>(NodeClock::now() -
                                                              nNow)},
                     (uint32_t)mode, (uint64_t)coins_count,
                     (uint64_t)coins_mem_usage, (bool)fFlushForPrune);
        }
      }
    ...
    if (full_flush_completed && m_chainman.m_options.signals) {
      ...
      m_chainman.m_options.signals->ChainStateFlushed(
          this->GetRole(), GetLocator(m_chain.Tip()));
    }
  } catch (const std::runtime_error &e) {
    return FatalError(
        m_chainman.GetNotifications(), state,
        strprintf(_("System error while flushing: %s"), e.what()));
  }
  return true;
}
...
static void UpdateTipLog(const ChainstateManager &chainman,
                         const CCoinsViewCache &coins_tip,
                         const CBlockIndex *tip, const std::string &func_name,
...
void Chainstate::UpdateTip(const CBlockIndex *pindexNew) {
  ...
  if (!m_chainman.IsInitialBlockDownload()) {
    auto bits = m_chainman.m_versionbitscache.CheckUnknownActivations(
        pindexNew, m_chainman.GetParams());
    for (auto [bit, active] : bits) {
      const bilingual_str warning =
          strprintf(_("Unknown new rules activated (versionbit %i)"), bit);
      if (active) {
        m_chainman.GetNotifications().warningSet(
            kernel::Warning::UNKNOWN_NEW_RULES_ACTIVATED, warning);
      } else {
 ...
 * should make the mempool consistent again by calling
 * MaybeUpdateMempoolForReorg. with cs_main held.
 *
...
bool Chainstate::DisconnectTip(BlockValidationState &state,
  ...
  const auto time_start{SteadyClock::now()};
  {
    ...
    bool flushed = view.Flush(
        /*will_reuse_cache=*/false); // local CCoinsViewCache goes out of scope
    assert(flushed);
  }
  ...
  {
    // Prune locks that began at or after the tip should be moved backward so
    // they get a chance to reorg
...
/**
 * Connect a new block to m_chain. block_to_connect is either nullptr or a
 * pointer to a CBlock corresponding to pindexNew, to bypass loading it again
 * from disk.
 *
 * The block is added to connectTrace if connection succeeds.
 */
bool Chainstate::ConnectTip(BlockValidationState &state, CBlockIndex *pindexNew,
...
bool Chainstate::ActivateBestChain(BlockValidationState &state,
  ...
  // lock up other threads for too long during large connects - and to allow for
  // e.g. the callback queue to drain we use m_chainstate_mutex to enforce
  ...
  if (WITH_LOCK(::cs_main, return m_target_utxohash)) {
    LogError("%s", STR_INTERNAL_BUG("m_target_utxohash is set - this "
                                    "chainstate should not be in operation."));
    return Assume(false);
  }
  ...
  do {
...

    {
          ...
          for (const PerBlockConnectTrace &trace :
            ...
            if (m_chainman.m_options.signals) {
              m_chainman.m_options.signals->BlockConnected(
                  chainstate_role, trace.pblock, trace.pindex);
            }
          }

          // Break this do-while to ensure we don't advance past the target
          // block.
          if (ReachedTarget()) {
            break;
          }
                 ...
                 (starting_tip &&
                  CBlockIndexWorkComparator()(m_chain.Tip(), starting_tip)));
        if (!blocks_connected)
          return true;
        ...
        if (this == &m_chainman.ActiveChainstate() &&
          ...
          if (kernel::IsInterrupted(m_chainman.GetNotifications().blockTip(
                  /*state=*/GetSynchronizationState(
                      still_in_ibd, m_chainman.m_blockman.m_blockfiles_indexed),
                  /*index=*/*pindexNewTip,
                  /*verification_progress=*/
                  m_chainman.GuessVerificationProgress(pindexNewTip)))) {
      ...
      // Notify external listeners about the new tip, even if pindexFork ==
      // pindexNewTip.
      if (m_chainman.m_options.signals &&
          this == &m_chainman.ActiveChainstate()) {
        m_chainman.m_options.signals->ActiveTipChange(
            *Assert(pindexNewTip), m_chainman.IsInitialBlockDownload());
      }
    } // release cs_main
    ...
    // We check interrupt only after giving ActivateBestChainStep a chance to
    // run once so that we never interrupt before connecting the genesis block
    // during LoadChainTip(). Previously this caused an assert() failure during
    // interrupt in such cases as the UTXO DB flushing checks that the best
    // block hash is non-null.
...
bool Chainstate::InvalidateBlock(BlockValidationState &state,
  ...
  while (true) {
    ...
    MaybeUpdateMempoolForReorg(
        disconnectpool, /* fAddToMempool = */ (++disconnected <= 10) && ret);
    if (!ret)
      return false;
    ...
    if (invalid_walk_tip == to_mark_failed->pprev &&
        (to_mark_failed->nStatus & BLOCK_FAILED_VALID)) {
      // We only want to mark the last disconnected block as BLOCK_FAILED_VALID;
      // its children need to be BLOCK_FAILED_CHILD instead.
      to_mark_failed->nStatus =
          (to_mark_failed->nStatus ^ BLOCK_FAILED_VALID) | BLOCK_FAILED_CHILD;
      m_blockman.m_dirty_blockindex.insert(to_mark_failed);
    }

    // Mark out-of-chain descendants of the invalidated block as invalid
    // (possibly replacing a pre-existing BLOCK_FAILED_VALID with
    // BLOCK_FAILED_CHILD) Add any equal or more work headers that are not
    // invalidated to setBlockIndexCandidates Recalculate m_best_header if it
    // became invalid.
    auto candidate_it = highpow_outofchain_headers.lower_bound(
        invalid_walk_tip->pprev->nChainWork);

    const bool best_header_needs_update{
        m_chainman.m_best_header->GetAncestor(invalid_walk_tip->nHeight) ==
        invalid_walk_tip};
    if (best_header_needs_update) {
      // pprev is definitely still valid at this point, but there may be better
      // ones
    ...
    while (candidate_it != highpow_outofchain_headers.end()) {
      ...
      if (candidate->GetAncestor(invalid_walk_tip->nHeight) ==
          invalid_walk_tip) {
        // Children of failed blocks should be marked as BLOCK_FAILED_CHILD
        // instead.
      ...
      if (!CBlockIndexWorkComparator()(candidate, invalid_walk_tip->pprev) &&
        ...
        // Do not remove candidate from the highpow_outofchain_headers cache,
        // because it might be a descendant of the block being invalidated which
        // needs to be marked failed later.
      }
      if (best_header_needs_update &&
    ...
    // BLOCK_FAILED_CHILD status in future iterations, or, if it's the last one,
    // call InvalidChainFound on it.
...

  {
    ...
    if (m_chain.Contains(to_mark_failed)) {
      // If the to-be-marked invalid block is in the active chain, something is
      // interfering and we can't proceed.
      return false;
    }
  ...
  if (pindex_was_in_chain) {
    ...
    (void)m_chainman.GetNotifications().blockTip(
        ...
        WITH_LOCK(m_chainman.GetMutex(),
                  return m_chainman.GuessVerificationProgress(
                      to_mark_failed->pprev)));

    // Fire ActiveTipChange now for the current chain tip to make sure clients
    // are notified. ActivateBestChain may call this as well, but not
    // necessarily.
    if (m_chainman.m_options.signals) {
      m_chainman.m_options.signals->ActiveTipChange(
          *Assert(m_chain.Tip()), m_chainman.IsInitialBlockDownload());
    }
  }
  return true;
}
...
void Chainstate::SetBlockFailureFlags(CBlockIndex *invalid_block) {
  ...
  for (auto &[_, block_index] : m_blockman.m_block_index) {
    if (invalid_block != &block_index &&
        block_index.GetAncestor(invalid_block->nHeight) == invalid_block) {
      block_index.nStatus =
          (block_index.nStatus & ~BLOCK_FAILED_VALID) | BLOCK_FAILED_CHILD;
...
void Chainstate::ResetBlockFailureFlags(CBlockIndex *pindex) {
  ...
  // Remove the invalidity flag from this block and all its descendants and
  // ancestors.
  for (auto &[_, block_index] : m_blockman.m_block_index) {
...
void ChainstateManager::ReceivedBlockTransactions(const CBlock &block,
  ...
  // Typically m_chain_tx_count will be 0 at this point, but it can be nonzero
  // if this is a pruned block which is being downloaded again, or if this is an
  // assumeutxo snapshot block which has a hardcoded m_chain_tx_count value from
  // the snapshot metadata. If the pindex is not the snapshot block and the
  ...
  if (!Assume(pindexNew->m_chain_tx_count == 0 ||
              ...
              std::ranges::any_of(m_chainstates,
                                  [&](const auto &cs)
                                      EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
                                        return cs->SnapshotBase() == pindexNew;
                                      }))) {
    LogWarning(
        "Internal bug detected: block %d has unexpected m_chain_tx_count %i "
        "that should be %i (%s %s). Please report this issue here: %s\n",
        pindexNew->nHeight, pindexNew->m_chain_tx_count,
        prev_tx_sum(*pindexNew), CLIENT_NAME, FormatFullVersion(),
        CLIENT_BUGREPORT);
  ...
  if (pindexNew->pprev == nullptr || pindexNew->pprev->HaveNumChainTxs()) {
    ...
    while (!queue.empty()) {
      ...
      if (!Assume(pindex->m_chain_tx_count == 0 ||
        ...
        LogWarning(
            "Internal bug detected: block %d has unexpected m_chain_tx_count "
            "%i that should be %i (%s %s). Please report this issue here: %s\n",
...
static bool CheckWitnessMalleation(const CBlock &block,
  ...
  if (expect_witness_commitment) {
    ...
    if (commitpos != NO_WITNESS_COMMITMENT) {
      ...
      CHash256()
          .Write(hash_witness)
          .Write(witness_stack[0])
          .Finalize(hash_witness);
      if (memcmp(hash_witness.begin(),
...
bool CheckBlock(const CBlock &block, BlockValidationState &state,
  ...
  for (const auto &tx : block.vtx) {
    ...
    if (!CheckTransaction(*tx, tx_state)) {
      ...
      return state.Invalid(
          BlockValidationResult::BLOCK_CONSENSUS, tx_state.GetRejectReason(),
...
void ChainstateManager::UpdateUncommittedBlockStructures(
    CBlock &block, const CBlockIndex *pindexPrev) const {
  int commitpos = GetWitnessCommitmentIndex(block);
...
std::vector<unsigned char> ChainstateManager::GenerateCoinbaseCommitment(
    CBlock &block, const CBlockIndex *pindexPrev) const {
...
bool IsBlockMutated(const CBlock &block, bool check_witness_root) {
  ...
  if (block.vtx.empty() || !block.vtx[0]->IsCoinBase()) {
    ...
    return std::any_of(block.vtx.begin(), block.vtx.end(), [](auto &tx) {
      return GetSerializeSize(TX_NO_WITNESS(tx)) == 64;
    });
  } else {
...
arith_uint256
CalculateClaimedHeadersWork(std::span<const CBlockHeader> headers) {
 ...
 *
 *  NOTE: failing to check the header's height against the last checkpoint's
 * opened a DoS vector between v0.12 and v0.15 (when no additional protection
...
ContextualCheckBlockHeader(const CBlockHeader &block,
  ...
  // Testnet4 and regtest only: Check timestamp against prev for
  // difficulty-adjustment blocks to prevent timewarp attacks (see
  ...
  if (consensusParams.enforce_BIP94) {
    ...
    if (nHeight % consensusParams.DifficultyAdjustmentInterval() == 0) {
      if (block.GetBlockTime() < pindexPrev->GetBlockTime() - MAX_TIMEWARP) {
        return state.Invalid(
            BlockValidationResult::BLOCK_INVALID_HEADER, "time-timewarp-attack",
  ...
  // Check timestamp
  if (block.Time() >
      NodeClock::now() + std::chrono::seconds{MAX_FUTURE_BLOCK_TIME}) {
...
static bool ContextualCheckBlock(const CBlock &block,
  ...
  // containing a witness reserved value (unconstrained).
  ...
  //   root, witness reserved value). In case there are multiple, the last one
  //   is used.
    ...
    if (miSelf != m_blockman.m_block_index.end()) {
      ...
      if (pindex->nStatus & BLOCK_FAILED_MASK) {
        ...
        return state.Invalid(BlockValidationResult::BLOCK_CACHED_INVALID,
                             "duplicate-invalid",
                             strprintf("block %s was previously marked invalid",
                                       hash.ToString()));
      }
      return true;
    }
...
bool ChainstateManager::ProcessNewBlockHeaders(
    std::span<const CBlockHeader> headers, bool min_pow_checked,
...
void ChainstateManager::ReportHeadersPresync(const arith_uint256 &work,
  ...
  AssertLockNotHeld(GetMutex());
  {
    ...
    // post-minchainwork header chain. This means we lose reporting for
    // potentially legitimate, but unlikely, deep reorgs, but prevent attackers
    // that spam low-work headers from filling our logs.
    ...
    if (now < m_last_presync_update + std::chrono::milliseconds{250})
      return;
...
bool ChainstateManager::AcceptBlock(const std::shared_ptr<const CBlock> &pblock,
  ...
  // fRequested This requires some new chain data structure to efficiently look
  // up if a block is in a chain leading to a candidate for best tip, despite
  // not being such a candidate itself. Note that this would break the
  ...
  try {
    ...
    if (dbp) {
      ...
      if (blockPos.IsNull()) {
        state.Error(
            strprintf("%s: Failed to find position to write new block to disk",
                      __func__));
        return false;
      }
    }
    ...
    return FatalError(
        ...
        strprintf(_("System error while saving block to disk: %s"), e.what()));
  }
...
bool ChainstateManager::ProcessNewBlock(
...

  {
    ...
    // CheckBlock() does not support multi-threaded block validation because
    // CBlock::fChecked can cause data race. Therefore, the following critical
    // section must include the CheckBlock() call as well.
    LOCK(cs_main);

    // Skipping AcceptBlock() for CheckBlock() failures means that we will never
    // mark a block as invalid if CheckBlock() fails.  This is protective
    // against consensus failure if there are any unknown forms of block
...
ChainstateManager::ProcessTransaction(const CTransactionRef &tx,
  ...
  active_chainstate.GetMempool()->check(active_chainstate.CoinsTip(),
...
BlockValidationState TestBlockValidity(Chainstate &chainstate,
  ...
  // 2. To prevent a CheckBlock() race condition for fChecked, see
  // ProcessNewBlock()
  ...
  // Set fJustCheck to true in order to update, and not clear, validation
  // caches.
  if (!chainstate.ConnectBlock(block, state, &index_dummy, view_dummy,
                               /*fJustCheck=*/true)) {
    if (state.IsValid())
      NONFATAL_UNREACHABLE();
    return state;
...
void PruneBlockFilesManual(Chainstate &active_chainstate,
  ...
  if (!active_chainstate.FlushStateToDisk(state, FlushStateMode::NONE,
                                          nManualPruneHeight)) {
    LogWarning("Failed to flush state after manual prune (%s)",
               state.ToString());
  }
}
...
bool Chainstate::LoadChainTip() {
  ...
  assert(!coins_cache.GetBestBlock()
              .IsNull()); // Never called when the coins view is empty
  CBlockIndex *tip = m_chain.Tip();
  ...
  // Make sure our chain tip before shutting down scores better than any other
  // candidate to maintain a consistent best tip over reboots in case of a tie.
  auto target = tip;
  while (target) {
  ...
  // Block index candidates are loaded before the chain tip, so we need to
  // replace this entry Otherwise the scoring will be based on the memory
  // address location instead of the nSequenceId
  ...
  if (!this->GetRole().historical) {
    ...
    (void)m_chainman.GetNotifications().blockTip(
        /*state=*/GetSynchronizationState(
            /*init=*/true, m_chainman.m_blockman.m_blockfiles_indexed),
        /*index=*/*pindex,
...
VerifyDBResult CVerifyDB::VerifyDB(Chainstate &chainstate,
  ...
  for (pindex = chainstate.m_chain.Tip(); pindex && pindex->pprev;
    ...
    const int percentageDone = std::max(
        1, std::min(
               99,
               (int)(((double)(chainstate.m_chain.Height() - pindex->nHeight)) /
    ...
    if ((chainstate.m_blockman.IsPruneMode() || is_snapshot_cs) &&
      ...
      LogInfo("Block verification stopping at height %d (no data). This could "
              "be due to pruning or use of an assumeutxo snapshot.",
              pindex->nHeight);
    ...
    if (!chainstate.m_blockman.ReadBlock(block, *pindex)) {
      LogError("Verification error: ReadBlock failed at %d, hash=%s",
    ...
    size_t curr_coins_usage =
        coins.DynamicMemoryUsage() + chainstate.CoinsTip().DynamicMemoryUsage();
    ...
    if (nCheckLevel >= 3) {
      if (curr_coins_usage <= chainstate.m_coinstip_cache_size_bytes) {
        ...
        if (res == DISCONNECT_FAILED) {
          LogError("Verification error: irrecoverable inconsistency in block "
                   "data at %d, hash=%s",
                   pindex->nHeight, pindex->GetBlockHash().ToString());
          return VerifyDBResult::CORRUPTED_BLOCK_DB;
        }
  ...
  if (pindexFailure) {
    LogError("Verification error: coin database inconsistencies found (last %i "
  ...
  if (skipped_l3_checks) {
    LogWarning("Skipped verification of level >=3 (insufficient database cache "
               "size). Consider increasing -dbcache.");
  }
  ...
  if (nCheckLevel >= 4 && !skipped_l3_checks) {
    while (pindex != chainstate.m_chain.Tip()) {
      ...
      if (!chainstate.ConnectBlock(block, state, pindex, coins)) {
        LogError(
            "Verification error: found unconnectable block at %d, hash=%s (%s)",
            pindex->nHeight, pindex->GetBlockHash().ToString(),
            state.ToString());
        return VerifyDBResult::CORRUPTED_BLOCK_DB;
      }
  ...
  LogInfo("Verification: No coin database inconsistencies in last %i blocks "
          "(%i transactions)",
...
bool Chainstate::ReplayBlocks() {
  ...
  if (pindexOld != pindexFork) {
    LogInfo("Rolling back from %s (%i to %i)",
    ...
    while (pindexOld != pindexFork) {
      if (pindexOld->nHeight > 0) { // Never disconnect the genesis block.
        ...
        if (pindexOld->nHeight % 10'000 == 0) {
          LogInfo("Rolling back %s (%i)", pindexOld->GetBlockHash().ToString(),
                  pindexOld->nHeight);
        }
        ...
        // deleted, or an existing UTXO was overwritten. It corresponds to cases
        // where the block-to-be-disconnect never had all its operations applied
        // to the UTXO set. However, as both writing a UTXO and deleting a UTXO
        // are idempotent operations, the result is still a version of the UTXO
        // set with the effects of that block undone.
      }
  ...
  if (nForkHeight < pindexNew->nHeight) {
    LogInfo("Rolling forward to %s (%i to %i)",
            pindexNew->GetBlockHash().ToString(), nForkHeight,
            pindexNew->nHeight);
    for (int nHeight = nForkHeight + 1; nHeight <= pindexNew->nHeight;
      ...
      if (nHeight % 10'000 == 0) {
        LogInfo("Rolling forward %s (%i)", pindex.GetBlockHash().ToString(),
                nHeight);
      }
...
bool Chainstate::NeedsRedownload() const {
  ...
  // At and above m_params.SegwitHeight, segwit consensus rules must be
  // validated
  CBlockIndex *block{m_chain.Tip()};

  while (block != nullptr &&
         DeploymentActiveAt(*block, m_chainman, Consensus::DEPLOYMENT_SEGWIT)) {
...
bool ChainstateManager::LoadBlockIndex() {
  ...
  if (m_blockman.m_blockfiles_indexed) {
    bool ret{m_blockman.LoadBlockIndexDB(
        CurrentChainstate().m_from_snapshot_blockhash)};
    if (!ret)
      return false;
...
void ChainstateManager::LoadExternalBlockFile(
  ...
  int nLoaded = 0;
  try {
    BufferedFile blkdat{file_in, 2 * MAX_BLOCK_SERIALIZED_SIZE,
                        MAX_BLOCK_SERIALIZED_SIZE + 8};
    ...
    while (!blkdat.eof()) {
      ...
      try {
        ...
        // Skip the rest of this block (this may read from disk into memory);
        // position to the marker before the next block, but it's still possible
        // to rewind to the start of the current block (without a disk read).
        nRewind = nBlockPos + nSize;
        blkdat.SkipTo(nRewind);

        std::shared_ptr<CBlock>
            pblock{}; // needs to remain available after the cs_main lock is
                      // released to avoid duplicate reads from disk

        {
          ...
          if (!pindex || (pindex->nStatus & BLOCK_HAVE_DATA) == 0) {
            // This block can be processed immediately; rewind to its start,
        ...
        // ActivateBestChain only connects blocks which are in the block tree
        // db, which only contains blocks whose parents are in it. But do this
        // only if genesis isn't activated yet, to avoid connecting many blocks
        ...
        if (m_blockman.IsPruneMode() && m_blockman.m_blockfiles_indexed &&
            pblock) {
          // must update the tip for pruning to work while importing with
          // -loadblock. this is a tradeoff to conserve disk space at the
          // expense of time spent updating the tip to be able to prune.
          // otherwise, ActivateBestChain won't be called by the import process
          // until after all of the block files are loaded. ActivateBestChain
          // can be called by concurrent network message processing. but, that
          // is not reliable for the purpose of pruning while importing.
        ...
        while (!queue.empty()) {
          ...
          while (range.first != range.second) {
            ...
            if (m_blockman.ReadBlock(*pblockrecursive, it->second, {})) {
              ...
              LogDebug(BCLog::REINDEX,
                       "%s: Processing out of order child %s of %s", __func__,
                       block_hash.ToString(), head.ToString());
              LOCK(cs_main);
              BlockValidationState dummy;
              if (AcceptBlock(pblockrecursive, dummy, nullptr, true,
        ...
        // historical bugs added extra data to the block files that does not
        // deserialize cleanly. commonly this data is between readable blocks,
        // but it does not really matter. such data is not fatal to the import
        // process. the code that reads the block files deals with invalid data
        // by simply ignoring it. it continues to search for the next {4 byte
        // magic message start bytes + 4 byte length + block} that does
        // deserialize cleanly and passes all of the other block validation
        // checks dealing with POW and the merkle root, etc... we merely note
        // with this informational log message when unexpected data is
        // encountered. we could also be experiencing a storage system read
        // error, or a read of a previous bad write. these are possible, but
        // less likely scenarios. we don't have enough information to tell a
        // difference here. the reindex process is not the place to attempt to
        // clean and/or compact the block files. if so desired, a studious node
        // operator may use knowledge of the fact that the block files are not
        // entirely pristine in order to prepare a set of pristine, and perhaps
    ...
    GetNotifications().fatalError(strprintf(
        _("System error while loading external block file: %s"), e.what()));
  }
  LogInfo("Loaded %i blocks from external file in %dms", nLoaded,
...
void ChainstateManager::CheckBlockIndex() const {
  ...
  // ActivateBestChain, so we have the genesis block in m_blockman.m_block_index
  ...
  // For performance reasons, indexes of the best header chain are stored in a
  // vector (within CChain). All remaining blocks are stored in a multimap. The
  // best header chain can differ from the active chain: E.g. its entries may
  // belong to blocks that are not yet validated.
  ...
  for (auto &[_, block_index] : m_blockman.m_block_index) {
    ...
    if (!best_hdr_chain.Contains(&block_index)) {
      // Only genesis, which must be part of the best header chain, can have a
      // nullptr parent.
  ...
  size_t nNodes = 0;
  int nHeight = 0;
  const CBlockIndex *pindexFirstInvalid =
      nullptr; // Oldest ancestor of pindex which is invalid.
  const CBlockIndex *pindexFirstMissing =
      nullptr; // Oldest ancestor of pindex which does not have BLOCK_HAVE_DATA,
               // since assumeutxo snapshot if used.
  const CBlockIndex *pindexFirstNeverProcessed =
      nullptr; // Oldest ancestor of pindex for which nTx == 0, since assumeutxo
               // snapshot if used.
  const CBlockIndex *pindexFirstNotTreeValid =
      nullptr; // Oldest ancestor of pindex which does not have BLOCK_VALID_TREE
               // (regardless of being valid or not).
  const CBlockIndex *pindexFirstNotTransactionsValid =
      nullptr; // Oldest ancestor of pindex which does not have
               // BLOCK_VALID_TRANSACTIONS (regardless of being valid or not),
               // since assumeutxo snapshot if used.
  const CBlockIndex *pindexFirstNotChainValid =
      nullptr; // Oldest ancestor of pindex which does not have
               // BLOCK_VALID_CHAIN (regardless of being valid or not), since
               // assumeutxo snapshot if used.
  const CBlockIndex *pindexFirstNotScriptsValid =
      nullptr; // Oldest ancestor of pindex which does not have
               // BLOCK_VALID_SCRIPTS (regardless of being valid or not), since
               // assumeutxo snapshot if used.
  ...
  const CBlockIndex *snap_first_missing{}, *snap_first_notx{},
      *snap_first_notv{}, *snap_first_nocv{}, *snap_first_nosv{};
  ...
  while (pindex != nullptr) {
    ...
    if (pindex->pprev == nullptr) {
      ...
      assert(
          pindex->GetBlockHash() ==
          GetConsensus().hashGenesisBlock); // Genesis block's hash must match.
      for (const auto &c : m_chainstates) {
        if (c->m_chain.Genesis() != nullptr) {
          assert(pindex == c->m_chain.Genesis()); // The chain's genesis block
                                                  // must be this block.
        }
      }
    }
    // nSequenceId can't be set higher than SEQ_ID_INIT_FROM_DISK{1} for blocks
    // that aren't linked (negative is used for preciousblock,
    // SEQ_ID_BEST_CHAIN_FROM_DISK{0} for active chain when loaded from disk)
    if (!pindex->HaveNumChainTxs())
      assert(pindex->nSequenceId <= SEQ_ID_INIT_FROM_DISK);
    // VALID_TRANSACTIONS is equivalent to nTx > 0 for all nodes (whether or not
    // pruning has occurred). HAVE_DATA is only equivalent to nTx > 0 (or
    // VALID_TRANSACTIONS) if no pruning has occurred.
    if (!m_blockman.m_have_pruned) {
    ...
    // being VALID_TRANSACTIONS, which is equivalent to HaveNumChainTxs().
    // HaveNumChainTxs will also be set in the assumeutxo snapshot block from
    // snapshot metadata.
    ...
    assert(
        ...
        pindex->nChainWork >=
            pindex->pprev
                ->nChainWork); // For every block except the genesis block, the
                               // chainwork must be larger than the parent's.
    assert(nHeight < 2 ||
           (pindex->pskip && (pindex->pskip->nHeight <
                              nHeight))); // The pskip pointer must point back
                                          // for all but the first 2 blocks.
    assert(pindexFirstNotTreeValid ==
           nullptr); // All m_blockman.m_block_index entries must at least be
                     // TREE valid
    ...
    if (pindexFirstInvalid == nullptr) {
      ...
      assert((pindex->nStatus & BLOCK_FAILED_MASK) ==
             0); // The failed mask cannot be set for blocks without invalid
                 // parents.
    } else {
      assert(pindex->nStatus &
             BLOCK_FAILED_MASK); // Invalid blocks and their descendants must be
                                 // marked as invalid
    }
    ...
    if (!pindex->pprev) {
      ...
      // If previous m_chain_tx_count is set and number of transactions in block
      ...
      if (!CBlockIndexWorkComparator()(pindex, c->m_chain.Tip()) &&
        ...
        if (pindexFirstInvalid == nullptr) {
          ...
          if (pindexFirstMissing == nullptr || pindex == c->m_chain.Tip() ||
            ...
            // block.
            if (!c->TargetBlock() ||
                c->TargetBlock()->GetAncestor(pindex->nHeight) == pindex) {
              assert(c->setBlockIndexCandidates.contains(
                  const_cast<CBlockIndex *>(pindex)));
            }
          }
          // If some parent is missing, then it could be that this block was in
          // setBlockIndexCandidates but had to be removed because of the
          // missing data. In this case it must be in m_blocks_unlinked -- see
          // test below.
        }
    ...
    if (pindex->pprev && (pindex->nStatus & BLOCK_HAVE_DATA) &&
      ...
      // and has no invalid parents, it must be in m_blocks_unlinked.
      assert(foundInUnlinked);
    }
    if (!(pindex->nStatus & BLOCK_HAVE_DATA))
      assert(!foundInUnlinked); // Can't be in m_blocks_unlinked if we don't
                                // HAVE_DATA
    if (pindexFirstMissing == nullptr)
      assert(!foundInUnlinked); // We aren't missing data for any parent --
                                // cannot be in m_blocks_unlinked.
    if (pindex->pprev && (pindex->nStatus & BLOCK_HAVE_DATA) &&
      ...
      //    tip.
      // So if this block is itself better than any m_chain.Tip() and it wasn't
      // in setBlockIndexCandidates, then it must be in m_blocks_unlinked.
      for (const auto &c : m_chainstates) {
        if (!CBlockIndexWorkComparator()(pindex, c->m_chain.Tip()) &&
            !c->setBlockIndexCandidates.contains(
                const_cast<CBlockIndex *>(pindex))) {
          if (pindexFirstInvalid == nullptr) {
            if (!c->TargetBlock() ||
                c->TargetBlock()->GetAncestor(pindex->nHeight) == pindex) {
    ...
    // Try descending into the first subnode. Always process forks first and the
    // best header chain after.
    snap_update_firsts();
    ...
    if (range.first != range.second) {
      ...
      if (!pindex)
        break; // we are finished, since the best header chain is always
               // processed last
      continue;
    }
    // This is a leaf node.
    ...
    while (pindex) {
      ...
      while (rangePar.first->second != pindex) {
        assert(rangePar.first !=
               rangePar.second); // Our parent must have at least the node we're
      ...
      if (rangePar.first != rangePar.second) {
        ...
        pindex = best_hdr_chain[nHeight];
        // There will not be a next block if (and only if) parent block is the
        // best header.
...
bool Chainstate::ResizeCoinsCaches(size_t coinstip_size, size_t coinsdb_size) {
  ...
  LogInfo("[%s] resized coinsdb cache to %.1f MiB", this->ToString(),
          coinsdb_size * (1.0 / 1024 / 1024));
  LogInfo("[%s] resized coinstip cache to %.1f MiB", this->ToString(),
...
double
ChainstateManager::GuessVerificationProgress(const CBlockIndex *pindex) const {
  ...
  if (pindex->m_chain_tx_count == 0) {
    LogDebug(BCLog::VALIDATION,
             "Block %d has unset m_chain_tx_count. Unable to estimate "
  ...
  const auto block_time{
      (Assume(m_best_header) &&
       std::abs(nNow - pindex->GetBlockTime()) <=
           Ticks<std::chrono::seconds>(2h) &&
       Assume(m_best_header->nHeight >= pindex->nHeight))
          ...
          // may be off.
          nNow - (m_best_header->nHeight - pindex->nHeight) *
...
Chainstate &ChainstateManager::InitializeChainstate(CTxMemPool *mempool) {
  ...
  m_chainstates.emplace_back(
      std::make_unique<Chainstate>(mempool, m_blockman, *this));
...
util::Result<CBlockIndex *> ChainstateManager::ActivateSnapshot(
...

  {
    ...
    if (!GetParams().AssumeutxoForBlockhash(base_blockhash).has_value()) {
      ...
      return util::Error{Untranslated(strprintf(
          ...
          "%s). The following snapshot heights are available: %s",
    ...
    if (!snapshot_start_block) {
      return util::Error{Untranslated(strprintf(
          ...
          "sure all headers are syncing, and call loadtxoutset again",
          base_blockhash.ToString()))};
    }

    bool start_block_invalid =
        snapshot_start_block->nStatus & BLOCK_FAILED_MASK;
    if (start_block_invalid) {
      return util::Error{Untranslated(
          strprintf("The base block header (%s) is part of an invalid chain",
                    base_blockhash.ToString()))};
    }
    ...
    if (!m_best_header ||
      ...
      return util::Error{
          Untranslated("A forked headers-chain with more work than the chain "
                       "with the snapshot base block header exists. Please "
                       "proceed to sync without AssumeUtxo.")};
    }

    auto mempool{CurrentChainstate().GetMempool()};
    if (mempool && mempool->size() > 0) {
      return util::Error{
          Untranslated("Can't activate a snapshot when mempool not empty")};
    }
  }
  ...
  // cache capacity to the snapshot to aid the bulk load later in this function.
...

  {
    ...
    //
    // Allocate the majority of the cache to the incoming snapshot chainstate,
    // since (optimistically) getting to its tip will be the top priority. We'll
    // need to call `MaybeRebalanceCaches()` once we're done with this function
    // to ensure the right allocation (including the possibility that no
    // snapshot was activated and that we should restore the active chainstate
  ...
  auto snapshot_chainstate = WITH_LOCK(
      ::cs_main, return std::make_unique<Chainstate>(
  ...
  if (auto res{this->PopulateAndValidateSnapshot(*snapshot_chainstate,
                                                 coins_file, metadata)};
      !res) {
    LOCK(::cs_main);
    return cleanup_bad_snapshot(Untranslated(
        strprintf("Population failed: %s", util::ErrorString(res).original)));
  }
  ...
  // very late in the IBD process, and we wouldn't want to load a useless
  // chainstate.
...
util::Result<void> ChainstateManager::PopulateAndValidateSnapshot(
  ...
  // It's okay to release cs_main before we're done using `coins_cache` because
  // we know that nothing else will be referencing the newly created
  ...
  CBlockIndex *snapshot_start_block =
      WITH_LOCK(::cs_main, return m_blockman.LookupBlockIndex(base_blockhash));

  if (!snapshot_start_block) {
    // Needed for ComputeUTXOStats to determine the
    // height and to avoid a crash when base_blockhash.IsNull()
    return util::Error{
        Untranslated(strprintf("Did not find snapshot start blockheader %s",
                               base_blockhash.ToString()))};
  }
  ...
  if (!maybe_au_data) {
    return util::Error{Untranslated(
        strprintf("Assumeutxo height in snapshot metadata not recognized "
  ...
  LogInfo("[snapshot] loading %d coins from snapshot %s", coins_left,
          base_blockhash.ToString());
  ...
  while (coins_left > 0) {
    try {
      ...
      if (coins_per_txid > coins_left) {
        return util::Error{Untranslated("Mismatch in coins count in snapshot "
      ...
      for (size_t i = 0; i < coins_per_txid; i++) {
        ...
        if (coin.nHeight > base_height ||
            outpoint.n >=
                std::numeric_limits<
                    decltype(outpoint.n)>::max() // Avoid integer wrap-around in
                                                 // coinstats.cpp:ApplyHash
        ) {
          return util::Error{Untranslated(
              strprintf("Bad snapshot data after deserializing %d coins",
                        coins_count - coins_left))};
        }
        if (!MoneyRange(coin.out.nValue)) {
          return util::Error{
              Untranslated(strprintf("Bad snapshot data after deserializing %d "
        ...
        if (coins_processed % 120000 == 0) {
          if (m_interrupt) {
            return util::Error{
                Untranslated("Aborting after an interrupt was requested")};
          }
          ...
          if (snapshot_cache_state >= CoinsCacheSizeState::CRITICAL) {
            // This is a hack - we don't know what the actual best block is, but
            // that doesn't matter for the purposes of flushing the cache here.
            // We'll set this to its correct value (`base_blockhash`) below
            // after the coins are loaded.
            coins_cache.SetBestBlock(GetRandHash());

            // No need to acquire cs_main since this chainstate isn't being used
            // yet.
      ...
      return util::Error{
          Untranslated(strprintf("Bad snapshot format or truncated snapshot "
  ...
  if (!out_of_coins) {
    return util::Error{Untranslated(
        strprintf("Bad snapshot - coins left over after deserializing %d coins",
                  coins_count))};
  }

  LogInfo("[snapshot] loaded %d (%.2f MB) coins from snapshot %s", coins_count,
          coins_cache.DynamicMemoryUsage() / (1000 * 1000),
          base_blockhash.ToString());
  ...
  if (AssumeutxoHash{maybe_stats->hashSerialized} != au_data.hash_serialized) {
    return util::Error{
        Untranslated(strprintf("Bad snapshot content hash: expected %s, got %s",
...
ChainstateManager::MaybeValidateSnapshot(Chainstate &validated_cs,
  ...
  auto handle_invalid_snapshot = [&]() EXCLUSIVE_LOCKS_REQUIRED(::cs_main) {
    bilingual_str user_error = strprintf(
        _("%s failed to validate the -assumeutxo snapshot state. "
          ...
          "loaded. As a result of this, the node will shut down and stop using "
          "any "
          "state that was built on the snapshot, resetting the chain height "
          "from %d to %d. On the next "
          "restart, the node will resume syncing from %d "
          "without using any snapshot data. "
          "Please report this incident to %s, including how you obtained the "
          "snapshot. "
          "The invalid snapshot chainstate will be left on disk in case it is "
          "helpful in diagnosing the issue that caused this error."),
        CLIENT_NAME, unvalidated_cs.m_chain.Height(),
        validated_cs.m_chain.Height(), validated_cs.m_chain.Height(),
        CLIENT_BUGREPORT);

    LogError("[snapshot] !!! %s\n", user_error.original);
    LogError("[snapshot] deleting snapshot, reverting to validated chain, and "
             "stopping node\n");
  ...
  const auto &maybe_au_data =
      m_options.chainparams.AssumeutxoForHeight(validated_cs.m_chain.Height());
  if (!maybe_au_data) {
  ...
  try {
    validated_cs_stats = ComputeUTXOStats(
        CoinStatsHashType::HASH_SERIALIZED, &validated_coins_db, m_blockman,
        [&interrupt = m_interrupt] { SnapshotUTXOHashBreakpoint(interrupt); });
  } catch (StopHashingException const &) {
    return SnapshotCompletionResult::STATS_FAILED;
  }

  // XXX note that this function is slow and will hold cs_main for potentially
  // minutes.
  if (!validated_cs_stats) {
    LogWarning("[snapshot] failed to generate stats for validation coins db");
    // While this isn't a problem with the snapshot per se, this condition
    // prevents us from validating the snapshot, so we should shut down and let
    // the user handle the issue manually.
    handle_invalid_snapshot();
    return SnapshotCompletionResult::STATS_FAILED;
  }
  ...
  if (AssumeutxoHash{validated_cs_stats->hashSerialized} !=
...
void ChainstateManager::MaybeRebalanceCaches() {
  ...
  if (!historical_cs && !current_cs.m_from_snapshot_blockhash) {
    ...
    // If both chainstates exist, determine who needs more cache based on IBD
    // status.
    //
...
void ChainstateManager::ResetChainstates() { m_chainstates.clear(); }
...
static ChainstateManager::Options &&Flatten(ChainstateManager::Options &&opts) {
  if (!opts.check_block_index.has_value())
    opts.check_block_index = opts.chainparams.DefaultConsistencyChecks();
  if (!opts.minimum_chain_work.has_value())
    opts.minimum_chain_work =
  ...
  if (!opts.assumed_valid_block.has_value())
    opts.assumed_valid_block =
        opts.chainparams.GetConsensus().defaultAssumeValid;
...
Chainstate *ChainstateManager::LoadAssumeutxoChainstate() {
  ...
  std::optional<uint256> base_blockhash =
...
Chainstate &
ChainstateManager::AddChainstate(std::unique_ptr<Chainstate> chainstate) {
...
bool IsBIP30Repeat(const CBlockIndex &block_index) {
  return (block_index.nHeight == 91842 &&
          block_index.GetBlockHash() ==
              uint256{"00000000000a4d0a398161ffc163c503763b1f4360639393e0e4c8e3"
                      "00e0caec"}) ||
         (block_index.nHeight == 91880 &&
          block_index.GetBlockHash() ==
              uint256{"00000000000743f190a18c5577a3c2d2a1f610ae9601ac046a38084c"
                      "cb7cd721"});
}

bool IsBIP30Unspendable(const uint256 &block_hash, int block_height) {
  return (block_height == 91722 &&
          block_hash == uint256{"00000000000271a2dc26e7667f8419f2e15416dc6955e5"
                                "a6c6cdf3f2574dd08e"}) ||
         (block_height == 91812 &&
          block_hash == uint256{"00000000000af0aed4792b1acee3d966af36cf5def1493"
                                "5db8de83d6f9306f2f"});
}
...
util::Result<void> Chainstate::InvalidateCoinsDBOnDisk() {
  ...
  LogInfo("[snapshot] renaming snapshot datadir %s to %s", db_path_str,
          invalid_path_str);

  // The invalid storage directory is simply moved and not deleted because we
  // may want to do forensics later during issue investigation. The user is
  // instructed accordingly in MaybeValidateSnapshot().
  try {
    ...
    return util::Error{strprintf(
        _("Rename of '%s' -> '%s' failed. "
          "You should resolve this by manually moving or deleting the invalid "
          "snapshot directory %s, otherwise you will encounter the same error "
          "again "
          "on the next startup."),
...
bool ChainstateManager::DeleteChainstate(Chainstate &chainstate) {
  ...
  if (!DeleteCoinsDBFromDisk(db_path, /*is_snapshot=*/bool{
                                 chainstate.m_from_snapshot_blockhash})) {
    LogError("Deletion of %s failed. Please remove it manually to continue "
             "reindexing.",
             fs::PathToString(db_path));
    return false;
  }
  std::unique_ptr<Chainstate> prev_chainstate{
      Assert(RemoveChainstate(chainstate))};
  Chainstate &curr_chainstate{CurrentChainstate()};
  assert(prev_chainstate->m_mempool->size() == 0);
  assert(!curr_chainstate.m_mempool);
  std::swap(curr_chainstate.m_mempool, prev_chainstate->m_mempool);
...
ChainstateRole Chainstate::GetRole() const {
  return ChainstateRole{.validated = m_assumeutxo == Assumeutxo::VALIDATED,
...
bool ChainstateManager::ValidatedSnapshotCleanup(Chainstate &validated_cs,
                                                 Chainstate &unvalidated_cs) {
  AssertLockHeld(::cs_main);
  ...
  LogInfo("[snapshot] deleting background chainstate directory (now "
          "unnecessary) (%s)",
  ...
  LogInfo("[snapshot] moving snapshot chainstate (%s) to "
          ...
          fs::PathToString(assumed_valid_path),
          fs::PathToString(validated_path));
...
std::pair<int, int> Chainstate::GetPruneRange(int last_height_can_prune) const {
  ...
  int max_prune =
      std::max<int>(0, m_chain.Height() - static_cast<int>(MIN_BLOCKS_TO_KEEP));
...
ChainstateManager::GetHistoricalBlockRange() const {
...
util::Result<void> ChainstateManager::ActivateBestChains() {
  ...
  // the chainman unique_ptrs since ABC requires us not to be holding cs_main,
  ...
  std::vector<Chainstate *> chainstates;
  {
    ...
    for (const auto &chainstate : m_chainstates) {
      if (chainstate && chainstate->m_assumeutxo != Assumeutxo::INVALID &&
          !chainstate->m_target_utxohash) {
  ...
  for (Chainstate *chainstate : chainstates) {
    ...
    if (!chainstate->ActivateBestChain(state, nullptr)) {
      LOCK(GetMutex());
      return util::Error{
          Untranslated(strprintf("%s Failed to connect best block (%s)",
                                 chainstate->ToString(), state.ToString()))};
    }
  }
  return {};
}
