...
// (blk???.dat and rev???.dat) At 1MB per block, 288 blocks = 288MB. Add 15% for
// Undo data = 331MB Add 20% for Orphan block rate = 397MB We want the low water
// mark after pruning to be at least 397 MB and since we prune in full block
// file chunks, we need the high water mark which triggers the prune to be one
// 128MB block file + added 15% undo data = 147MB greater for a total of 545MB
...
bool FatalError(kernel::Notifications &notifications,
                BlockValidationState &state, const bilingual_str &message);
 ...
 *package). Here are the expected fields and properties of a result depending on
 *its ResultType, applicable to results returned from package evaluation:
 *+---------------------------+----------------+-------------------+------------------+----------------+-------------------+
 *| Field or property         |    VALID       |                 INVALID |
 *MEMPOOL_ENTRY | DIFFERENT_WITNESS | |                           |
 *|--------------------------------------|                |                   |
 *|                           |                | TX_RECONSIDERABLE |     Other
 *|                |                   |
 *+---------------------------+----------------+-------------------+------------------+----------------+-------------------+
 *| txid in mempool?          | yes            | no                | no* | yes
 *| yes               | | wtxid in mempool?         | yes            | no | no*
 *| yes            | no                | | m_state                   | yes,
 *IsValid() | yes, IsInvalid()  | yes, IsInvalid() | yes, IsValid() | yes,
 *IsValid()    | | m_vsize                   | yes            | no | no | yes |
 *no                | | m_base_fees               | yes            | no | no |
 *yes            | no                | | m_effective_feerate       | yes | yes
 *| no               | no             | no                | |
 *m_wtxids_fee_calculations | yes            | yes               | no | no | no
 *| | m_other_wtxid             | no             | no                | no | no
 *| yes               |
 *+---------------------------+----------------+-------------------+------------------+----------------+-------------------+
 * (*) Individual transaction acceptance doesn't return MEMPOOL_ENTRY and
 *DIFFERENT_WITNESS. It returns INVALID, with the errors txn-already-in-mempool
 *and txn-same-nonwitness-data-in-mempool respectively. In those cases, the txid
 *or wtxid may be in the mempool for a TX_CONFLICT.
 */
struct MempoolAcceptResult {
  ...
  enum class ResultType {
    ...
    DIFFERENT_WITNESS, //!> Not validated. A same-txid-different-witness tx (see
                       //!m_other_wtxid) already exists in the mempool and was
                       //!not replaced.
  };
  ...
  /** Virtual size as used by the mempool, calculated using serialized size and
   * sigops. */
   ...
   * Includes this transaction's wtxid and may include others if this
   * transaction was validated as part of a package. This is not necessarily
   * equivalent to the list of transactions passed to ProcessNewPackage(). Only
   * present when m_result_type = ResultType::VALID. */
  const std::optional<std::vector<Wtxid>> m_wtxids_fee_calculations;

  /** The wtxid of the transaction in the mempool which has the same txid but
   * different witness. */
  ...
  static MempoolAcceptResult
  Success(std::list<CTransactionRef> &&replaced_txns, int64_t vsize,
          CAmount fees, CFeeRate effective_feerate,
          const std::vector<Wtxid> &wtxids_fee_calculations) {
  ...
  static MempoolAcceptResult
  MempoolTxDifferentWitness(const Wtxid &other_wtxid) {
    return MempoolAcceptResult(other_wtxid);
  }

  // Private constructors. Use static methods MempoolAcceptResult::Success, etc.
  // to construct.
private:
  ...
  explicit MempoolAcceptResult(
      std::list<CTransactionRef> &&replaced_txns, int64_t vsize, CAmount fees,
      CFeeRate effective_feerate,
      const std::vector<Wtxid> &wtxids_fee_calculations)
      : m_result_type(ResultType::VALID),
        m_replaced_transactions(std::move(replaced_txns)), m_vsize{vsize},
        m_base_fees(fees), m_effective_feerate(effective_feerate),
  ...
  explicit MempoolAcceptResult(const Wtxid &other_wtxid)
      : m_result_type(ResultType::DIFFERENT_WITNESS),
        m_other_wtxid(other_wtxid) {}
};
...
struct PackageMempoolAcceptResult {
   ...
   * was a package-wide error (see result in m_state), m_tx_results will be
   * empty.
   */
  ...
  /** Constructor to create a PackageMempoolAcceptResult from a single
   * MempoolAcceptResult */
  explicit PackageMempoolAcceptResult(const Wtxid &wtxid,
 ...
 * the mempool. It is also used to determine when the entry expires.
 * @param[in]  bypass_limits      When true, don't enforce mempool fee and
 * capacity limits, and set entry_sequence to zero.
...
/**
 * Validate (and maybe submit) a package to the mempool. See
 * doc/policy/packages.md for full details on package validation rules.
 * @param[in]    test_accept         When true, run validation checks but don't
 * submit to mempool.
 * @param[in]    client_maxfeerate    If exceeded by an individual transaction,
 * rest of (sub)package evaluation is aborted. Only for sanity checks against
 * local submission of transactions.
 * @returns a PackageMempoolAcceptResult which includes a MempoolAcceptResult
 * for each transaction. If a transaction fails, validation will exit early and
 * some results may be missing. It is also possible for the package to be
 * partially submitted.
 */
 ...
 * relevant coins, or a CCoinsViewMemPool that is connected to the mempool and
 * chainstate UTXO set. In the latter case, the caller is responsible for
 * holding the appropriate locks to ensure that calls to GetCoin() return
 * correct coins.
 ...
 * which this transaction is final. Simulates calling SequenceLocks() with data
 * from the tip passed in. The LockPoints should not be considered valid if
 * CheckSequenceLocksAtTip returns false.
 */
...
public:
  CScriptCheck(const CTxOut &outIn, const CTransaction &txToIn,
               ...
               script_verify_flags flags, bool cacheIn,
               PrecomputedTransactionData *txdataIn)
      : m_tx_out(outIn), ptxTo(&txToIn), nIn(nInIn), m_flags(flags),
...
BlockValidationState TestBlockValidity(Chainstate &chainstate,
                                       const CBlock &block, bool check_pow,
...
/** Return the sum of the claimed work on a given set of headers. No
 * verification of PoW is done. */
arith_uint256
...
public:
  ...
  VerifyDB(Chainstate &chainstate, const Consensus::Params &consensus_params,
...
inline constexpr std::array FlushStateModeNames{"NONE", "IF_NEEDED", "PERIODIC",
                                                "ALWAYS"};
enum class FlushStateMode : uint8_t { NONE, IF_NEEDED, PERIODIC, ALWAYS };
...
public:
  ...
  //! instances, but it *does not* create a CCoinsViewCache instance by default.
  //! This is done separately because the presence of the cache has implications
  //! on whether or not we're allowed to flush the cache's state to disk, which
  //! should not be done until the health of the database is verified.
  //!
...
enum class Assumeutxo {
  ...
  VALIDATED,
  //! Blocks after an assumeutxo snapshot have been validated but the snapshot
...
protected:
   ...
   * ActivateBestChain() and InvalidateBlock()
   */
  ...
  mutable const CBlockIndex *m_cached_snapshot_base GUARDED_BY(::cs_main){
      nullptr};

  //! Cached result of LookupBlockIndex(*m_target_blockhash)
  mutable const CBlockIndex *m_cached_target_block GUARDED_BY(::cs_main){
      nullptr};

  std::optional<const char *>
      m_last_script_check_reason_logged GUARDED_BY(::cs_main){};
...
public:
  ...
  /**
   * Initialize the CoinsViews UTXO set database management data structures. The
   * in-memory cache is initialized separately.
   *
  ...
  /**
   * The blockhash which is the base of the snapshot this chainstate was created
   * from.
   *
  ...
  void SetTargetBlockHash(uint256 block_hash)
      EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
  ...
  bool ReachedTarget() const EXCLUSIVE_LOCKS_REQUIRED(::cs_main) {
    ...
    assert(!target_block ||
           target_block->GetAncestor(m_chain.Height()) == m_chain.Tip());
  ...
  /** Remove invalidity status from a block, its descendants and ancestors and
  ...
  /** Whether the chain state needs to be redownloaded due to lack of witness
   * data */
  ...
  /** Update the chain tip based on database information, i.e. CoinsTip()'s best
   * block. */
  ...
  std::pair<int, int> GetPruneRange(int last_height_can_prune) const
      EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
...
protected:
  bool ActivateBestChainStep(BlockValidationState &state,
      ...
      EXCLUSIVE_LOCKS_REQUIRED(cs_main, m_mempool->cs);
  bool ConnectTip(BlockValidationState &state, CBlockIndex *pindexNew,
...
private:
   ...
   * m_block_index. Caller must set min_pow_checked=true in order to add a new
   * header to the block index (permanent memory storage), indicating that the
   * header is known to be part of a sufficiently high-work chain (anti-dos
   * check).
   */
  bool AcceptBlockHeader(const CBlockHeader &block, BlockValidationState &state,
  ...
  MockableSteadyClock::time_point
      m_last_presync_update GUARDED_BY(GetMutex()){};
...
public:
  ...
  const Consensus::Params &GetConsensus() const {
    return m_options.chainparams.GetConsensus();
  }
   ...
   * (SEQ_ID_BEST_CHAIN_FROM_DISK{0} if they belong to the best chain loaded
   * from disk), so start the counter after that. **/
  ...
  /** Best header we've seen so far for which the block is not known to be
     invalid (used, among others, for getheaders queries' starting points). In
     case of multiple best headers with the same work, it could point to any
  ...
  ActivateSnapshot(AutoFile &coins_file, const node::SnapshotMetadata &metadata,
                   bool in_memory);
  ...
  SnapshotCompletionResult MaybeValidateSnapshot(Chainstate &validated_cs,
  ...
  Chainstate &CurrentChainstate() const EXCLUSIVE_LOCKS_REQUIRED(GetMutex()) {
    for (auto &cs : m_chainstates) {
      if (cs && cs->m_assumeutxo != Assumeutxo::INVALID &&
          !cs->m_target_blockhash)
        return *cs;
    }
    abort();
  }
  ...
  Chainstate *HistoricalChainstate() const
      EXCLUSIVE_LOCKS_REQUIRED(GetMutex()) {
    for (auto &cs : m_chainstates) {
      if (cs && cs->m_assumeutxo != Assumeutxo::INVALID &&
          cs->m_target_blockhash && !cs->m_target_utxohash)
        return cs.get();
    }
    return nullptr;
  }
  ...
  Chainstate &ValidatedChainstate() const EXCLUSIVE_LOCKS_REQUIRED(GetMutex()) {
    for (auto *cs : {&CurrentChainstate(), HistoricalChainstate()}) {
      if (cs && cs->m_assumeutxo == Assumeutxo::VALIDATED)
        return *cs;
    }
    abort();
  }
  ...
  //! Remove a chainstate.
  std::unique_ptr<Chainstate> RemoveChainstate(Chainstate &chainstate)
      EXCLUSIVE_LOCKS_REQUIRED(GetMutex()) {
    auto it{std::find_if(m_chainstates.begin(), m_chainstates.end(),
                         [&](auto &cs) { return cs.get() == &chainstate; })};
   ...
   * file and attempts to process them (add them to the block index). The blocks
   * may be out of order within each file and across files. Often this function
   * reads a block but finds that its parent hasn't been read yet, so the block
   * can't be processed yet. The function will add an entry to the
   ...
   *
   * Because a block's parent may be in a later file, not just later in the same
   * file, the blocks_with_unknown_parent map must be passed in and out with
   * each call. It's a multimap, rather than just a map, because multiple blocks
   * may have the same parent (when chain splits or stale blocks exist). It maps
   * from parent-hash to child-disk-position.
   *
   * This function can also be used to read blocks from user-specified block
   * files using the -loadblock= option. There's no unknown-parent tracking, so
   * the last two arguments are omitted.
   *
   *
   * @param[in]     file_in                       File containing blocks to read
   * @param[in]     dbp                           (optional) Disk block position
   * (only for reindex)
   * @param[in,out] blocks_with_unknown_parent    (optional) Map of disk
   * positions for blocks with unknown parent, key is parent block hash (only
   * used for reindex)
   * */
   ...
   * @param[in]   force_processing Process this block even if unrequested; used
   * for non-network block sources.
   ...
   * @param[out]  new_block A boolean which is set to indicate if the block was
   ...
   * processing them
   * @param[out] ppindex If set, the pointer will be set to point to the last
   * new block index object for the given headers
   * @returns false if AcceptBlockHeader fails on any of the headers, true
   * otherwise (including if headers were already known)
   */
  ...
  bool AcceptBlock(const std::shared_ptr<const CBlock> &pblock,
                   ...
                   bool fRequested, const FlatFilePos *dbp, bool *fNewBlock,
  ...
  GenerateCoinbaseCommitment(CBlock &block,
                             const CBlockIndex *pindexPrev) const;
  ...
  //! Add new chainstate.
  Chainstate &AddChainstate(std::unique_ptr<Chainstate> chainstate)
      EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
  ...
  [[nodiscard]] bool DeleteChainstate(Chainstate &chainstate)
      EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
  ...
  bool ValidatedSnapshotCleanup(Chainstate &validated_cs,
                                Chainstate &unvalidated_cs)
      EXCLUSIVE_LOCKS_REQUIRED(::cs_main);

  //! Get range of historical blocks to download.
  std::optional<std::pair<const CBlockIndex *, const CBlockIndex *>>
  GetHistoricalBlockRange() const EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
...
/** Identifies blocks that overwrote an existing coinbase output in the UTXO set
 * (see BIP30) */
bool IsBIP30Repeat(const CBlockIndex &block_index);

/** Identifies blocks which coinbase output was subsequently overwritten in the
 * UTXO set (see BIP30) */
