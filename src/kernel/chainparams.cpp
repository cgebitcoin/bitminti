...
static CBlock CreateGenesisBlock(uint32_t nTime, uint32_t nNonce,
                                 uint32_t nBits, int32_t nVersion,
                                 const CAmount &genesisReward) {
  const char *pszTimestamp = "BTC3 Fork 2025 - A New Beginning for Bitcoin";
  const CScript genesisOutputScript =
      CScript()
...
public:
  CMainParams() {
    ...
    consensus.vDeployments[Consensus::DEPLOYMENT_TESTDUMMY].threshold =
        1815; // 90%
    ...
    consensus.vDeployments[Consensus::DEPLOYMENT_TAPROOT].nStartTime =
        1619222400; // April 24th, 2021
    consensus.vDeployments[Consensus::DEPLOYMENT_TAPROOT].nTimeout =
        1628640000; // August 11th, 2021
    consensus.vDeployments[Consensus::DEPLOYMENT_TAPROOT]
        .min_activation_height = 709632; // Approximately November 12th, 2021
    consensus.vDeployments[Consensus::DEPLOYMENT_TAPROOT].threshold =
        1815; // 90%
    ...
    // support a subset of possible options. This is fine at runtime as we'll
    // fall back to using them as an addrfetch if they don't support the service
    // bits we want, but we should get them updated to support all service bits
    // wanted by any release ASAP to avoid it where possible.
    vSeeds.clear();
    vSeeds.emplace_back("seed.btc3.mrio.xyz."); // Primary DNS Seeder
    ...
    m_assumeutxo_data = {
        {
            .height = 840'000,
            .hash_serialized =
                AssumeutxoHash{uint256{"a2a5521b1b5ab65f67818e5e8eccabb7171a517"
                                       "f9e2382208f77687310768f96"}},
            .m_chain_tx_count = 991032194,
            .blockhash =
                consteval_ctor(uint256{"0000000000000000000320283a032748cef8227"
                                       "873ff4872689bf23f1cda83a5"}),
        },
        {
            .height = 880'000,
            .hash_serialized =
                AssumeutxoHash{uint256{"dbd190983eaf433ef7c15f78a278ae42c00ef52"
                                       "e0fd2a54953782175fbadcea9"}},
            .m_chain_tx_count = 1145604538,
            .blockhash =
                consteval_ctor(uint256{"000000000000000000010b17283c3c400507969"
                                       "a9c2afd1dcf2082ec5cca2880"}),
        },
        {
            .height = 910'000,
            .hash_serialized =
                AssumeutxoHash{uint256{"4daf8a17b4902498c5787966a2b51c613acdab5"
                                       "df5db73f196fa59a4da2f1568"}},
            .m_chain_tx_count = 1226586151,
            .blockhash =
                consteval_ctor(uint256{"0000000000000000000108970acb9522ffd516e"
                                       "ae17acddcb1bd16469194a821"}),
        }};
...
public:
  CTestNetParams() {
    ...
    consensus.CSVHeight =
        770112; // 00000000025e930139bac5c6c31a403776da130831ab85be56578f3fa75369bb
    consensus.SegwitHeight =
        834624; // 00000000002b980fcd729daaa248fd9316a5200e9b367f4ff2c42453e84201ca
    ...
    consensus.defaultAssumeValid =
        uint256{"0000000000000065c6c38258e201971a3fdfcc2ceee0dd6e85a6c022d45dee"
                "34"}; // 4550000
    ...
    consensus.hashGenesisBlock = genesis.GetHash();
    // assert(consensus.hashGenesisBlock ==
    //        uint256{"000000000933ea01ad0ee984209779baaec3ced90fa3f408719526f8d77"
    //                "f4943"});
    // assert(genesis.hashMerkleRoot ==
    //        uint256{"4a5e1e4baab89f3a32518a88c31bc87f618f76673e2cc77ab2127b7afde"
    //                "da33b"});
    ...
    m_assumeutxo_data = {{
        .height = 2'500'000,
        .hash_serialized =
            AssumeutxoHash{uint256{"f841584909f68e47897952345234e37fcd9128cd818"
                                   "f41ee6c3ca68db8071be7"}},
        .m_chain_tx_count = 66484552,
        .blockhash = consteval_ctor(uint256{"0000000000000093bcb68c03a9a168ae25"
                                            "2572d348a2eaeba2cdf9231d73206f"}),
    }};
...
public:
  CTestNet4Params() {
    ...
    consensus.nMinimumChainWork = uint256{
        "00000000000000000000000000000000000000000000034a4690fe592dc49c7c"};
    consensus.defaultAssumeValid =
        uint256{"000000000000000180a58e7fa3b0db84b5ea76377524894f53660d93ac839d"
                "9b"}; // 91000
    ...
    const char *testnet4_genesis_msg =
        "03/May/2024 "
    ...
    const CScript testnet4_genesis_script =
        ...
        << OP_CHECKSIG;
    genesis =
        CreateGenesisBlock(testnet4_genesis_msg, testnet4_genesis_script,
                           1714777860, 393743547, 0x1d00ffff, 1, 50 * COIN);
    consensus.hashGenesisBlock = genesis.GetHash();
    assert(consensus.hashGenesisBlock ==
           uint256{"00000000da84f2bafbbc53dee25a72ae507ff4914b867c565be350b0da8"
                   "bf043"});
    assert(genesis.hashMerkleRoot ==
           uint256{"7aa0a7ae1e223414cb807e40cd57e667b718e42aaf9306db9102fe28912"
                   "b7b4e"});
    ...
    vFixedSeeds = std::vector<uint8_t>(std::begin(chainparams_seed_testnet4),
                                       std::end(chainparams_seed_testnet4));
    ...
    m_assumeutxo_data = {{
        .height = 90'000,
        .hash_serialized =
            AssumeutxoHash{uint256{"784fb5e98241de66fdd429f4392155c9e7db5c01714"
                                   "8e66e8fdbc95746f8b9b5"}},
        .m_chain_tx_count = 11347043,
        .blockhash = consteval_ctor(uint256{"0000000002ebe8bcda020e0dd6ccfbdfac"
                                            "531d2f6a81457191b99fc2df2dbe3b"}),
    }};
...
public:
  explicit SigNetParams(const SigNetOptions &options) {
    ...
    if (!options.challenge) {
      ...
      vFixedSeeds = std::vector<uint8_t>(std::begin(chainparams_seed_signet),
    ...
    genesis =
        CreateGenesisBlock(1598918400, 52613770, 0x1e0377ae, 1, 50 * COIN);
    consensus.hashGenesisBlock = genesis.GetHash();
    // assert(consensus.hashGenesisBlock ==
    //        uint256{"00000008819873e925422c1ff0f99f7cc9bbb232af63a077a480a3633be"
    //                "e1ef6"});
    // assert(genesis.hashMerkleRoot ==
    //        uint256{"4a5e1e4baab89f3a32518a88c31bc87f618f76673e2cc77ab2127b7afde"
    //                "da33b"});
    ...
    m_assumeutxo_data = {{
        .height = 160'000,
        .hash_serialized =
            AssumeutxoHash{uint256{"fe0a44309b74d6b5883d246cb419c6221bcccf0b308"
                                   "c9b59b7d70783dbdf928a"}},
        .m_chain_tx_count = 2289496,
        .blockhash = consteval_ctor(uint256{"0000003ca3c99aff040f2563c2ad8f8ec8"
                                            "8bd0fd6b8f0895cfaf1ef90353a62c"}),
    }};
...
public:
  explicit CRegTestParams(const RegTestOptions &opts) {
    ...
    consensus.vDeployments[Consensus::DEPLOYMENT_TESTDUMMY].threshold =
        108; // 75%
    consensus.vDeployments[Consensus::DEPLOYMENT_TESTDUMMY].period =
    ...
    for (const auto &[deployment_pos, version_bits_params] :
         opts.version_bits_parameters) {
      consensus.vDeployments[deployment_pos].nStartTime =
          version_bits_params.start_time;
      consensus.vDeployments[deployment_pos].nTimeout =
          version_bits_params.timeout;
    ...
    consensus.hashGenesisBlock = genesis.GetHash();
    // assert(consensus.hashGenesisBlock ==
    //        uint256{"0f9188f13cb7b2c71f2a335e3a4fc328bf5beb436012afca590b1a11466"
    //                "e2206"});
    // assert(genesis.hashMerkleRoot ==
    //        uint256{"4a5e1e4baab89f3a32518a88c31bc87f618f76673e2cc77ab2127b7afde"
    //                "da33b"});
    ...
    m_assumeutxo_data = {
        {
            ...
            .height = 110,
            .hash_serialized =
                AssumeutxoHash{uint256{"b952555c8ab81fec46f3d4253b7af256d766ceb"
                                       "39fb7752b9d18cdf4a0141327"}},
            .m_chain_tx_count = 111,
            .blockhash =
                consteval_ctor(uint256{"6affe030b7965ab538f820a56ef56c8149b7dc1"
                                       "d1c144af57113be080db7c397"}),
        },
        {
            // For use by fuzz target src/test/fuzz/utxo_snapshot.cpp
            .height = 200,
            .hash_serialized =
                AssumeutxoHash{uint256{"17dcc016d188d16068907cdeb38b75691a118d4"
                                       "3053b8cd6a25969419381d13a"}},
            .m_chain_tx_count = 201,
            .blockhash =
                consteval_ctor(uint256{"385901ccbd69dff6bbd00065d01fb8a9e464ded"
                                       "e7cfe0372443884f9b1dcf6b9"}),
        },
        {
            // For use by test/functional/feature_assumeutxo.py
            .height = 299,
            .hash_serialized =
                AssumeutxoHash{uint256{"d2b051ff5e8eef46520350776f4100dd710a634"
                                       "47a8e01d917e92e79751a63e2"}},
            .m_chain_tx_count = 334,
            .blockhash =
                consteval_ctor(uint256{"7cc695046fec709f8c9394b6f928f81e81fd3ac"
                                       "20977bb68760fa1faa7916ea2"}),
        },
    };
...
CChainParams::SigNet(const SigNetOptions &options) {
  return std::make_unique<const SigNetParams>(options);
}

std::unique_ptr<const CChainParams>
CChainParams::RegTest(const RegTestOptions &options) {
