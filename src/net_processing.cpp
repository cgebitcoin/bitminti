...
static_assert(INVENTORY_BROADCAST_MAX >= INVENTORY_BROADCAST_TARGET,
              "INVENTORY_BROADCAST_MAX too low");
static_assert(INVENTORY_BROADCAST_MAX <= node::MAX_PEER_TX_ANNOUNCEMENTS,
              "INVENTORY_BROADCAST_MAX too high");
...
struct Peer {
  ...
  /** Whether this peer should be disconnected and marked as discouraged (unless
   * it has NetPermissionFlags::NoBan permission). */
  ...
  /** Set to true once initial VERSION message was sent (only relevant for
   * outbound peers). */
  bool m_outbound_version_message_sent
   ...
   * peer. It is *not* a p2p protocol violation for the peer to send us
  ...
  std::chrono::microseconds
      m_next_send_feefilter GUARDED_BY(NetEventsInterface::g_msgproc_mutex){0};

  struct TxRelay {
    mutable RecursiveMutex m_bloom_filter_mutex;
    /** Whether we relay transactions to this peer. */
    bool m_relay_txs GUARDED_BY(m_bloom_filter_mutex){false};
    /** A bloom filter for which transactions to announce to the peer. See
     * BIP37. */
    ...
    /** The mempool sequence num at which we sent the last `inv` message to this
     * peer. Can relay txs with lower sequence numbers than this (see
     * CTxMempool::info_for_relay). */
    uint64_t m_last_inv_sequence GUARDED_BY(m_tx_inventory_mutex){1};

    /** Minimum fee rate with which to filter transaction announcements to this
     * node. See BIP133. */
    std::atomic<CAmount> m_fee_filter_received{0};
  };

  /* Initializes a TxRelay struct for this peer. Can be called at most once for
   * a peer. */
  ...
  double m_addr_token_bucket GUARDED_BY(NetEventsInterface::g_msgproc_mutex){
      1.0};
...
public:
  ...
  void BlockConnected(const ChainstateRole &role,
  ...
  void InitializeNode(const CNode &node, ServiceFlags our_services) override
  ...
  bool SendMessages(CNode *pto) override
      EXCLUSIVE_LOCKS_REQUIRED(!m_peer_mutex, !m_most_recent_block_mutex,
                               g_msgproc_mutex, !m_tx_download_mutex);

  /** Implement PeerManager */
  void StartScheduledTasks(CScheduler &scheduler) override;
  void CheckForStaleTipAndEvictPeers() override;
  util::Expected<void, std::string>
  FetchBlock(NodeId peer_id, const CBlockIndex &block_index) override
      EXCLUSIVE_LOCKS_REQUIRED(!m_peer_mutex);
  bool GetNodeStateStats(NodeId nodeid, CNodeStateStats &stats) const override
      EXCLUSIVE_LOCKS_REQUIRED(!m_peer_mutex);
  std::vector<node::TxOrphanage::OrphanInfo> GetOrphanTransactions() override
      EXCLUSIVE_LOCKS_REQUIRED(!m_tx_download_mutex);
  PeerManagerInfo GetInfo() const override
      EXCLUSIVE_LOCKS_REQUIRED(!m_peer_mutex);
  void SendPings() override EXCLUSIVE_LOCKS_REQUIRED(!m_peer_mutex);
  void RelayTransaction(const Txid &txid, const Wtxid &wtxid) override
      EXCLUSIVE_LOCKS_REQUIRED(!m_peer_mutex);
  ...
  void ProcessMessage(CNode &pfrom, const std::string &msg_type,
                      DataStream &vRecv,
...
private:
  ...
  /** If we have extra outbound peers, try to disconnect the one with the oldest
   * block announcement */
  void EvictExtraOutboundPeers(std::chrono::seconds now)
      EXCLUSIVE_LOCKS_REQUIRED(cs_main);

  /** Retrieve unbroadcast transactions from the mempool and reattempt sending
   * to peers */
   ...
   * should punish peers differently depending on whether the data was provided
   * in a compact block message or not. If the compact block had a valid header,
   * but contained invalid txs, the peer should not be punished. See BIP 152.
   */
   ...
   * @param[in]   first_time_failure            Whether we should consider
   * inserting into vExtraTxnForCompact, adding a new orphan to resolve, or
   * looking for a package to submit. Set to true for transactions just received
   * over p2p. Set to false if the tx has already been rejected before, e.g. is
   * already in the orphanage, to avoid adding duplicate entries. Updates
   * m_txrequest, m_lazy_recent_rejects, m_lazy_recent_rejects_reconsiderable,
   * m_orphanage, and vExtraTxnForCompact.
   *
   * @returns a PackageToValidate if this transaction has a reconsiderable
   * failure and an eligible package was found, or std::nullopt otherwise.
   */
  std::optional<node::PackageToValidate>
  ProcessInvalidTx(NodeId nodeid, const CTransactionRef &tx,
                   const TxValidationState &result, bool first_time_failure)
      EXCLUSIVE_LOCKS_REQUIRED(!m_peer_mutex, g_msgproc_mutex,
                               m_tx_download_mutex);

  /** Handle a transaction whose result was
   * MempoolAcceptResult::ResultType::VALID. Updates m_txrequest, m_orphanage,
   * and vExtraTxnForCompact. Also queues the tx for relay. */
   ...
   * ProcessInvalidTx for individual transactions, and caches rejection for the
   * package as a group.
   */
  void ProcessPackageResult(const node::PackageToValidate &package_to_validate,
   ...
   * mempool.
   *
   * @peer[in]  peer     The peer whose orphan transactions we will reconsider.
   * Generally only one orphan will be reconsidered on each call of this
   * function. If an accepted orphan has orphaned children, those will need to
   * be reconsidered, creating more work, possibly for other peers.
   * @return             True if meaningful work was done (an orphan was
   * accepted/rejected). If no meaningful work was done, then the work set for
   ...
   * @param[in]   headers   The headers received. Note that this may be modified
   * within ProcessHeadersMessage.
   ...
   */
  /** Return true if headers are continuous and have valid proof-of-work (DoS
   * points assigned on failure) */
   ...
   * announcements for blocks interacting with the 2hr (MAX_FUTURE_BLOCK_TIME)
   * rule). */
   ...
   */
  void HeadersDirectFetchBlocks(CNode &pfrom, const Peer &peer,
  ...
  void MakeAndPushMessage(CNode &node, std::string msg_type,
    ...
    m_connman.PushMessage(
        &node, NetMsg::Make(std::move(msg_type), std::forward<Args>(args)...));
  }
  ...
  /** Send a single `sendheaders` message, after we have completed headers sync
   * with a peer. */
  ...
  /** Synchronizes tx download including TxRequestTracker, rejection filters,
   * and TxOrphanage. Lock invariants:
   * - A txhash (txid or wtxid) in m_txrequest is not also in m_orphanage.
   * - A txhash (txid or wtxid) in m_txrequest is not also in
   * m_lazy_recent_rejects.
   * - A txhash (txid or wtxid) in m_txrequest is not also in
   * m_lazy_recent_rejects_reconsiderable.
   * - A txhash (txid or wtxid) in m_txrequest is not also in
   * m_lazy_recent_confirmed_transactions.
   * - Each data structure's limits hold (m_orphanage max size, m_txrequest
   * per-peer limits, etc).
   */
  ...
  /** Get a pointer to a const CNodeState, used when not mutating the CNodeState
   * object. */
  ...
  std::map<uint64_t, std::chrono::microseconds>
      m_next_inv_to_inbounds_per_network_key GUARDED_BY(g_msgproc_mutex);
   ...
   * distributed) timer for all peers with the same network key. If we used a
   * separate timer for each peer, a spy node could make multiple inbound
   * connections to us to accurately determine when we received a transaction
   * (and potentially determine the transaction's origin). Each network key has
   * its own timer to make fingerprinting harder. */
  std::chrono::microseconds
  NextInvToInbounds(std::chrono::microseconds now,
                    std::chrono::seconds average_interval, uint64_t network_key)
      EXCLUSIVE_LOCKS_REQUIRED(g_msgproc_mutex);
  ...
  // Data about the low-work headers synchronization, aggregated from all peers'
  // HeadersSyncStates.
  ...
  bool IsBlockRequestedFromOutbound(const uint256 &hash)
   ...
   */
  void RemoveBlockRequest(const uint256 &hash, std::optional<NodeId> from_peer)
      EXCLUSIVE_LOCKS_REQUIRED(cs_main);
  ...
  void TryDownloadingHistoricalBlocks(const Peer &peer, unsigned int count,
                                      std::vector<const CBlockIndex *> &vBlocks,
  ...
  CTransactionRef FindTxForGetData(const Peer::TxRelay &tx_relay,
                                   const GenTxid &gtxid)
      EXCLUSIVE_LOCKS_REQUIRED(!m_most_recent_block_mutex,
  ...
  std::vector<std::pair<Wtxid, CTransactionRef>>
  ...
  /**
   * Estimates the distance, in blocks, between the best-known block and the
   * network chain tip. Utilizes the best-block time and the chainparams blocks
  ...
  bool PrepareBlockFilterRequest(
      ...
      uint32_t start_height, const uint256 &stop_hash, uint32_t max_height_diff,
...
PeerManagerImpl::NextInvToInbounds(std::chrono::microseconds now,
  ...
  auto [it, inserted] =
      m_next_inv_to_inbounds_per_network_key.try_emplace(network_key, 0us);
...
bool PeerManagerImpl::BlockRequested(NodeId nodeid, const CBlockIndex &block,
...
void PeerManagerImpl::MaybeSetPeerAsAnnouncingHeaderAndIDs(NodeId nodeid) {
  ...
  m_connman.ForNode(nodeid, [this](CNode *pfrom) EXCLUSIVE_LOCKS_REQUIRED(
    ...
    if (lNodesAnnouncingHeaderAndIDs.size() >= 3) {
      ...
      m_connman.ForNode(lNodesAnnouncingHeaderAndIDs.front(),
                        [this](CNode *pnodeStop) {
...
int64_t PeerManagerImpl::ApproximateBestBlockDepth() const {
  return (GetTime<std::chrono::seconds>() - m_best_block_time.load()).count() /
...
// Logic for calculating which blocks to download from a given peer, given our
// current tip.
void PeerManagerImpl::FindNextBlocksToDownload(
  ...
  // When we sync with AssumeUtxo and discover the snapshot is not in the peer's
  // best chain, abort: We can't reorg to this chain due to missing undo data
  // until the background sync has finished, so downloading blocks from it would
  // be futile.
  ...
  // pindexLastCommonBlock is required to be an ancestor of
  // pindexBestKnownBlock, and will be used as a starting point. It is being set
  // to the fork point between the peer's best known block and the current tip,
  // unless it is already set to an ancestor with more work than the fork point.
  auto fork_point =
      LastCommonAncestor(state->pindexBestKnownBlock, m_chainman.ActiveTip());
  ...
  // BLOCK_DOWNLOAD_WINDOW + 1 beyond the last linked block we have in common
  // with this peer. The +1 is so we can detect stalling, namely if we would be
  // able to download that next block if the window were 1 larger.
  int nWindowEnd =
...
void PeerManagerImpl::TryDownloadingHistoricalBlocks(
  ...
  if (state->pindexBestKnownBlock == nullptr ||
      state->pindexBestKnownBlock->GetAncestor(target_block->nHeight) !=
          target_block) {
    // This peer can't provide us the complete series of blocks leading up to
    // the assumeutxo snapshot base.
    //
    // Presumably this peer's chain has less work than our ActiveChain()'s tip,
    // or else we will eventually crash when we try to reorg to it. Let other
    // logic deal with whether we disconnect this peer.
    //
...
void PeerManagerImpl::FindNextBlocks(std::vector<const CBlockIndex *> &vBlocks,
  ...
  while (pindexWalk->nHeight < nMaxHeight) {
    ...
    // of pindexWalk (towards pindexBestKnownBlock) into vToFetch. We fetch 128,
    ...
    // ones that are not yet downloaded and not in flight to vBlocks. In the
    // meantime, update pindexLastCommonBlock as long as all ancestors are
    // already downloaded, or if it's already part of our chain (and therefore
    ...
    for (const CBlockIndex *pindex : vToFetch) {
      ...
      if (IsBlockRequested(pindex->GetBlockHash())) {
        if (waitingfor == -1) {
          ...
          waitingfor = mapBlocksInFlight.lower_bound(pindex->GetBlockHash())
                           ->second.first;
        }
        continue;
      }
      ...
      if (is_limited_peer &&
          (state->pindexBestKnownBlock->nHeight - pindex->nHeight >=
           static_cast<int>(NODE_NETWORK_LIMITED_MIN_BLOCKS) -
               2 /* two blocks buffer for possible races */)) {
        continue;
      }
...
void PeerManagerImpl::PushNodeVersion(CNode &pnode, const Peer &peer) {
  ...
  MakeAndPushMessage(
      ...
      your_services,
      CNetAddr::V1(addr_you), // Together the pre-version-31402 serialization of
                              // CAddress "addrYou" (without nTime)
      my_services,
      CNetAddr::V1(CService{}), // Together the pre-version-31402 serialization
  ...
  if (fLogIPs) {
    LogDebug(BCLog::NET,
             ...
             "txrelay=%d, peer=%d\n",
             PROTOCOL_VERSION, nNodeStartingHeight, addr_you.ToStringAddrPort(),
             tx_relay, nodeid);
  } else {
    LogDebug(
        BCLog::NET,
...
void PeerManagerImpl::InitializeNode(const CNode &node,
  ...
  PeerRef peer =
      std::make_shared<Peer>(nodeid, our_services, node.IsInboundConn());
  {
    LOCK(m_peer_mutex);
...
bool PeerManagerImpl::HasAllDesirableServiceFlags(ServiceFlags services) const {
  // Shortcut for (services & GetDesirableServiceFlags(services)) ==
  // GetDesirableServiceFlags(services)
  return !(GetDesirableServiceFlags(services) & (~services));
}

ServiceFlags
PeerManagerImpl::GetDesirableServiceFlags(ServiceFlags services) const {
...
bool PeerManagerImpl::GetNodeStateStats(NodeId nodeid,
  ...
  // still responsive, since pingtime does not update until the ping is
  ...
  if (auto tx_relay = peer->GetTxRelay(); tx_relay != nullptr) {
    stats.m_relay_txs =
        WITH_LOCK(tx_relay->m_bloom_filter_mutex, return tx_relay->m_relay_txs);
...
PeerManagerImpl::GetOrphanTransactions() {
  LOCK(m_tx_download_mutex);
...
void PeerManagerImpl::Misbehaving(Peer &peer, const std::string &message) {
  ...
  TRACEPOINT(net, misbehaving_connection, peer.m_id, message.c_str());
}
...
void PeerManagerImpl::MaybePunishNodeForBlock(NodeId nodeid,
  ...
  case BlockValidationResult::BLOCK_CACHED_INVALID: {
    ...
    // Exempt HB compact block peers. Manual connections are always protected
    // from discouragement.
...
util::Expected<void, std::string>
PeerManagerImpl::FetchBlock(NodeId peer_id, const CBlockIndex &block_index) {
  if (m_chainman.m_blockman.LoadingBlocks())
    return util::Unexpected{"Loading blocks ..."};

  // Ensure this peer exists and hasn't been disconnected
  PeerRef peer = GetPeerRef(peer_id);
  if (peer == nullptr)
    return util::Unexpected{"Peer does not exist"};

  // Ignore pre-segwit peers
  if (!CanServeWitnesses(*peer))
    return util::Unexpected{"Pre-SegWit peer"};
  ...
  if (!BlockRequested(peer_id, block_index))
...
PeerManagerImpl::PeerManagerImpl(CConnman &connman, AddrMan &addrman,
    ...
    : m_rng{opts.deterministic_rng},
      ...
      m_mempool(pool), m_txdownloadman(node::TxDownloadOptions{
                           pool, m_rng, opts.deterministic_rng}),
      m_warnings{warnings}, m_opts{opts} {
  // While Erlay support is incomplete, it must be enabled explicitly via
  // -txreconciliation. This argument can go away after Erlay support is
  // complete.
  if (opts.reconcile_txs) {
    m_txreconciliation =
        std::make_unique<TxReconciliationTracker>(TXRECONCILIATION_VERSION);
  }
}
...
void PeerManagerImpl::ActiveTipChange(const CBlockIndex &new_tip, bool is_ibd) {
  // Ensure mempool mutex was released, otherwise deadlock may occur if another
  // thread holding m_tx_download_mutex waits on the mempool mutex.
  AssertLockNotHeld(m_mempool.cs);
  AssertLockNotHeld(m_tx_download_mutex);

  if (!is_ibd) {
    LOCK(m_tx_download_mutex);
    // If the chain tip has changed, previously rejected transactions might now
    // be valid, e.g. due to a timelock. Reset the rejection filters to give
    // those transactions another chance if we see them again.
...
void PeerManagerImpl::BlockConnected(
    const ChainstateRole &role, const std::shared_ptr<const CBlock> &pblock,
    const CBlockIndex *pindex) {
  ...
  // In case the dynamic timeout was doubled once or more, reduce it slowly back
...
void PeerManagerImpl::NewPoWValidBlock(
  ...
  auto pcmpctblock = std::make_shared<const CBlockHeaderAndShortTxIDs>(
...

  {
    auto most_recent_block_txs =
        std::make_unique<std::map<GenTxid, CTransactionRef>>();
  ...
  m_connman.ForEachNode([this, pindex, &lazy_ser, &hashBlock](
    ...
    if (state.m_requested_hb_cmpctblocks && !PeerHasHeader(&state, pindex) &&
...
void PeerManagerImpl::UpdatedBlockTip(const CBlockIndex *pindexNew,
  ...
  SetBestBlock(pindexNew->nHeight,
               std::chrono::seconds{pindexNew->GetBlockTime()});
...
/**
 * Handle invalid block rejection and consequent peer discouragement, maintain
 * which peers announce compact blocks.
 */
void PeerManagerImpl::BlockChecked(const std::shared_ptr<const CBlock> &block,
                                   const BlockValidationState &state) {
  LOCK(cs_main);
  ...
  // If the block failed validation, we know where it came from and we're still
...
void PeerManagerImpl::RelayTransaction(const Txid &txid, const Wtxid &wtxid) {
  ...
  for (auto &it : m_peer_map) {
    ...
    const uint256 &hash{peer.m_wtxid_relay ? wtxid.ToUint256()
                                           : txid.ToUint256()};
...
void PeerManagerImpl::ProcessGetBlockData(CNode &pfrom, Peer &peer,
                                          const CInv &inv) {
  ...
  bool need_activate_chain = false;
  {
    LOCK(cs_main);
    const CBlockIndex *pindex =
        m_chainman.m_blockman.LookupBlockIndex(inv.hash);
    if (pindex) {
      if (pindex->HaveNumChainTxs() && !pindex->IsValid(BLOCK_VALID_SCRIPTS) &&
        ...
        // validated it, we might be in the middle of connecting it (ie in the
        // unlock of cs_main before ActivateBestChain but after AcceptBlock). In
        // this case, we need to run ActivateBestChain prior to checking the
        // relay conditions below.
  ...
  FlatFilePos block_pos{};
  {
    ...
    if (m_connman.OutboundTargetReached(true) &&
        ...
        !pfrom.HasPermission(
            NetPermissionFlags::Download) // nodes with the download permission
                                          // may exceed target
    ) {
    ...
    if (!pfrom.HasPermission(NetPermissionFlags::NoBan) &&
      ...
      LogDebug(
          ...
          pfrom.DisconnectMsg(fLogIPs));
      // disconnect node and prevent it from stalling (would otherwise wait for
      // the missing block)
      pfrom.fDisconnect = true;
      return;
    }
  ...
  if (a_recent_block && a_recent_block->GetHash() == inv.hash) {
    ...
    // Fast-path: in this case it is possible to serve the block directly from
    // disk, as the network format matches the format on disk
  ...
  if (pblock) {
    if (inv.IsMsgBlk()) {
      ...
      if (sendMerkleBlock) {
        ...
        // CMerkleBlock just contains hashes, so also push any transactions in
        // the block the client did not see This avoids hurting performance by
        // pointlessly requiring a round-trip Note that there is currently no
        // way for a node to request any single transactions we didn't send here
        // - they must either disconnect and retry or request the full block.
        // Thus, the protocol spec specified allows for us to provide duplicate
        // txn here, however we MUST always provide at least what the remote
        // peer needs
        for (const auto &[tx_idx, _] : merkleBlock.vMatchedTxn)
          MakeAndPushMessage(pfrom, NetMsgType::TX,
                             TX_NO_WITNESS(*pblock->vtx[tx_idx]));
      }
      // else
      // no response
      ...
      if (can_direct_fetch &&
        ...
        if (a_recent_compact_block &&
            a_recent_compact_block->header.GetHash() == inv.hash) {
  ...
  {
    LOCK(peer.m_block_inv_mutex);
    // Trigger the peer node to send a getblocks request for the next batch of
    // inventory
...
CTransactionRef PeerManagerImpl::FindTxForGetData(const Peer::TxRelay &tx_relay,
                                                  const GenTxid &gtxid) {
  // If a tx was in the mempool prior to the last INV for this peer, permit the
  // request.
  auto txinfo{std::visit(
      [&](const auto &id) {
        return m_mempool.info_for_relay(
...
void PeerManagerImpl::ProcessGetData(
    CNode &pfrom, Peer &peer, const std::atomic<bool> &interruptMsgProc) {
  ...
  if (it != peer.m_getdata_requests.end() && !pfrom.fPauseSend) {
    ...
    // NOTE: previously we wouldn't do so and the peer sending us a malformed
    // GETDATA could result in never making progress and this thread using 100%
    // allocated CPU. See
...
void PeerManagerImpl::SendBlockTransactions(
  ...
  LogDebug(BCLog::CMPCTBLOCK,
           "Peer %d sent us a GETBLOCKTXN for block %s, sending a BLOCKTXN "
           "with %u txns. (%u bytes)\n",
           pfrom.GetId(), block.GetHash().ToString(), resp.txn.size(),
           tx_requested_size);
...
bool PeerManagerImpl::CheckHeadersPoW(const std::vector<CBlockHeader> &headers,
  ...
  if (!HasValidProofOfWork(headers, consensusParams, m_chainman)) {
...
bool PeerManagerImpl::IsContinuationOfLowWorkHeadersSync(
  ...
  if (peer.m_headers_sync) {
    auto result = peer.m_headers_sync->ProcessNextHeaders(
        headers, headers.size() == m_opts.max_headers_result);
    // If it is a valid continuation, we should treat the existing getheaders
    ...
    if (peer.m_headers_sync->GetState() == HeadersSyncState::State::FINAL) {
      ...
      if (best_it == m_headers_presync_stats.end()) {
        // If the cached best peer is outdated, iterate over all remaining ones
        // (including newly updated one) to find the best one.
        NodeId peer_best{-1};
  ...
  if (total_work < minimum_chain_work) {
    ...
    if (headers.size() == m_opts.max_headers_result) {
      ...
      peer.m_headers_sync.reset(new HeadersSyncState(
          peer.m_id, m_chainparams.GetConsensus(), m_chainparams.HeadersSync(),
...
bool PeerManagerImpl::IsAncestorOfBestHeaderOrTip(const CBlockIndex *header) {
  if (header == nullptr) {
    return false;
  } else if (m_chainman.m_best_header != nullptr &&
             header == m_chainman.m_best_header->GetAncestor(header->nHeight)) {
    return true;
 ...
 * CanDirectFetch()).
 */
void PeerManagerImpl::HeadersDirectFetchBlocks(CNode &pfrom, const Peer &peer,
                                               const CBlockIndex &last_header) {
  LOCK(cs_main);
  ...
  if (CanDirectFetch() && last_header.IsValid(BLOCK_VALID_TREE) &&
    ...
    if (!m_chainman.ActiveChain().Contains(pindexWalk)) {
      ...
      if (vGetData.size() > 0) {
        if (!m_opts.ignore_incoming_txs && nodestate->m_provides_cmpctblocks &&
...
void PeerManagerImpl::ProcessHeadersMessage(CNode &pfrom, Peer &peer,
  ...
  const CBlockIndex *chain_start_header{WITH_LOCK(
      ::cs_main,
  ...
  BlockValidationState state;
  const bool processed{m_chainman.ProcessNewBlockHeaders(
      headers,
      /*min_pow_checked=*/true, state, &pindexLast)};
  if (!processed) {
    if (state.IsInvalid()) {
      if (!pfrom.IsInboundConn() &&
          state.GetResult() == BlockValidationResult::BLOCK_CACHED_INVALID) {
        // Warn user if outgoing peers send us headers of blocks that we
        // previously marked as invalid.
        LogWarning("%s (received from peer=%i). "
                   "If this happens with all peers, consider database "
                   "corruption (that -reindex may fix) "
...
std::optional<node::PackageToValidate>
PeerManagerImpl::ProcessInvalidTx(NodeId nodeid, const CTransactionRef &ptx,
  ...
  LogDebug(BCLog::MEMPOOLREJ,
           "%s (wtxid=%s) from peer=%d was not accepted: %s\n",
           ptx->GetHash().ToString(), ptx->GetWitnessHash().ToString(), nodeid,
           state.ToString());

  const auto &[add_extra_compact_tx, unique_parents, package_to_validate] =
      m_txdownloadman.MempoolRejectedTx(ptx, state, nodeid, first_time_failure);
...
void PeerManagerImpl::ProcessValidTx(
    NodeId nodeid, const CTransactionRef &tx,
    const std::list<CTransactionRef> &replaced_transactions) {
  AssertLockNotHeld(m_peer_mutex);
  AssertLockHeld(g_msgproc_mutex);
  AssertLockHeld(m_tx_download_mutex);
  ...
  LogDebug(BCLog::MEMPOOL,
           "AcceptToMemoryPool: peer=%d: accepted %s (wtxid=%s) (poolsz %u "
           "txn, %u kB)\n",
           nodeid, tx->GetHash().ToString(), tx->GetWitnessHash().ToString(),
...
void PeerManagerImpl::ProcessPackageResult(
    const node::PackageToValidate &package_to_validate,
    const PackageMempoolAcceptResult &package_result) {
  AssertLockNotHeld(m_peer_mutex);
  AssertLockHeld(g_msgproc_mutex);
  AssertLockHeld(m_tx_download_mutex);
  ...
  // Iterate backwards to erase in-package descendants from the orphanage before
  // they become relevant in AddChildrenToWorkSet.
  ...
  while (package_iter != package.rend()) {
    ...
    const auto it_result{
        package_result.m_tx_results.find(tx->GetWitnessHash())};
    ...
    if (it_result != package_result.m_tx_results.end()) {
      ...
      case MempoolAcceptResult::ResultType::DIFFERENT_WITNESS: {
        // Don't add to vExtraTxnForCompact, as these transactions should have
        // already been added there when added to the orphanage or rejected for
        // TX_RECONSIDERABLE. This should be updated if package submission is
        // ever used for transactions that haven't already been validated
        // before.
      ...
      case MempoolAcceptResult::ResultType::MEMPOOL_ENTRY: {
        // AlreadyHaveTx() should be catching transactions that are already in
        // mempool.
        Assume(false);
        break;
      }
...
// NOTE: the orphan processing used to be uninterruptible and quadratic, which
// could allow a peer to stall the node for hours with specially crafted
// transactions. See https://bitcoincore.org/en/2024/07/03/disclose-orphan-dos.
bool PeerManagerImpl::ProcessOrphanTx(Peer &peer) {
  ...
  while (CTransactionRef porphanTx =
             m_txdownloadman.GetTxToReconsider(peer.m_id)) {
    ...
    if (result.m_result_type == MempoolAcceptResult::ResultType::VALID) {
      ...
      LogDebug(BCLog::TXPACKAGES,
               "   invalid orphan tx %s (wtxid=%s) from peer=%d. %s\n",
               orphanHash.ToString(), orphan_wtxid.ToString(), peer.m_id,
               state.ToString());
...
bool PeerManagerImpl::PrepareBlockFilterRequest(
...

  {
    ...
    // Check that the stop block exists and the peer would be allowed to fetch
    // it.
  ...
  if (start_height > stop_height) {
    LogDebug(BCLog::NET,
             "peer sent invalid getcfilters/getcfheaders with "
  ...
  if (stop_height - start_height >= max_height_diff) {
    LogDebug(BCLog::NET,
             "peer requested too many cfilters/cfheaders: %d / %d, %s\n",
             stop_height - start_height + 1, max_height_diff,
...
void PeerManagerImpl::ProcessGetCFHeaders(CNode &node, Peer &peer,
  ...
  if (start_height > 0) {
    ...
    if (!filter_index->LookupFilterHeader(prev_block, prev_header)) {
      LogDebug(BCLog::NET,
               "Failed to find block filter header in index: filter_type=%s, "
               "block_hash=%s\n",
               BlockFilterTypeName(filter_type),
...
void PeerManagerImpl::ProcessGetCFCheckPt(CNode &node, Peer &peer,
  ...
  if (!PrepareBlockFilterRequest(
          ...
          /*max_height_diff=*/std::numeric_limits<uint32_t>::max(), stop_index,
          filter_index)) {
    return;
  }
...
void PeerManagerImpl::ProcessCompactBlockTxns(
  ...
  bool fBlockRead{false};
  {
    ...
    auto range_flight =
        mapBlocksInFlight.equal_range(block_transactions.blockhash);
    size_t already_in_flight =
        std::distance(range_flight.first, range_flight.second);
    bool requested_block_from_this_peer{false};

    // Multimap ensures ordering of outstanding requests. It's either empty or
    // first in line.
    ...
    if (partialBlock.header.IsNull()) {
      // It is possible for the header to be empty if a previous call to
      // FillBlock wiped the header, but left the PartiallyDownloadedBlock
      // pointer around (i.e. did not call RemoveBlockRequest). In this case, we
    ...
    // We should not have gotten this far in compact block processing unless
    // it's attached to a known header
    const CBlockIndex *prev_block{Assume(m_chainman.m_blockman.LookupBlockIndex(
        partialBlock.header.hashPrevBlock))};
    ReadStatus status = partialBlock.FillBlock(
        *pblock, block_transactions.txn,
        /*segwit_active=*/
        DeploymentActiveAfter(prev_block, m_chainman,
                              Consensus::DEPLOYMENT_SEGWIT));
    if (status == READ_STATUS_INVALID) {
      RemoveBlockRequest(
          block_transactions.blockhash,
          pfrom.GetId()); // Reset in-flight state in case Misbehaving does not
      ...
      if (first_in_flight) {
        ...
        // We keep the failed partialBlock to disallow processing another
        // compact block announcement from the same peer for the same block. We
        // let the full block download below continue under the same
        // m_downloading_since timer.
      ...
      RemoveBlockRequest(block_transactions.blockhash,
                         pfrom.GetId()); // it is now an empty pointer
      fBlockRead = true;
  ...
  if (fBlockRead) {
    // Since we requested this block (it was in mapBlocksInFlight), force it to
    // be processed, even if it would not be a candidate for new tip (missing
    // previous block, chain not long enough, etc) This bypasses some anti-DoS
    // logic in AcceptBlock (eg to prevent disk-space attacks), but this should
    // be safe due to the protections in the compact block handler -- see
    // related comment in compact block optimistic reconstruction handling.
...
void PeerManagerImpl::LogBlockHeader(const CBlockIndex &index,
                                     const CNode &peer,
                                     bool via_compact_block) {
  // To prevent log spam, this function should only be called after it was
  // determined that a header is both new and valid.
  //
  ...
  // mining. In addition it can be used to identify peers which send us a
  // header, but don't followup with a complete and valid (compact) block.
  // Having this log by default when not in IBD ensures broad availability of
  // this data in case investigation is merited.
  const auto msg = strprintf("Saw new %sheader hash=%s height=%d peer=%d%s",
                             via_compact_block ? "cmpctblock " : "",
                             index.GetBlockHash().ToString(), index.nHeight,
                             peer.GetId(), peer.LogIP(fLogIPs));
...
void PeerManagerImpl::ProcessMessage(
    CNode &pfrom, const std::string &msg_type, DataStream &vRecv,
    const std::chrono::microseconds time_received,
    const std::atomic<bool> &interruptMsgProc) {
  AssertLockHeld(g_msgproc_mutex);
  ...
  if (msg_type == NetMsgType::VERSION) {
    ...
    if (pfrom.ExpectServicesFromConn() &&
      ...
      LogDebug(BCLog::NET,
               "peer does not offer the expected services (%08x offered, %08x "
               "expected), %s\n",
               nServices, GetDesirableServiceFlags(nServices),
               pfrom.DisconnectMsg(fLogIPs));
      pfrom.fDisconnect = true;
      return;
    }
    ...
    if (!vRecv.empty()) {
      // The version message includes information about the sending node which
      // we don't use:
    ...
    if (greatest_common_version >= 70016) {
      ...
      // some implementations reject messages they don't know. As a courtesy,
      // don't send it to nodes with a version before 70016, as no software is
      // known to support BIP155 that doesn't announce at least that protocol
      // version number.
    ...
    if (greatest_common_version >= WTXID_RELAY_VERSION && m_txreconciliation) {
      ...
      if (tx_relay &&
        ...
        const uint64_t recon_salt =
            m_txreconciliation->PreRegisterPeer(pfrom.GetId());
    ...
    if (send_getaddr) {
      ...
      // empty, so this mechanism is important to help us connect to the
      // network. We skip this for block-relay-only peers. We want to avoid
    ...
    LogDebug(BCLog::NET,
             ...
             "txrelay=%d, peer=%d%s%s\n",
             cleanSubVer, pfrom.nVersion, peer->m_starting_height,
             addrMe.ToStringAddrPort(), fRelay, pfrom.GetId(),
             pfrom.LogIP(fLogIPs),
    ...
    if (!pfrom.IsInboundConn()) {
      ...
      // harder for others to create false warnings about our clock being out of
      // sync.
  ...
  if (pfrom.nVersion == 0) {
    ...
    LogDebug(BCLog::NET,
             "non-version message before version handshake. Message \"%s\" "
             "from peer=%d\n",
  ...
  if (msg_type == NetMsgType::VERACK) {
    ...
    auto new_peer_msg = [&]() {
      const auto mapped_as{m_connman.GetMappedAS(pfrom.addr)};
      return strprintf(
          "New %s peer connected: transport: %s, version: %d, blocks=%d "
          "peer=%d%s%s\n",
    ...
    // Log successful connections unconditionally for outbound, but not for
    // inbound as those can be triggered by an attacker at high rate.
    if (pfrom.IsInboundConn()) {
    ...
    if (m_txreconciliation) {
      if (!peer->m_wtxid_relay ||
          !m_txreconciliation->IsPeerRegistered(pfrom.GetId())) {
        // We could have optimistically pre-registered/registered the peer. In
        // that case, we should forget about the reconciliation state here if
        // this wasn't followed by WTXIDRELAY (since WTXIDRELAY can't be
        // announced later).
...

    {
      ...
      m_txdownloadman.ConnectedPeer(
          pfrom.GetId(), node::TxDownloadConnectionInfo{
  ...
  if (msg_type == NetMsgType::SENDCMPCT) {
    ...
    // (receiving sendcmpct(1) signals high-bandwidth, sendcmpct(0)
    // low-bandwidth)
  ...
  // VERSION and VERACK to avoid relay problems from switching after a
  // connection is up.
  ...
  // BIP155 defines feature negotiation of addrv2 and sendaddrv2, which must
  ...
  // Received from a peer demonstrating readiness to announce transactions via
  // reconciliations. This feature negotiation must happen between VERSION and
  // VERACK to avoid relay problems from switching announcement protocols after
  // the connection is up.
  if (msg_type == NetMsgType::SENDTXRCNCL) {
    if (!m_txreconciliation) {
      LogDebug(BCLog::NET,
               "sendtxrcncl from peer=%d ignored, as our node does not have "
               "txreconciliation enabled\n",
               pfrom.GetId());
      return;
    }
    ...
    // Peer must not offer us reconciliations if we specified no tx relay
    // support in VERSION.
    if (RejectIncomingTxs(pfrom)) {
      LogDebug(BCLog::NET,
               "sendtxrcncl received to which we indicated no tx relay, %s\n",
               pfrom.DisconnectMsg(fLogIPs));
      pfrom.fDisconnect = true;
      return;
    }

    // Peer must not offer us reconciliations if they specified no tx relay
    // support in VERSION. This flag might also be false in other cases, but the
    // RejectIncomingTxs check above eliminates them, so that this flag fully
    // represents what we are looking for.
    const auto *tx_relay = peer->GetTxRelay();
    if (!tx_relay || !WITH_LOCK(tx_relay->m_bloom_filter_mutex,
                                return tx_relay->m_relay_txs)) {
      LogDebug(BCLog::NET,
               "sendtxrcncl received which indicated no tx relay to us, %s\n",
               pfrom.DisconnectMsg(fLogIPs));
      pfrom.fDisconnect = true;
      return;
    }
    ...
    const ReconciliationRegisterResult result =
        m_txreconciliation->RegisterPeer(pfrom.GetId(), pfrom.IsInboundConn(),
  ...
  if (msg_type == NetMsgType::ADDR || msg_type == NetMsgType::ADDRV2) {
    ...
    LogDebug(BCLog::NET,
             "Received addr: %u addresses (%u processed, %u rate-limited) from "
             "peer=%d\n",
  ...
  if (msg_type == NetMsgType::INV) {
    ...
    for (CInv &inv : vInv) {
      ...
      // Note that orphan parent fetching always uses MSG_TX GETDATAs regardless
      // of the wtxidrelay setting. This is fine as no INV messages are involved
      // in that process.
      if (peer->m_wtxid_relay) {
        if (inv.IsMsgTx())
          continue;
      ...
      if (inv.IsMsgBlk()) {
        ...
        if (!m_chainman.IsInitialBlockDownload()) {
          const bool fAlreadyHave{m_txdownloadman.AddTxAnnouncement(
              pfrom.GetId(), gtxid, current_time)};
          LogDebug(BCLog::NET, "got inv: %s  %s peer=%d\n", inv.ToString(),
                   fAlreadyHave ? "have" : "new", pfrom.GetId());
        }
      } else {
  ...
  if (msg_type == NetMsgType::GETBLOCKS) {
    ...
    if (locator.vHave.size() > MAX_LOCATOR_SZ) {
      LogDebug(BCLog::NET, "getblocks locator size %lld > %d, %s\n",
               locator.vHave.size(), MAX_LOCATOR_SZ,
               pfrom.DisconnectMsg(fLogIPs));
      pfrom.fDisconnect = true;
      return;
    }
    ...
    for (; pindex; pindex = m_chainman.ActiveChain().Next(pindex)) {
      ...
      // still have for some reasonable time window (1 hour) that block relay
      // might require.
  ...
  if (msg_type == NetMsgType::GETBLOCKTXN) {
    ...
    vRecv >> req;
    // Verify differential encoding invariant: indexes must be strictly
    // increasing DifferenceFormatter should guarantee this property during
    // deserialization
    ...
    if (!block_pos.IsNull()) {
      CBlock block;
      const bool ret{
          m_chainman.m_blockman.ReadBlock(block, block_pos, req.blockhash)};
    ...
    // The message processing loop will go around again (without pausing) and
    // we'll respond then
    return;
  }
  ...
  if (msg_type == NetMsgType::GETHEADERS) {
    ...
    if (m_chainman.ActiveTip() == nullptr ||
      ...
      LogDebug(BCLog::NET,
               "Ignoring getheaders from peer=%d because active chain has too "
               "little work; sending empty response\n",
               pfrom.GetId());
    ...
    // if our peer has m_chainman.ActiveChain().Tip() (and thus we are sending
    // an empty headers message). In both cases it's safe to update
  ...
  if (msg_type == NetMsgType::TX) {
    if (RejectIncomingTxs(pfrom)) {
      LogDebug(BCLog::NET, "transaction sent in violation of protocol, %s",
               pfrom.DisconnectMsg(fLogIPs));
      pfrom.fDisconnect = true;
      return;
    }
    ...
    if (!should_validate) {
      if (pfrom.HasPermission(NetPermissionFlags::ForceRelay)) {
        ...
        if (!m_mempool.exists(txid)) {
          LogInfo("Not relaying non-mempool transaction %s (wtxid=%s) from "
                  "forcerelay peer=%d\n",
    ...
    if (state.IsInvalid()) {
      if (auto package_to_validate{ProcessInvalidTx(
              pfrom.GetId(), ptx, state, /*first_time_failure=*/true)}) {
        const auto package_result{ProcessNewPackage(
            m_chainman.ActiveChainstate(), m_mempool,
            package_to_validate->m_txns, /*test_accept=*/false,
            /*client_maxfeerate=*/std::nullopt)};
  ...
  if (msg_type == NetMsgType::CMPCTBLOCK) {
...

    {
      ...
      const CBlockIndex *prev_block = m_chainman.m_blockman.LookupBlockIndex(
      ...
      if (!prev_block) {
        ...
        if (!m_chainman.IsInitialBlockDownload()) {
          MaybeSendGetHeaders(pfrom, GetLocator(m_chainman.m_best_header),
                              *peer);
        }
        return;
      } else if (prev_block->nChainWork +
    ...
    if (!m_chainman.ProcessNewBlockHeaders(
            {{cmpctblock.header}}, /*min_pow_checked=*/true, state, &pindex)) {
      if (state.IsInvalid()) {
...

    {
      ...
      if (received_new_header &&
          pindex->nChainWork > m_chainman.ActiveChain().Tip()->nChainWork) {
        nodestate->m_last_block_announcement = GetTime();
      }
      ...
      if (pindex->nChainWork <= m_chainman.ActiveChain()
                                    .Tip()
        ...
        if ((already_in_flight < MAX_CMPCTBLOCKS_INFLIGHT_PER_BLOCK &&
          ...
          if (!BlockRequested(pfrom.GetId(), *pindex, &queuedBlockIt)) {
            if (!(*queuedBlockIt)->partialBlock)
              (*queuedBlockIt)
                  ->partialBlock.reset(
                      new PartiallyDownloadedBlock(&m_mempool));
            else {
          ...
          if (status == READ_STATUS_INVALID) {
            RemoveBlockRequest(
                pindex->GetBlockHash(),
                pfrom.GetId()); // Reset in-flight state in case Misbehaving
                                // does not result in a disconnect
            Misbehaving(*peer, "invalid compact block");
            return;
          } else if (status == READ_STATUS_FAILED) {
            if (first_in_flight) {
              // Duplicate txindexes, the block is now in-flight, so just
              // request it
              std::vector<CInv> vInv(1);
              vInv[0] = CInv(MSG_BLOCK | GetFetchFlags(*peer), blockhash);
              MakeAndPushMessage(pfrom, NetMsgType::GETDATA, vInv);
          ...
          if (req.indexes.empty()) {
            ...
            // what we can get), or
            // - it's not the final parallel download slot (which we may reserve
            // for first outbound)
            req.blockhash = pindex->GetBlockHash();
            MakeAndPushMessage(pfrom, NetMsgType::GETBLOCKTXN, req);
          } else {
          ...
          const CBlockIndex *prev_block{
              Assume(m_chainman.m_blockman.LookupBlockIndex(
                  cmpctblock.header.hashPrevBlock))};
          status = tempBlock.FillBlock(
              *pblock, dummy,
              /*segwit_active=*/
              DeploymentActiveAfter(prev_block, m_chainman,
                                    Consensus::DEPLOYMENT_SEGWIT));
    ...
    if (fRevertToHeaderProcessing) {
      ...
      // disconnect the peer if the header turns out to be for an invalid block.
      // Note that if a peer tries to build on an invalid chain, that
      // will be detected and the peer will be disconnected/discouraged.
      return ProcessHeadersMessage(pfrom, *peer, {cmpctblock.header},
                                   /*via_compact_block=*/true);
    }
  ...
  if (msg_type == NetMsgType::HEADERS) {
    ...
    ProcessHeadersMessage(pfrom, *peer, std::move(headers),
    ...
    if (m_headers_presync_should_signal.exchange(false)) {
      ...
      if (stats.second) {
        m_chainman.ReportHeadersPresync(stats.first, stats.second->first,
                                        stats.second->second);
      }
    }
  ...
  if (msg_type == NetMsgType::BLOCK) {
    ...
    const CBlockIndex *prev_block{WITH_LOCK(
        m_chainman.GetMutex(),
        return m_chainman.m_blockman.LookupBlockIndex(pblock->hashPrevBlock))};

    // Check for possible mutation if it connects to something we know so we can
    // check for DEPLOYMENT_SEGWIT being active
    if (prev_block &&
        IsBlockMutated(
            /*block=*/*pblock,
    ...
    bool min_pow_checked = false;
    {
      ...
      if (prev_block &&
          prev_block->nChainWork +
                  CalculateClaimedHeadersWork({{pblock->GetBlockHeader()}}) >=
              GetAntiDoSWorkThreshold()) {
        min_pow_checked = true;
      }
    }
  ...
  if (msg_type == NetMsgType::GETADDR) {
    ...
    // sending getaddr messages. Making nodes which are behind NAT and can only
    // make outgoing connections ignore the getaddr message mitigates the
    // attack.
    if (!pfrom.IsInboundConn()) {
    ...
    if (pfrom.HasPermission(NetPermissionFlags::Addr)) {
      vAddr = m_connman.GetAddressesUnsafe(
          MAX_ADDR_TO_SEND, MAX_PCT_ADDR_TO_SEND, /*network=*/std::nullopt);
    } else {
      vAddr =
  ...
  if (msg_type == NetMsgType::MEMPOOL) {
    ...
    if (!(peer->m_our_services & NODE_BLOOM) &&
      ...
      if (!pfrom.HasPermission(NetPermissionFlags::NoBan)) {
        LogDebug(BCLog::NET,
                 "mempool request with bloom filters disabled, %s\n",
                 pfrom.DisconnectMsg(fLogIPs));
        pfrom.fDisconnect = true;
      }
      return;
    ...
    if (m_connman.OutboundTargetReached(false) &&
      ...
      if (!pfrom.HasPermission(NetPermissionFlags::NoBan)) {
        LogDebug(BCLog::NET,
                 "mempool request with bandwidth limit reached, %s\n",
                 pfrom.DisconnectMsg(fLogIPs));
        pfrom.fDisconnect = true;
      }
      return;
  ...
  if (msg_type == NetMsgType::PING) {
    if (pfrom.GetCommonVersion() > BIP0031_VERSION) {
      ...
      //    can avoid sending us more work, like chain download requests.
      //
      ...
      // node takes 5 seconds to respond to each, the 5th ping the remote sends
      // would appear to return very quickly.
  ...
  if (msg_type == NetMsgType::PONG) {
    ...
    if (!(sProblem.empty())) {
      LogDebug(BCLog::NET,
               ...
               pfrom.GetId(), sProblem, peer->m_ping_nonce_sent, nonce, nAvail);
    }
    if (bPingFinished) {
  ...
  if (msg_type == NetMsgType::FILTERADD) {
    ...
    // Nodes must NEVER send a data item > MAX_SCRIPT_ELEMENT_SIZE bytes (the
    // max size for a script data object, and thus, the maximum size any matched
    // object can have) in a filteradd message
    bool bad = false;
  ...
  if (msg_type == NetMsgType::NOTFOUND) {
    ...
    if (vInv.size() <=
        node::MAX_PEER_TX_ANNOUNCEMENTS + MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
      for (CInv &inv : vInv) {
        if (inv.IsGenTxMsg()) {
...
bool PeerManagerImpl::MaybeDiscourageAndDisconnect(CNode &pnode, Peer &peer) {
  ...
  if (pnode.addr.IsLocal()) {
    // We disconnect local peers for bad behavior but don't discourage (since
    // that would discourage all peers on the same local address)
    LogDebug(BCLog::NET,
...
bool PeerManagerImpl::ProcessMessages(CNode *pfrom,
  ...
  TRACEPOINT(net, inbound_message, pfrom->GetId(), pfrom->m_addr_name.c_str(),
  ...
  try {
    ...
    LogDebug(BCLog::NET, "%s(%s, %u bytes): Exception '%s' (%s) caught\n",
             __func__, SanitizeString(msg.m_type), msg.m_message_size, e.what(),
             typeid(e).name());
  } catch (...) {
...
void PeerManagerImpl::ConsiderEviction(CNode &pto, Peer &peer,
  ...
  if (!state.m_chain_sync.m_protect && pto.IsOutboundOrBlockRelayConn() &&
    ...
    if (state.pindexBestKnownBlock != nullptr &&
        state.pindexBestKnownBlock->nChainWork >=
            m_chainman.ActiveChain().Tip()->nChainWork) {
      // The outbound peer has sent us a block with at least as much work as our
      // current tip, so reset the timeout if it was set
      ...
      // At this point we know that the outbound peer has either never sent us a
      // block/header or they have, but its tip is behind ours AND we are
      // noticing this for the first time (m_timeout is 0) OR we noticed this at
      // some point within the last CHAIN_SYNC_TIMEOUT + HEADERS_RESPONSE_TIME
      // seconds and set a timeout for them, they caught up to our tip at the
      // time of setting the timer but not to our current one (we've also
      // advanced). Either way, set a new timeout based on our current tip.
      ...
      // that of our tip, when we first detected it was behind. Send a single
      // getheaders message to give the peer a chance to update us.
      if (state.m_chain_sync.m_sent_getheaders) {
        ...
        MaybeSendGetHeaders(
            pto, GetLocator(state.m_chain_sync.m_work_header->pprev), peer);
        LogDebug(BCLog::NET,
...
void PeerManagerImpl::EvictExtraOutboundPeers(std::chrono::seconds now) {
  ...
  // it's given us a block -- in which case, compare with the second-youngest,
  // and out of those two, disconnect the peer who least recently gave us a
  // block. The youngest block-relay-only peer would be the extra peer we
  // connected to temporarily in order to sync our tip; see net.cpp. Note that
  // we use higher nodeid as a measure for most recent connection.
  if (m_connman.GetExtraBlockRelayCount() > 0) {
    ...
    m_connman.ForNode(
        to_disconnect, [&](CNode *pnode) EXCLUSIVE_LOCKS_REQUIRED(::cs_main) {
          AssertLockHeld(::cs_main);
          ...
          if (node_state == nullptr ||
            ...
            pnode->fDisconnect = true;
            LogDebug(BCLog::NET,
                     "disconnecting extra block-relay-only peer=%d (last block "
                     "received at time %d)\n",
  ...
  if (m_connman.GetExtraFullOutboundCount() > 0) {
    ...
    m_connman.ForEachNode([&](CNode *pnode) EXCLUSIVE_LOCKS_REQUIRED(
                              ::cs_main, m_connman.GetNodesMutex()) {
      AssertLockHeld(::cs_main);
      ...
      if (!m_connman.MultipleManualOrFullOutboundConns(
              pnode->addr.GetNetwork()))
        return;
...
void PeerManagerImpl::CheckForStaleTipAndEvictPeers() {
  ...
  if (now > m_stale_tip_check_time) {
    ...
    if (!m_chainman.m_blockman.LoadingBlocks() &&
        ...
        TipMayBeStale()) {
      LogInfo("Potential stale tip detected, will try using extra outbound "
...
void PeerManagerImpl::MaybeSendPing(CNode &node_to, Peer &peer,
  ...
  if (m_connman.ShouldRunInactivityChecks(
    ...
    LogDebug(BCLog::NET, "ping timeout: %fs, %s",
             0.000001 * count_microseconds(now - peer.m_ping_start.load()),
             node_to.DisconnectMsg(fLogIPs));
...
void PeerManagerImpl::MaybeSendAddr(CNode &node, Peer &peer,
  ...
  auto addr_already_known =
      [&peer](const CAddress &addr) EXCLUSIVE_LOCKS_REQUIRED(g_msgproc_mutex) {
  ...
  if (peer.m_wants_addrv2) {
    MakeAndPushMessage(node, NetMsgType::ADDRV2,
                       CAddress::V2_NETWORK(peer.m_addrs_to_send));
  } else {
    MakeAndPushMessage(node, NetMsgType::ADDR,
                       CAddress::V1_NETWORK(peer.m_addrs_to_send));
  }
  peer.m_addrs_to_send.clear();
...
void PeerManagerImpl::MaybeSendFeefilter(
  ...
  if (pto.HasPermission(NetPermissionFlags::ForceRelay))
    return;
  // Don't send feefilter messages to outbound block-relay-only peers since they
  // should never announce transactions to us, regardless of feefilter state.
  ...
  if (current_time > peer.m_next_send_feefilter) {
    ...
    filterToSend =
        std::max(filterToSend, m_mempool.m_opts.min_relay_feerate.GetFeePerK());
...
bool PeerManagerImpl::SetupAddressRelay(const CNode &node, Peer &peer) {
  ...
  if (!peer.m_addr_relay_enabled.exchange(true)) {
    ...
    // or on first addr-related message we have received (inbound peers),
    // initialize m_addr_known.
...

  {
      ...
      if ((nSyncStarted == 0 && sync_blocks_and_headers_from_peer) ||
        ...
        if (MaybeSendGetHeaders(*pto, GetLocator(pindexStart), *peer)) {
          ...
          peer->m_headers_sync_timeout =
              ...
              (
                  // Convert HEADERS_DOWNLOAD_TIMEOUT_PER_HEADER to microseconds
                  // before scaling to maintain precision
                  std::chrono::microseconds{
                      HEADERS_DOWNLOAD_TIMEOUT_PER_HEADER} *
                  Ticks<std::chrono::seconds>(
                      NodeClock::now() - m_chainman.m_best_header->Time()) /
    ...
    //
    {
        ...
        if (vHeaders.size() == 1 && state.m_requested_hb_cmpctblocks) {
          ...
          std::optional<CSerializedNetMsg> cached_cmpctblock_msg;
          {
            ...
            if (m_most_recent_block_hash == pBestIndex->GetBlockHash()) {
              cached_cmpctblock_msg = NetMsg::Make(
                  NetMsgType::CMPCTBLOCK, *m_most_recent_compact_block);
            }
          }
          ...
          MakeAndPushMessage(*pto, NetMsgType::HEADERS,
      ...
      if (fRevertToInv) {
        ...
        // The last entry in m_blocks_for_headers_relay was our tip at some
        // point in the past.
        if (!peer->m_blocks_for_headers_relay.empty()) {
          ...
          if (m_chainman.ActiveChain()[pindex->nHeight] != pindex) {
            LogDebug(BCLog::NET,
                     ...
                     m_chainman.ActiveChain().Tip()->GetBlockHash().ToString());
          }
    ...
    if (auto tx_relay = peer->GetTxRelay(); tx_relay != nullptr) {
      ...
      if (fSendTrickle) {
        ...
        // priority reasons. A heap is used so that not all items need sorting
        // if only a few are being sent.
        ...
        size_t broadcast_max{INVENTORY_BROADCAST_TARGET +
                             (tx_relay->m_tx_inventory_to_send.size() / 1000) *
                                 5};
        broadcast_max =
            std::min<size_t>(INVENTORY_BROADCAST_MAX, broadcast_max);
        while (!vInvTx.empty() && nRelayedTransactions < broadcast_max) {
          ...
          // `TxRelay::m_tx_inventory_known_filter` contains either txids or
          // wtxids depending on whether our peer supports wtxid-relay.
          // Therefore, first construct the inv and then use its hash for the
          // filter check.
          const auto inv = peer->m_wtxid_relay
                               ? CInv{MSG_WTX, wtxid.ToUint256()}
          ...
          if (tx_relay->m_bloom_filter &&
              !tx_relay->m_bloom_filter->IsRelevantAndUpdate(*txinfo.tx))
            continue;
          // Send
          vInv.push_back(inv);
        ...
        // Ensure we'll respond to GETDATA requests for anything we've just
        // announced
        LOCK(m_mempool.cs);
    ...
    if (state.m_stalling_since.count() &&
      ...
      LogInfo("Peer is stalling block download, %s\n",
              pto->DisconnectMsg(fLogIPs));
      pto->fDisconnect = true;
      // Increase timeout for the next peer so that we don't disconnect multiple
      // peers if our own bandwidth is insufficient.
      const auto new_timeout =
          std::min(2 * stalling_timeout, BLOCK_STALLING_TIMEOUT_MAX);
      if (stalling_timeout != new_timeout &&
          m_block_stalling_timeout.compare_exchange_strong(stalling_timeout,
                                                           new_timeout)) {
        LogDebug(BCLog::NET,
    ...
    if (state.fSyncStarted &&
      ...
      if (m_chainman.m_best_header->Time() <= NodeClock::now() - 24h) {
        if (current_time > peer->m_headers_sync_timeout && nSyncStarted == 1 &&
            (m_num_preferred_download_peers - state.fPreferredDownload >= 1)) {
          // Disconnect a peer (without NetPermissionFlags::NoBan permission) if
          // it is our only sync peer, and we have others we could be using
          // instead. Note: If all our peers are inbound, then we won't
    ...
    if (CanServeBlocks(*peer) &&
      ...
      if (historical_blocks && !IsLimitedPeer(*peer)) {
        ...
        const CBlockIndex *from_tip = LastCommonAncestor(
            historical_blocks->first, historical_blocks->second);
        TryDownloadingHistoricalBlocks(*peer, get_inflight_budget(),
    ...
    {
      LOCK(m_tx_download_mutex);
      for (const GenTxid &gtxid :
           m_txdownloadman.GetRequestsToSend(pto->GetId(), current_time)) {
        vGetData.emplace_back(gtxid.IsWtxid() ? MSG_WTX
                                              : (MSG_TX | GetFetchFlags(*peer)),
                              gtxid.ToUint256());
