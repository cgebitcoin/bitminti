 ...
 *
 * The definition of LocaleIndependentAtoi<int>() requires that non-numeric
 * string values like "foo", return 0. This means that if a user unintentionally
 * supplies a non-integer argument here, the return value is always false. This
 * means that -foo=false does what the user probably expects, but -foo=true is
 * well defined but does not do what they probably expected.
 *
 * The return value of LocaleIndependentAtoi<int>(...) is zero when given input
...
std::optional<common::SettingsValue> InterpretValue(const KeyInfo &key,
  ...
  if (key.negated) {
    if (flags & ArgsManager::DISALLOW_NEGATION) {
      error = strprintf(
          "Negating of -%s is meaningless and therefore forbidden", key.name);
      return std::nullopt;
    }
    // Double negatives like -nofoo=0 are supported (but discouraged)
    if (value && !InterpretBool(*value)) {
      LogWarning("Parsed potentially confusing double-negative -%s=%s",
                 key.name, *value);
      return true;
    }
    return false;
  ...
  if (!value && (flags & ArgsManager::DISALLOW_ELISION)) {
    error = strprintf(
        "Can not set -%s with no value. Please specify value with -%s=value.",
        key.name, key.name);
    return std::nullopt;
  }
  return value ? *value : "";
...
std::list<SectionInfo> ArgsManager::GetUnrecognizedSections() const {
  ...
  unrecognized.remove_if([](const SectionInfo &appeared) {
    return available_sections.contains(appeared.m_name);
  });
  return unrecognized;
}
...
bool ArgsManager::ParseParameters(int argc, const char *const argv[],
  ...
  if (auto *includes =
          common::FindKey(m_settings.command_line_options, "includeconf")) {
    const common::SettingsSpan values{*includes};
    // Range may be empty if -noincludeconf was passed
    if (!values.empty()) {
      error = "-includeconf cannot be used from commandline; -includeconf=" +
              values.begin()->write();
...
bool ArgsManager::ReadSettingsFile(std::vector<std::string> *errors) {
  ...
  for (const auto &setting : m_settings.rw_settings) {
    KeyInfo key = InterpretKey(
        setting.first); // Split setting key into section and argname
...
void ArgsManager::AddCommand(const std::string &cmd, const std::string &help) {
  ...
  std::map<std::string, Arg> &arg_map =
      m_available_args[OptionsCategory::COMMANDS];
...
void ArgsManager::CheckMultipleCLIArgs() const {
  ...
  if (cmds != m_available_args.end()) {
    ...
    if (found.size() > 1) {
      throw std::runtime_error(strprintf("Only one of %s may be specified.",
                                         util::Join(found, ", ")));
    }
  }
}
...
bool HelpRequested(const ArgsManager &args) {
  return args.IsArgSet("-?") || args.IsArgSet("-h") || args.IsArgSet("-help") ||
         args.IsArgSet("-help-debug");
}

void SetupHelpOptions(ArgsManager &args) {
  args.AddArg("-help", "Print this help message and exit (also -h or -?)",
...
const std::vector<std::string> TEST_OPTIONS_DOC{
    ...
    "reindex_after_failure_noninteractive_yes (When asked for a reindex after "
    "failure interactively, simulate as-if answered with 'yes')",
...
bool HasTestOption(const ArgsManager &args, const std::string &test_option) {
  ...
  return std::any_of(
      ...
      [test_option](const auto &option) { return option == test_option; });
}
...
std::variant<ChainType, std::string> ArgsManager::GetChainArg() const {
  auto get_net = [&](const std::string &arg) {
    ...
    common::SettingsValue value =
        common::GetSetting(m_settings, /* section= */ "", SettingName(arg),
  ...
  if ((int)chain_arg.has_value() + (int)fRegTest + (int)fSigNet +
          (int)fTestNet + (int)fTestNet4 >
      1) {
    throw std::runtime_error(
...
ArgsManager::GetSettingsList(const std::string &arg) const {
  LOCK(cs_args);
  return common::GetSettingsList(m_settings, m_network, SettingName(arg),
