...
void BlockTreeDB::WriteBatchSync(
  ...
  for (const CBlockIndex *bi : blockinfo) {
    batch.Write(std::make_pair(DB_BLOCK_INDEX, bi->GetBlockHash()),
                CDiskBlockIndex{bi});
  }
...
bool BlockTreeDB::LoadBlockIndexGuts(
...
std::string CBlockFileInfo::ToString() const {
  return strprintf(
      ...
      nBlocks, nSize, nHeightFirst, nHeightLast, FormatISO8601Date(nTimeFirst),
...
bool CBlockIndexWorkComparator::operator()(const CBlockIndex *pa,
                                           const CBlockIndex *pb) const {
...
void BlockManager::FindFilesToPruneManual(std::set<int> &setFilesToPrune,
  ...
  const auto [min_block_to_prune, last_block_can_prune] =
      chain.GetPruneRange(nManualPruneHeight);
...
void BlockManager::FindFilesToPrune(std::set<int> &setFilesToPrune,
  ...
  const auto target = std::max(MIN_DISK_SPACE_FOR_BLOCK_FILES,
                               GetPruneTarget() / num_chainstates);
  ...
  const auto [min_block_to_prune, last_block_can_prune] =
      chain.GetPruneRange(last_prune);
  ...
  if (nCurrentUsage + nBuffer >= target) {
    ...
    if (chainman.IsInitialBlockDownload() &&
        target_sync_height > (uint64_t)chain_tip_height) {
      // Since this is only relevant during IBD, we assume blocks are at least 1
      // MB on average
...
bool BlockManager::LoadBlockIndex(
  ...
  if (snapshot_blockhash) {
    const std::optional<AssumeutxoData> maybe_au_data =
        GetParams().AssumeutxoForBlockhash(*snapshot_blockhash);
    if (!maybe_au_data) {
      m_opts.notifications.fatalError(strprintf(
          _("Assumeutxo data not found for the given blockhash '%s'."),
    ...
    // snapshot height is null. This is relevant during snapshot completion,
    // when the blockman may be loaded with a height that then needs to be
    // cleared after the snapshot is fully validated.
  ...
  for (CBlockIndex *pindex : vSortedByHeight) {
    ...
    // some point, or blocks that are assumed-valid on the basis of snapshot
    // load (see PopulateAndValidateSnapshot()). Pruned nodes may have deleted
    // the block.
    if (pindex->nTx > 0) {
      if (pindex->pprev) {
        if (m_snapshot_height && pindex->nHeight == *m_snapshot_height &&
          ...
          pindex->m_chain_tx_count =
              pindex->pprev->m_chain_tx_count + pindex->nTx;
        } else {
...
void BlockManager::WriteBlockIndexDB() {
  ...
  for (std::set<int>::iterator it = m_dirty_fileinfo.begin();
       it != m_dirty_fileinfo.end();) {
    vFiles.emplace_back(*it, &m_blockfile_info[*it]);
    m_dirty_fileinfo.erase(it++);
  }
  std::vector<const CBlockIndex *> vBlocks;
  vBlocks.reserve(m_dirty_blockindex.size());
  for (std::set<CBlockIndex *>::iterator it = m_dirty_blockindex.begin();
       it != m_dirty_blockindex.end();) {
...
bool BlockManager::LoadBlockIndexDB(
  ...
  LogInfo("Loading block index db: last block file info: %s",
...
BlockManager::GetFirstBlock(const CBlockIndex &upper_block,
  ...
  assert((last_block->nStatus & status_mask) ==
         status_mask); // 'upper_block' must satisfy the status mask
  while (last_block->pprev &&
         ((last_block->pprev->nStatus & status_mask) == status_mask)) {
    if (lower_block) {
      // Return if we reached the lower_block
      if (last_block == lower_block)
        return *lower_block;
      // if range was surpassed, means that 'lower_block' is not part of the
      // 'upper_block' chain and so far this is not allowed.
...
bool BlockManager::CheckBlockDataAvailability(const CBlockIndex &upper_block,
  ...
  if (!(upper_block.nStatus & BLOCK_HAVE_DATA))
    return false;
  return &GetFirstBlock(upper_block, BLOCK_HAVE_DATA, &lower_block) ==
         &lower_block;
}
...
void BlockManager::CleanupBlockRevFiles() const {
  ...
  LogInfo("Removing unusable blk?????.dat and rev?????.dat files for -reindex "
          "with -prune");
  for (fs::directory_iterator it(m_opts.blocks_dir);
       it != fs::directory_iterator(); it++) {
    const std::string path = fs::PathToString(it->path().filename());
    if (fs::is_regular_file(*it) && path.length() == 12 &&
        path.ends_with(".dat")) {
...
bool BlockManager::ReadBlockUndo(CBlockUndo &blockundo,
  ...
  if (file.IsNull()) {
    LogError("OpenUndoFile failed for %s while reading block undo",
             pos.ToString());
    return false;
  }
  ...
  try {
    // Read block
    HashVerifier verifier{filein}; // Use HashVerifier, as reserializing may
                                   // lose data, c.f. commit d3424243
    ...
    if (hashChecksum != verifier.GetHash()) {
      LogError("Checksum mismatch at %s while reading block undo",
               pos.ToString());
      return false;
    }
...
bool BlockManager::FlushUndoFile(int block_file, bool finalize) {
  ...
  if (!m_undo_file_seq.Flush(undo_pos_old, finalize)) {
    m_opts.notifications.flushError(_("Flushing undo file to disk failed. This "
                                      "is likely the result of an I/O error."));
    return false;
  }
  return true;
}
...
bool BlockManager::FlushBlockFile(int blockfile_num, bool fFinalize,
  ...
  if (m_blockfile_info.size() < 1) {
    ...
    // chainstate init, when we call ChainstateManager::MaybeRebalanceCaches()
    // (which then calls FlushStateToDisk()), resulting in a call to this
    // function before we have populated `m_blockfile_info` via
    // LoadBlockIndexDB().
    return true;
  }
  ...
  if (!m_block_file_seq.Flush(block_pos_old, fFinalize)) {
    m_opts.notifications.flushError(
        _("Flushing block file to disk failed. This is likely the result of an "
          "I/O error."));
    success = false;
  }
  // we do not always flush the undo file, as the chain tip may be lagging
  // behind the incoming blocks, e.g. during IBD or a sync after a node going
  // offline
...
void BlockManager::UnlinkPrunedFiles(
  ...
  for (std::set<int>::iterator it = setFilesToPrune.begin();
    ...
    const bool removed_blockfile{
        fs::remove(m_block_file_seq.FileName(pos), ec)};
...
FlatFilePos BlockManager::FindNextBlockPos(unsigned int nAddSize,
  ...
  if (!m_blockfile_cursors[chain_type]) {
    // If a snapshot is loaded during runtime, we may not have initialized this
    // cursor yet.
  ...
  while (m_blockfile_info[nFile].nSize + nAddSize >= max_blockfile_size) {
    // when the undo file is keeping up with the block file, we want to flush it
    // explicitly when it is lagging behind (more blocks arrive than are being
    // connected), we let the undo block write case handle it
  ...
  if (nFile != last_blockfile) {
    ...
    if (!FlushBlockFile(last_blockfile, /*fFinalize=*/true, finalize_undo)) {
      LogWarning("Failed to flush previous block file %05i (finalize=1, "
...
bool BlockManager::FindUndoPos(BlockValidationState &state, int nFile,
  ...
  bool out_of_space;
  size_t bytes_allocated =
      m_undo_file_seq.Allocate(pos, nAddSize, out_of_space);
  if (out_of_space) {
...
bool BlockManager::WriteBlockUndo(const CBlockUndo &blockundo,
  ...
  if (block.GetUndoPos().IsNull()) {
    FlatFilePos pos;
    const auto blockundo_size{
        static_cast<uint32_t>(GetSerializeSize(blockundo))};
    if (!FindUndoPos(state, block.nFile, pos,
                     blockundo_size + UNDO_DATA_DISK_OVERHEAD)) {
      LogError("FindUndoPos failed for %s while writing block undo",
               pos.ToString());
      return false;
    }
    ...
    if (file.IsNull()) {
      LogError("OpenUndoFile failed for %s while writing block undo",
               pos.ToString());
      return FatalError(m_opts.notifications, state,
                        _("Failed to write undo data."));
    }
    {
      ...
      // BufferedWriter will flush pending data to file when fileout goes out of
      // scope.
    }

    // Make sure that the file is closed before we call `FlushUndoFile`.
    if (file.fclose() != 0) {
      LogError("Failed to close block undo file %s: %s", pos.ToString(),
               SysErrorString(errno));
      return FatalError(m_opts.notifications, state,
                        _("Failed to close block undo file."));
    }

    // rev files are written in block height order, whereas blk files are
    // written as blocks come in (often out of order) we want to flush the rev
    // (undo) file once we've written the last block, which is indicated by the
    // last height in the block file info as below; note that this does not
    // catch the case where the undo writes are keeping up with the block writes
    // (usually when a synced up node is getting newly mined blocks) -- this
    // case is caught in the FindNextBlockPos function
...
bool BlockManager::ReadBlock(
    CBlock &block, const FlatFilePos &pos,
  ...
  try {
    ...
    LogError("Deserialize or I/O error - %s at %s while reading block",
             e.what(), pos.ToString());
    return false;
  }
  ...
  // BTC3: Header validation happens in validation.cpp using contextual RandomX
  // seeds. Legacy SHA256 CheckProofOfWork is invalid here.
  /*
  if (!CheckProofOfWork(block_hash, block.nBits, GetConsensus())) {
      LogError("Errors in block header at %s while reading block",
  pos.ToString()); return false;
  }
  */

  // Signet only: check block solution
  if (GetConsensus().signet_blocks &&
      !CheckSignetBlockSolution(block, GetConsensus())) {
    LogError("Errors in block solution at %s while reading block",
             pos.ToString());
    return false;
  }
...
BlockManager::ReadRawBlockResult BlockManager::ReadRawBlock(
    const FlatFilePos &pos,
    std::optional<std::pair<size_t, size_t>> block_part) const {
  if (pos.nPos < STORAGE_HEADER_BYTES) {
    // If nPos is less than STORAGE_HEADER_BYTES, we can't read the header that
    // precedes the block data This would cause an unsigned integer underflow
    // when trying to position the file cursor This can happen after pruning or
    // default constructed positions
    LogError("Failed for %s while reading raw block storage header",
             pos.ToString());
    return util::Unexpected{ReadRawError::IO};
  }
  AutoFile filein{OpenBlockFile({pos.nFile, pos.nPos - STORAGE_HEADER_BYTES},
                                /*fReadOnly=*/true)};
  if (filein.IsNull()) {
    LogError("OpenBlockFile failed for %s while reading raw block",
             pos.ToString());
    return util::Unexpected{ReadRawError::IO};
  }
  ...
  try {
    ...
    if (blk_start != GetParams().MessageStart()) {
      LogError("Block magic mismatch for %s: %s versus expected %s while "
               "reading raw block",
               pos.ToString(), HexStr(blk_start),
               HexStr(GetParams().MessageStart()));
      return util::Unexpected{ReadRawError::IO};
    }

    if (blk_size > MAX_SIZE) {
      LogError("Block data is larger than maximum deserialization size for %s: "
               "%s versus %s while reading raw block",
    ...
    if (block_part) {
      ...
      if (size == 0 || SaturatingAdd(offset, size) > blk_size) {
        return util::Unexpected{
            ReadRawError::BadPartRange}; // Avoid logging - offset/size come
                                         // from untrusted REST input
      }
    ...
    std::vector<std::byte> data(
        blk_size); // Zeroing of memory is intentional here
    filein.read(data);
    return data;
  } catch (const std::exception &e) {
    LogError("Read from block file failed: %s for %s while reading raw block",
             e.what(), pos.ToString());
    return util::Unexpected{ReadRawError::IO};
  }
}
...
FlatFilePos BlockManager::WriteBlock(const CBlock &block, int nHeight) {
  ...
  FlatFilePos pos{FindNextBlockPos(block_size + STORAGE_HEADER_BYTES, nHeight,
                                   block.GetBlockTime())};
  if (pos.IsNull()) {
    LogError("FindNextBlockPos failed for %s while writing block",
             pos.ToString());
    return FlatFilePos();
  }
  ...
  if (file.fclose() != 0) {
    LogError("Failed to close block file %s: %s", pos.ToString(),
             SysErrorString(errno));
    m_opts.notifications.fatalError(
        _("Failed to close file when writing block."));
    return FlatFilePos();
  }
...
static auto InitBlocksdirXorKey(const BlockManager::Options &opts) {
  ...
  if (fs::exists(xor_key_path)) {
    ...
    AutoFile xor_key_file{
        fsbridge::fopen(xor_key_path,
#ifdef __MINGW64__
                        "wb" // Temporary workaround for
  ...
  if (!opts.use_xor && obfuscation != decltype(obfuscation){}) {
    throw std::runtime_error{
        strprintf("The blocksdir XOR-key can not be disabled when a random key "
                  "was already stored! "
...
BlockManager::BlockManager(const util::SignalInterrupt &interrupt, Options opts)
    : m_prune_mode{opts.prune_target > 0},
      m_obfuscation{InitBlocksdirXorKey(opts)}, m_opts{std::move(opts)},
      m_block_file_seq{FlatFileSeq{
          m_opts.blocks_dir, "blk",
          m_opts.fast_prune ? 0x4000 /* 16kB */ : BLOCKFILE_CHUNK_SIZE}},
      m_undo_file_seq{
          FlatFileSeq{m_opts.blocks_dir, "rev", UNDOFILE_CHUNK_SIZE}},
  ...
  if (m_opts.block_tree_db_params.wipe_data) {
    ...
    m_blockfiles_indexed = false;
    // If we're reindexing in prune mode, wipe away unusable block files and all
    // undo data files
    if (m_prune_mode) {
...
void ImportBlocks(ChainstateManager &chainman,
  ...
  if (!chainman.m_blockman.m_blockfiles_indexed) {
    ...
    // reindex); parent hash -> child disk position, multiple children can have
    // the same parent.
